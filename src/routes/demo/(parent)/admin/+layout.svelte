...
const primaryNavItems = [
	{ href: '/demo/admin', label: 'ホーム', icon: '🏠' },
	{ href: '/demo/admin/activities', label: '活動', icon: '📋' },
	{ href: '/demo/admin/children', label: 'こども', icon: '👧' },
	{ href: '/demo/admin/points', label: 'ポイント', icon: '⭐' },
	{ href: '/demo/admin/settings', label: '設定', icon: '⚙️' },
];

function isNavActive(itemHref: string, currentPath: string): boolean {
	if (itemHref === '/demo/admin') return currentPath === '/demo/admin';
	...
	<header class="sticky top-10 z-30 bg-white/80 backdrop-blur border-b border-gray-200 px-4 py-3">
		<div class="max-w-4xl mx-auto flex items-center justify-between">
			...
			<div class="flex items-center gap-2">
				...
				>
					<span aria-hidden="true">&larr;</span> デモトップ
				</a>
			</div>
		</div>
	</header>
