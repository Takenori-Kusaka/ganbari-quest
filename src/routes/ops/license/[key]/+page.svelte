...
import { OPS_LICENSE_KEY_LABELS } from '$lib/domain/labels';
...
<div class="flex flex-col gap-6">
	<div>
		...
		>{OPS_LICENSE_KEY_LABELS.backLink}</a>
	</div>

	<Card padding="lg">
		<div class="flex justify-between items-start gap-4 flex-wrap">
			<div>
				<div class="text-xs text-[var(--color-text-muted)] mb-1">{OPS_LICENSE_KEY_LABELS.keyLabel}</div>
			...
			{:else}
				<span class="status-badge status-missing">{OPS_LICENSE_KEY_LABELS.missingRecord}</span>
			{/if}
		</div>
		...
		{#if record}
			<dl class="mt-6 grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
				...
				<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldPlan}</dt>
				<dd>{record.plan}</dd>

				<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldKind}</dt>
				<dd>{record.kind ?? 'purchase'}</dd>

				<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldIssuedAt}</dt>
				<dd>{new Date(record.createdAt).toLocaleString('ja-JP')}</dd>

				{#if record.expiresAt}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldExpiresAt}</dt>
					<dd>{new Date(record.expiresAt).toLocaleString('ja-JP')}</dd>
				{/if}

				{#if record.issuedBy}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldIssuedBy}</dt>
					<dd class="font-mono text-xs">{record.issuedBy}</dd>
				{/if}

				{#if record.consumedBy}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldConsumedBy}</dt>
					<dd class="font-mono text-xs">{record.consumedBy}</dd>
				{/if}
				{#if record.consumedAt}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldConsumedAt}</dt>
					<dd>{new Date(record.consumedAt).toLocaleString('ja-JP')}</dd>
				{/if}

				{#if record.revokedAt}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldRevokedAt}</dt>
					<dd>{new Date(record.revokedAt).toLocaleString('ja-JP')}</dd>
				{/if}
				{#if record.revokedReason}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldRevokedReason}</dt>
					<dd>{reasonLabel(record.revokedReason)}</dd>
				{/if}
				{#if record.revokedBy}
					<dt class="text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.fieldRevokedBy}</dt>
					<dd class="font-mono text-xs">{record.revokedBy}</dd>
				{/if}
			</dl>
		{:else}
			<p class="mt-4 text-sm text-[var(--color-text-muted)]">
				{OPS_LICENSE_KEY_LABELS.noRecordNote}
				</p>
		{/if}
		...
		{#if canRevoke}
			<div class="mt-6 pt-4 border-t border-[var(--color-border-light)]">
				...
				>
					{OPS_LICENSE_KEY_LABELS.revokeButton}
				</button>
			</div>
		{/if}
		...
		{#if form?.revoked}
			<div class="mt-4 p-3 bg-[var(--color-feedback-success-bg)] text-[var(--color-feedback-success-text)] rounded text-sm">
				{OPS_LICENSE_KEY_LABELS.revokedSuccess(reasonLabel(form.reason ?? ''))}
			</div>
		{/if}
		{#if form?.error}
...
{#if confirmOpen}
	...
	>
		<div class="modal" role="dialog" aria-modal="true" aria-labelledby="revoke-title">
			<h3 id="revoke-title" class="text-base font-semibold m-0 mb-3">{OPS_LICENSE_KEY_LABELS.modalTitle}</h3>
			<p class="text-sm text-[var(--color-text-muted)] mb-4">
				{OPS_LICENSE_KEY_LABELS.modalDesc}
			</p>
			...
			>
				<div class="mb-3">
					<NativeSelect
						name="reason"
						label={OPS_LICENSE_KEY_LABELS.reasonLabel}
				...
				<label class="flex flex-col gap-1 mb-4">
					<span class="text-xs text-[var(--color-text-muted)]">{OPS_LICENSE_KEY_LABELS.noteLabel}</span>
					<textarea
						name="note"
						bind:value={note}
						rows="3"
						placeholder={OPS_LICENSE_KEY_LABELS.notePlaceholder}
				...
				<div class="flex gap-2 justify-end">
					<button
						...
						disabled={submitting}
					>
						{OPS_LICENSE_KEY_LABELS.cancelButton}
					</button>
					<button
						type="submit"
						...
						disabled={submitting}
					>
						{OPS_LICENSE_KEY_LABELS.submitButton(submitting)}
					</button>
				</div>
			</form>
		</div>
