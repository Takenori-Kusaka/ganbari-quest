...
## demo 配下の `?screenshot` モード（#1164 / #1209 / #1893）

LP SS 撮影用に `/demo/**` で demo 固有 UI を一括非表示 + 本番一致演出を強制 ON。SSOT は `src/routes/demo/+layout.svelte` の 1 箇所のみ。`setScreenshotModeContext()` で context 配置、配下は `getScreenshotMode()` / `getScreenshotModeKind()` で参照。
