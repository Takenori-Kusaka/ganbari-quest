...
// #2319 / #3954 で設定はハブ + 7 サブグループ (アカウント / 活動・ポイント / 通知 / データ /
// ごほうび・ボーナスルール / サポート / プラン) に分割済み。旧ガイドはサブページ内の selector
// (pin-settings 等) を参照していたが、ハブページ (/admin/settings) には存在しないため、
// 本ページではハブのカード構成を案内する。
// step 1 は selector 省略で画面中央 modal 表示。② はハブ冒頭の説明カード (小要素)、③ は先頭の
// 設定カード (アカウント、小要素) を target にする。グリッド全体 (settings-hub = mobile で 7 枚
