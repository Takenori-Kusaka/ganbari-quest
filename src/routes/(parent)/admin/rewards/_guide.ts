...
export const REWARDS_GUIDE: PageGuide = {
	...
	steps: [
		...
		{
			id: 'rewards-add',
			// ごほうび追加 (プリセット選択 + 作成フォーム) は横幅広の見出し / グリッドが縦に連なるため、
			// 特定要素を highlight すると driver.js が非重複でバブルを配置できない (CI 縦長バブルで顕著、
			// mobile で特に)。中央 modal (selector 省略) で追加操作を説明し確実に非重複・viewport 内に収める。
