...
async function savePlan() {
	...
	try {
		...
		if (result.pointsAwarded > 0) {
			pointsMessage = `+${result.pointsAwarded}pt`;
			setTimeout(() => {
				pointsMessage = '';
			}, 3000);
		}
...
<div class="px-[var(--sp-md)] py-[var(--sp-sm)]">
	...
	{#if mode === 'select'}
		<div class="section">
			<h2 class="section-title">目指す分野を選択</h2>
			<p class="section-desc">興味のある職業・分野を選んでください</p>
			{#if data.careerFields.length === 0}
				<div class="text-center py-8">
					<span class="text-4xl block mb-2">🌟</span>
					<p class="text-lg font-bold" style="color: var(--color-text)">データ準備中...</p>
	...
	{:else if mode === 'edit'}
		<div class="section">
			...
			<h2 class="section-title">マンダラチャート</h2>
			<p class="section-desc">中央に目標を記入し、周囲に必要なアクションを書き出そう</p>
			<MandalaChart bind:chart={mandalaChart} readonly={false} />
		</div>

		<div class="section">
			<h2 class="section-title">タイムライン</h2>
			<p class="section-desc">将来のビジョンを時系列で整理しよう</p>
			<CareerTimeline
				bind:timeline3y
				bind:timeline5y
				bind:timeline10y
	...
	{:else if mode === 'view' && data.plan}
		...
		<div class="edit-area">
			<button class="edit-btn" onclick={startEdit}>編集</button>
		</div>
	{/if}
</div>
