...
function streakStatusText(achievement: (typeof data.achievements)[number]): string | null {
	...
	return `現在${achievement.liveStreak}日連続中`;
}
...
function milestoneDisplayText(achievement: (typeof data.achievements)[number]): string | null {
	...
	return `${achievement.highestUnlockedMilestone}達成`;
}
...
<div class="px-[var(--sp-md)] py-[var(--sp-sm)]">
	...
	<div class="flex items-center justify-center gap-[var(--sp-sm)] mb-[var(--sp-lg)]">
		...
		<p class="text-lg font-bold">
			{unlockedCount} / {data.achievements.length} 達成
		</p>
	</div>
				...
				>
					...
					{#if achievement.repeatable && unlocked && achievement.highestUnlockedMilestone}
						<span class="text-[10px] font-bold text-[var(--theme-accent)]">
							{achievement.highestUnlockedMilestone}{achievement.conditionType === 'streak_days' ? '日' : ''}
						</span>
					{/if}
				...
				{#each data.customAchievements as ca (ca.id)}
					<div class="flex items-center gap-[var(--sp-sm)] p-[var(--sp-sm)] rounded-[var(--radius-md)] border-2 {ca.unlockedAt ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200 bg-gray-50'}">
						...
						<div class="flex-1 min-w-0">
							...
							{#if ca.unlockedAt}
								<p class="text-xs text-yellow-600 font-bold">達成！ +{ca.bonusPoints}P</p>
							{:else}
								<p class="text-[10px] text-[var(--color-text-muted)]">目標: {ca.conditionValue}</p>
							{/if}
						</div>
					</div>
				{/each}
		...
		<div class="flex flex-col items-center gap-[var(--sp-md)] text-center">
			...
			{#if unlocked && selectedAchievement.unlockedAt}
				<p class="text-xs text-[var(--color-text-muted)]">
					{new Date(selectedAchievement.unlockedAt).toLocaleDateString('ja-JP')} に達成
				</p>
			{/if}
		</div>
	{/if}
