...
<svelte:head>
	<title>将来の夢 - がんばりクエスト</title>
</svelte:head>

<div class="px-[var(--sp-md)] py-[var(--sp-sm)]">
	<h1 class="page-title">将来の夢</h1>
	...
	{#if mode === 'select'}
		<div class="section">
			...
			<p class="section-desc">すきなお仕事をえらんでね！</p>
			{#if data.careerFields.length === 0}
				<div class="text-center py-8">
					<span class="text-4xl block mb-2">🌟</span>
					<p class="text-lg font-bold" style="color: var(--color-text)">お仕事をじゅんび中...</p>
					<p class="text-xs mt-1" style="color: var(--color-text-muted)">おうちの人におねがいしてね</p>
				</div>
			{:else}
				<CareerFieldSelector
		...
		<div class="section">
			{#if selectedField}
				<div class="selected-field-banner">
					...
					<button class="change-btn" onclick={() => (mode = 'select')}>かえる</button>
				</div>
			{/if}

			<h2 class="section-title">夢のマンダラチャート</h2>
			<p class="section-desc">まん中に「夢」を書いて、まわりに「やること」を書こう！</p>
			<MandalaChart bind:chart={mandalaChart} readonly={false} />
		</div>

		<div class="section">
			<h2 class="section-title">未来のタイムライン</h2>
	...
	{:else if mode === 'view' && data.plan}
		...
		<div class="edit-area">
			<button class="edit-btn" onclick={startEdit}>へんしゅうする</button>
		</div>
	{/if}
</div>
