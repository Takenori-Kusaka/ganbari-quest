...
// 通知経路が無いままそう書けば **顧客に嘘をつく**ことになる (#4366 merge 時点は
// `!isAiAvailable()` 分岐が log を 1 行も出さない完全な silent だった)。そこで
// 「AI が使えない状態にある」ことを 1 行だけ log に出し、`infra/lib/ops-stack.ts` の
// MetricFilter + Alarm で運営に届ける。
//
