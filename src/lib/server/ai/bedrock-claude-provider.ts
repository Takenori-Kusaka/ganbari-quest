 ...
 *
 * 分散するのは**推論処理**であって保存先ではない。米国内 3 リージョンはいずれも運営者の
 * AWS アカウント内で、#4367 の主目的 (子供の活動テキストを AWS の外に出さない) は保たれる。
 * `global.` profile は米国外を含みうる = privacy の移転先国「米国」が崩れるため採らない。
 *
 * 「Bedrock は入力を保存しない」の一次情報は #4367 の比較表 (AWS 公式ドキュメントの引用) に
 * ある。**本 file の記述はそこからの参照であって、ここで独自に断定しているのではない** —
 * 前提が変わったら #4367 の比較表を更新し、privacy の開示 (第 3 条 / 第 10 条) と併せて見直す。
 *
