	...
	private async acceptInviteForUser(
		...
		try {
			...
			if ('error' in result) {
				...
				// #3555 ①: 受諾の拒否は受諾者に理由を伝えないと dead-end になる
				// (この後 fallback の新規テナント自動作成が走り、無説明の空 admin に着地する)。
				// 1 回限りの通知 cookie を積み、admin +layout が読み取って案内バナーを表示する。
				// #4704: 対象理由の集合は validation/auth.ts の SSOT で判定する (手書き allowlist は
				// 新しい失敗理由 MEMBER_LIMIT_REACHED を取りこぼし、案内が出ないまま dead-end になった)。
				if (isInviteAcceptErrorCookieValue(result.error)) {
