...
//   `isRetainedSuspendedContract`（`contract-state-matrix.md` §4 の 4 列が SSOT）
...
export async function cleanupExpiredData(
	...
	for (const tenant of tenants) {
		try {
			...
			if (isRetainedSuspendedContract(tenant)) {
				result.tenantsSkipped++;
				logger.info('[retention-cleanup] tenant skipped (contract retained)', {
					service: 'retention-cleanup',
					tenantId: tenant.tenantId,
					context: { reason: 'suspended-with-subscription', status: tenant.status, dryRun },
				});
				continue;
			}
