...
function getDemoConditionLabel(conditionType: string, conditionValue: number): string {
	switch (conditionType) {
		case 'category_deviation':
			return `へんさち${conditionValue}いじょう`;
		case 'streak_days':
		...
		case 'total_logs':
			return `${conditionValue}かい かつどう`;
		case 'avg_status':
			return `へんさちへいきん${conditionValue}いじょう`;
		default:
			return '';
	}
}
...
export function getDemoTitlesData(childId: number): TitleWithStatus[] {
	...
	return DEMO_TITLES.map((t) => {
		...
		return {
			...
			isActive,
			conditionLabel: getDemoConditionLabel(t.conditionType, t.conditionValue),
