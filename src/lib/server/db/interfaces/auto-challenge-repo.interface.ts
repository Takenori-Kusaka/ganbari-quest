...
export interface IAutoChallengeRepo {
	findActiveChallenge(childId: number, today: string, tenantId: string): Promise<AutoChallenge | null>;
	...
	updateChallenge(challengeId: number, input: UpdateAutoChallengeInput, tenantId: string): Promise<void>;
