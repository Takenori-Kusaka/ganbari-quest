...
export const SQL_CREATE_TABLES = `
	...
	CREATE TABLE IF NOT EXISTS parent_messages (
		...
		shown_at TEXT,
		-- #2267 (EPIC #2266): 応援機能 (cheer) で付与したボーナスポイント (reward_notice のみ使用)
		bonus_points INTEGER,
		-- #2267 (EPIC #2266): 応援機能 (cheer) のカテゴリ (reward_notice のみ使用)
	...
	CREATE INDEX IF NOT EXISTS idx_cloud_exports_status
		ON cloud_exports(status);
	...
	CREATE TABLE IF NOT EXISTS graduation_consent (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		tenant_id TEXT NOT NULL,
		nickname TEXT NOT NULL,
	...
	CREATE INDEX IF NOT EXISTS idx_graduation_consent_tenant
		ON graduation_consent(tenant_id);
	CREATE INDEX IF NOT EXISTS idx_graduation_consent_consented_date
