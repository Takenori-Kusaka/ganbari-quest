...
//   2. ASYNC index の直後に sys.jobs INDEX_BUILD=completed を poll (検証 3)。
//      → CREATE TABLE → ASYNC UNIQUE CREATE → job=completed poll → 書込開放 の順序を保証。
...
export async function applyDsqlMigrationPlan(
	...
	for (const stmt of plan.ddl) {
		...
		if (stmt.asyncIndexName) {
			...
			await pollAsyncIndexBuild(executor, stmt.asyncIndexName, opts);
		}
	}
