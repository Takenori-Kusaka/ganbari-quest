...
<div class="bg-[var(--color-surface-card)] rounded-lg shadow-sm {activity.isVisible ? '' : 'opacity-50'}">
	<div class="px-3 py-2 flex items-center gap-3">
		<CompoundIcon icon={activity.icon} size="md" />
		<div class="flex-1 min-w-0">
			<p class="text-sm font-bold text-[var(--color-text)] truncate">{getActivityDisplayNameForAdult(activity)}</p>
