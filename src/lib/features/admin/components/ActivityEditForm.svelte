...
<div class="border-t px-3 py-3 space-y-3 bg-[var(--color-surface-muted)] rounded-b-lg">
	...
	>
		...
		<div class="flex gap-2">
			...
			<button
				type="button"
				class="px-4 py-2 bg-[var(--color-feedback-error-bg-strong)] text-[var(--color-feedback-error-text)] rounded-lg font-bold text-sm hover:bg-[var(--color-feedback-error-bg-strong)] transition-colors"
	...
	{#if deleteConfirmId === activity.id}
		<div class="bg-[var(--color-feedback-error-bg)] border border-[var(--color-feedback-error-border)] rounded-lg p-3 space-y-2">
			...
			<div class="flex gap-2">
				...
				>
					<input type="hidden" name="id" value={activity.id} />
					<button type="submit" class="w-full py-2 {logCount > 0 ? 'bg-[var(--color-warning)] hover:bg-[var(--color-warning)]' : 'bg-[var(--color-action-danger)] hover:bg-[var(--color-action-danger)]'} text-white rounded-lg font-bold text-sm transition-colors">
