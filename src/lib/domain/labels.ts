...
export const AUTH_INVITE_LABELS = {
	...
	// #4704: 受諾時の席数検査 (MEMBER_LIMIT_REACHED) で拒否されたときの案内バナー。
	// 案内が無いと「無説明のまま新しい空の家族グループの owner にされる」dead-end になる。
	acceptErrorMemberLimitBanner:
		'ご家族の人数が上限に達しているため招待を受諾できず、新しい家族グループが作成されました。招待した方にプランのご変更、または使われていない招待の取り消しをご相談ください。',
 ...
 * 案内が無い理由が混ざると、顧客は「なぜか知らない空の家族グループの owner になっている」
 * 状態に無説明で着地する (#4704 で MEMBER_LIMIT_REACHED を足したとき、旧実装の
