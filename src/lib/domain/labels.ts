...
export const PAGE_GUIDE_LABELS = {
	...
	adminSettingsNotifications: {
		...
		steps: {
			'settings-notifications-intro': {
				...
				how: '上でこのブラウザの通知をオン・オフし、下で受け取るお知らせの種類と、送らない時間帯を決めます。',
			...
			'settings-notifications-types': {
				...
				what: '受け取るお知らせを選びます。いまお届けしているのは「達成通知（記録完了・レベルアップ時）」です。お子さまが活動を記録したときと、レベルが上がったときに届きます。',
				how: '1. 「達成通知（記録完了・レベルアップ時）」のチェックを入れる／外す\n2. 下の「通知設定を保存」を押します',
				goal: '記録のたびにお知らせが届き、離れていてもお子さまのがんばりに気づけます。',
			},
			// ④ サイレント時間帯 (常設)
	...
	adminStatus: {
		...
		steps: {
			'status-intro': {
				title: 'このページについて',
				what: 'お子さまの活動を「うんどう・べんきょう・せいかつ・こうりゅう・そうぞう」の5つの軸で可視化するページです。どの分野が得意で、どこが伸びしろかが分かります。見出しに表示中のお子さまの名前が出ます。',
...
export const SETTINGS_LABELS = {
	...
	// #4664 F5: リマインダー / ストリーク警告 は送信するスケジューラが存在せず (endpoint は
	//   あるが schedule-registry.ts にも cron-dispatcher にも登録が無い)、チェックしても
	//   通知は届かない。届かないものを設定画面で約束しない (ADR-0013 と同型) ため、
	//   実配信が入るまで UI から外す。値そのもの (notification_reminders_enabled 等) は
	//   保存したまま触らないので、配信を実装すれば設定は復元される。
