...
export const LP_FAQ_PHASEB_LABELS = {
	...
	// #4713: 招待ロールは 保護者 / こども の 2 択で「閲覧権限」ロールは存在しない。
	//   閲覧のみの共有は premium の別機能 (VIEWER_LINK_TERMS) に誘導する。
	//   #4500 が「招待メンバーからのコメント / スタンプ送付は未実装」と確定させたため、
	//   本文自体は #4500 の記述を採り、機能名だけ atom 参照に置き換える。
	k108: `招待されたメンバーは保護者として、${CHILD_TERMS.honorific}の記録の確認と活動の記録ができます（アカウントを持たずに記録を見せたい場合は、${PLAN_FULL_TERMS.premium}の${VIEWER_LINK_TERMS.name}をお使いください。閲覧専用です）。`,
