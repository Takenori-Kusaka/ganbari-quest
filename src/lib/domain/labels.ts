...
export const PLAN_GATE_LABELS = {
	 ...
	 *
	 * #4622: 引数を `number` に狭めた。`checkChecklistTemplateLimit` の `max` は
	 * `PlanLimitCheck` の discriminated union により `allowed: false` 側で `number` に確定する。
	 * null を渡す呼び出し (= 「1人あたり null 個までです」) はコンパイルで落ちる。
	 *
