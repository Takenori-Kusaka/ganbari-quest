...
export const PLAN_GATE_LABELS = {
	 ...
	 *
	 * #4622: 引数は `number` のみ。`PlanLimitCheck` が discriminated union になり、
	 * `!allowed` 側で `max` は number に確定するため、`null` を受けられる型にしておくと
	 * 「お子さま1人あたり null 個までです」を型検査が通してしまう (旧 checklistTemplate*
	 * 系 label と重複していた定義は本 SSOT に統合した)。
	 */
	...
	// `perChildLimitReached` / `perChildLimitReachedShort` が SSOT (#4512 でプラン名を
	// 「無料プラン」に統一済)。#4622 で別名 (checklistTemplateLimitReached*) を作っていたが、
	// 同一文言の二重定義になるため本 SSOT に統合した。
