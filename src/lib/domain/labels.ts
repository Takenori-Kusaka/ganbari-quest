...
export const SUBSCRIPTION_PAGE_LABELS = {
	...
	//
	// #4540 Q4 の保持期間告知と併記する。両者は**別の機構**であり、片方だけだと嘘になる:
	//   - 上限超過分 (子供 / 活動 / チェックリスト) = `downgrade-service.ts` の archive。
	//     削除されず `restoreArchivedResources` で戻る → ARCHIVE_RESTORE_ASSURANCE
	//   - 保持期間を過ぎた履歴 = `plan-limit-service.ts` の historyRetentionDays による物理削除。
	//     戻らない (ADR-0049) → FREE_PLAN_RETENTION_NOTICE
	// 「戻せるもの」→「戻せないもの」の順に並べ、最後が削除の事実で終わるようにする
	// (顧客に有利に見える方向で締めない)。
