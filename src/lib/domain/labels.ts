...
export const PAGE_TITLES = {
	...
	// #4671 F3 / #4715: 呼称は画面名 registry (中身は CHALLENGE_TERMS.canonical) に統一
...
export const REWARDS_LABELS = {
	...
	// #4656 F5 / #4715: 呼称 + icon は画面名 registry (中身は CONCEPT_ICONS.reward + ごほうび管理)。
	//   ガイド title と同一表記。
...
export const ADMIN_HOME_LABELS = {
	...
	// #4653 F6 / #4715: 旧「管理ダッシュボード」は画面上のどこにも無い語だった。
	//   呼称は画面名 registry (= `<title>` と同一の「ご家族の見守り画面」) に一本化する。
	//   グローバルナビの「ホーム」タブは home 画面の nav ラベル対象外 (fitness function も除外)。
...
export const ADMIN_CHECKLISTS_PAGE_LABELS = {
	...
	// #4715: nav / `<title>` / 見出しは画面名 registry (中身は CHECKLIST_ADMIN_TERMS.pageTitle) を引く
