...
export const SUBSCRIPTION_PAGE_LABELS = {
	...
	// 順序: いま何が起きているか → 復旧の案内 → 契約が終わったあとの保持期間 (末尾)。
	// S3 (grace_period) は licenseStatus=ACTIVE のまま有料 tier が維持され、表示も絞られないので
	// (c) の注記は S4 だけに置く。
	paymentSuspendedDesc: `お支払いを確認できないため、有料プランの機能を止めています。${WRITES_CONTINUE_ASSURANCE}ご契約が残っているあいだ、これまでの記録を削除することはありません。ただし${PLAN_FULL_TERMS.free}と同じ範囲でしか表示されないため、その期間を超えた記録は一時的に見えなくなります。お支払い方法を更新すると有料プランの機能に戻り、見えなくなっていた記録もまた表示されます。ご契約が終了したあとは、次の保持期間が適用されます。${FREE_PLAN_RETENTION_NOTICE}`,
	/** S5 契約終了 (解約確定) */
...
export const LP_LEGAL_PRIVACY_LABELS = {
	articleHeader: '<h1>プライバシーポリシー</h1><p class="meta">最終更新日: 2026年9月4日</p>',
	intro:
	...
	effective: '<p>以上</p><p>制定日: 2026年3月27日</p><p>最終改定日: 2026年9月4日</p>',
} as const;
