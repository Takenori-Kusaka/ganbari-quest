...
// #4704: 受諾失敗の理由コード。案内文の網羅を型で強制するために型だけを引く
import type { InviteAcceptErrorCode } from './validation/auth';
...
export const AUTH_INVITE_LABELS = {
	...
	/**
	 * #3555 ① / #4704: 受諾失敗 → 新規家族グループ自動作成後に admin 画面で表示する案内バナー。
	 *
	 * `Record<InviteAcceptErrorCode, string>` なので、**受諾の失敗理由を増やすと案内文を書くまで
	 * コンパイルが通らない**。理由を足したのに案内が無いと、顧客は「なぜか知らない空の家族グループの
	 * owner になっている」状態に無説明で着地する (#4704 で MEMBER_LIMIT_REACHED を足したとき、
	 * 旧実装の 2 件 allowlist はこの経路を素通りさせていた)。
	 * どの文も「なぜ参加できなかったか」+「次に何をすればよいか」を必ず含める。
	 */
	acceptErrorBanners: {
		...
		MEMBER_LIMIT_REACHED:
			'招待した方のプランで参加できる人数の上限に達していたため参加できず、新しい家族グループが作成されました。招待した方にプランの見直し、または使っていない招待の取り消しをお願いしてください。',
		INVALID_OR_EXPIRED:
			'招待の有効期限が切れていたため参加できず、新しい家族グループが作成されました。招待した方に新しい招待を発行してもらってください。',
		ALREADY_IN_TENANT:
			'すでにこの家族グループに参加しているため招待は使われず、新しい家族グループが作成されました。元の家族グループのアカウントでログインし直してください。',
	} satisfies Record<InviteAcceptErrorCode, string>,
} as const;
