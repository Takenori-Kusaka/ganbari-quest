...
export const PAGE_GUIDE_LABELS = {
	...
	adminSettings: {
		...
		steps: {
			...
			'settings-account': {
				title: `よく使う操作（アカウント・${OYAKAGI_TERMS.shortName}）`,
				what: `最初に確認したいのが「アカウント」カードです。ここから${OYAKAGI_TERMS.shortName}（4桁の数字）の変更ができ、お子さまが誤って${ADMIN_VIEW_TERMS.short}に入るのを防げます。`,
				how: `1. 「アカウント」カードをタップ\n2. ${OYAKAGI_TERMS.shortName}の変更画面で、現在のコードと新しいコードを入力\n3. 「変更する」をタップ`,
				goal: `${OYAKAGI_TERMS.shortName}が更新され、次回から新しいコードが必要になります。データのバックアップは「データ」カードから行えます。`,
				tips: [
					`${OYAKAGI_TERMS.shortName}の初期値や、ポイント表示・通貨設定は各カードの中で変更できます`,
				],
			},
		},
	},
	...
	marketplace: {
		...
		steps: {
			'marketplace-intro': {
				...
				how: 'まず一覧から気になるテンプレートを探します。カードをタップすると詳細が開き、そこから「使ってみる」で取り込み、配信するお子さまを選びます。',
				goal: '選んだテンプレートが活動管理・ごほうび管理・チェックリストに追加され、ご家庭に合わせて項目を足したり消したりして調整できます。',
			},
			'marketplace-browse': {
				title: 'よく使う操作（テンプレートを探す）',
				what: '上部の種類（活動セット・ごほうびセット・チェックリスト）や、年齢・タグでテンプレートを絞り込めます。気になるものはカードをタップして詳細を確認します。',
				how: '1. 種類のカードをタップして絞り込みます\n2. 年齢・タグでさらに絞り込めます\n3. テンプレートのカードをタップして詳細を開きます\n4. 詳細ページの「使ってみる」から取り込み、配信するお子さまを選びます',
				goal: 'お子さまの年齢や興味にぴったりのテンプレートが見つかり、数タップで活動やごほうびを増やせます。',
