...
export const LP_HERO_SPEC_BADGES_LABELS = {
	ageRange: '3〜18 歳',
	ageRangeSuffix: '対応',
	// #1788: 「プリセット済み」誤認回避のため「学年別の活動候補」へ刷新（実態は親がセットアップで選ぶ 300+ 候補プール）
	presetCount: '300+',
	presetSuffix: '学年別の活動候補から選べる',
	setupTime: '約 5 分',
	setupSuffix: 'で初期設定',
} as const;
...
// #1787 (2026-05-01): 4 階層 → 2 階層、6 STEP → 3 STEP に圧縮。
//   STEP 1/2 バッジ・親子両視点バナー・緑 sticky note を廃止し、
//   各 layer は「badge + h3 + 1 段落 + scrshot」の規範 B 整合構成へ。
//   `parentPerspective*` / `childPerspective*` / `l*Step*` namespace は site/index.html では参照しなくなるが、
//   pamphlet 等の互換性のため定義は残す。
// #1788 (2026-05-01): l1Desc / shopNote 等を honest 表現へ刷新（プリセット = 親が選ぶ候補プール）。
export const LP_CORELOOP_LABELS = {
	...
	// 親視点サブタイトル（#1787 で site/index.html では未使用に変更。pamphlet 等の互換維持で保持）
	parentPerspectiveTitle: '親の視点',
	parentPerspectiveDesc:
		'活動候補から選んで初期設定するだけで、子供の取り組みをポイントで定量把握できます。',
	// 子供視点サブタイトル（同上、site/ では未使用）
	childPerspectiveTitle: '子供の視点',
	childPerspectiveDesc: '活動を重ねてポイントを貯め、ごほうびショップで欲しいものと交換できます。',
	// 仕組み 1: 毎日の活動（#1787 で 1 段落構成、honesty 反映）
	l1Title: '毎日の活動 — がんばりを記録する',
	l1Desc:
		'「はみがきした」「宿題おわった」を 2 タップで記録するだけで、ポイントが積み上がります。活動は学年別の候補から親が選んでセットアップ（初回 5 分）。日々の声かけを「タップ 1 回」に置き換えられます。',
	// 旧 STEP 1/2（site/index.html では未使用、pamphlet / 過去訴求互換のため保持）
	l1Step1Title: '活動を 2 タップで記録',
	l1Step1Desc:
		'「はみがきした」「宿題おわった」をタップするだけ。学年別の活動候補から親があらかじめ選んでおいた項目を、子供がタップする運用です。',
	l1Step2Title: 'ポイントが獲得できる',
	l1Step2Desc: '記録した活動に応じてポイントが加算。「今日どれだけ頑張ったか」が数字で見えます。',
	// 仕組み 2: 習慣カード（おみくじスタンプ）— #1787 で 1 段落構成
	l2Title: '習慣カード — 毎日の楽しみで続ける',
	l2Desc:
		'1 日 1 回までのおみくじスタンプ。レア度に応じてボーナスポイントが付き、毎日記録する習慣が自然に身につきます（煽らない設計）。1 週間続けるとスタンプカードが 1 枚完成し、三日坊主を防ぐ「継続の見える化」になります。',
	// 旧 STEP 1/2（site/index.html では未使用、互換のため保持）
	...
	// 仕組み 3: ごほうび交換 — #1787 で 1 段落構成
	l3Title: 'ごほうび交換 — ポイントを欲しいものに換える',
	l3Desc:
		'貯まったポイントはごほうびショップが唯一の出口。実物のプレゼント・お小遣い・特権（夜ふかし権など）を親が設定し、子供が自分で選んで交換します。「あと 50 ポイントで欲しいごほうび」と、子供自身が計画を立てるようになります。',
	// 旧 STEP 1/2（site/index.html では未使用、互換のため保持）
	...
	// 1-shot summary 画像 (#1787) のキャプション
	summaryAlt: '活動 → 習慣 → ごほうび の 3 つの仕組みが循環する図',
	summaryNote:
		'活動 → 習慣 → ごほうび の 3 つの仕組みが循環することで、毎日のがんばりが本物の報酬として戻ってきます。',
	// pamphlet用短文
	pamphletNote:
...
export const LP_FAQ_LABELS = {
	...
	text19:
		'プラン別の猶予期間（読み取り専用）— 無料: 即時削除 / スタンダード: 7 日 / ファミリー: 30 日',
	...
	text27:
		'プラン別の猶予期間中（無料: 不可 / スタンダード: 7 日 / ファミリー: 30 日）であれば、管理画面から解約申請を取り消して有料プランを継続できます。',
	text28:
...
export const LP_INDEX_EXTRA_LABELS = {
	...
	k20: '「はをみがく」「おかたづけ」など、幼児期に身につけたい活動が候補として用意されています。セットアップで好きなものを選ぶだけ。ログインボーナスで毎日おみくじが 1 回引けて、スタンプカードが週末に 1 枚完成します。',
	k21: 'デモを見る',
	...
	// 旧 k32-k51 を再構成: 旧 5 工夫から 3 工夫（長期 / 朝準備 / RPG）に圧縮、indexExtra namespace は新 LP では未参照だが SSOT 一貫性のため整合
	k32: 'コアループを支える 3 つの工夫',
	k33: '毎日の「活動 → 習慣 → ごほうび」の冒険を、長期の達成感・朝の準備・冒険のクライマックスから支える 3 つの工夫です。',
	k34: '長期 → 朝の準備 → 冒険のクライマックスの 3 つの工夫が、日々のがんばりを支えます。',
	k35: '&#9312; 長期の達成感',
	k36: '実績 &amp; 称号',
	k37: '「はじめのぼうけんしゃ」から「でんせつのゆうしゃ」へ。長期の積み重ねが称号として永久に刻まれ、子供の自己肯定感を支えます。',
	k38: '&#9313; 朝の準備をスムーズに',
	k39: '持ち物チェックリスト',
	k40: '通学や習い事の持ち物を、子ども自身がタップ確認。朝の「あれ持った？」を減らします。',
	k41: '&#9314; 冒険のクライマックス',
	k42: 'RPG バトル',
...
export const LP_INDEX_PHASEB_LABELS = {
	...
	k12: '「はをみがく」「おかたづけ」など、幼児期に身につけたい活動が候補として用意されています。セットアップで好きなものを選ぶだけ。ログインボーナスで毎日おみくじが 1 回引けて、スタンプカードが週末に 1 枚完成します。',
	...
	k21: 'コアループを支える 3 つの工夫',
	k22: '毎日の「活動 → 習慣 → ごほうび」の冒険を、長期の達成感・朝の準備・冒険のクライマックスから支える 3 つの工夫です。',
	k23: '&#9312; 長期の達成感',
	k24: '実績 &amp; 称号',
	...
	k37: '親が安心できる運用補助',
	k38: 'ゲーミフィケーションの裏で、親がちゃんと伴走できる設計。「遊ばせっぱなし」「設定が大変そう」の不安を取り除く 3 つの機能です。',
	k39: '成長の記録（月次レポート）',
	...
	softFamilySupportDesc:
		'設定時間で自動スリープし長時間利用を防止。必要な時だけ家族からおうえんメッセージを子供のホームへ届けられます。',
	softBenefitFamilySupport:
		'<strong>親が観測できること</strong>: 設定時間で自動スリープが働き、おうえんメッセージは子供が読むと既読が付く',
} as const;
...
export const LP_FAQ_PHASEB_LABELS = {
	...
	k19: '管理画面の「プラン・お支払い」→「解約」からいつでも解約できます。解約を申請すると、ご利用プランに応じた読み取り専用の<strong>猶予期間</strong>に入ります（無料プラン: 即時削除 / スタンダードプラン: 7 日間 / ファミリープラン: 30 日間）。',
	...
	k27: 'プラン別の猶予期間中（無料: 不可 / スタンダード: 7 日 / ファミリー: 30 日）であれば、管理画面から解約申請を取り消して有料プランを継続できます。',
...
export const LP_LEGAL_PRIVACY_LABELS = {
	...
	section6:
		'<h2>第6条（データの削除）</h2><ol><li><strong>個別データの削除</strong>: 特定の活動記録やお子さまの情報の削除は、本サービスの管理画面から即時実行できます。</li><li><strong>アカウント全体の削除</strong>: アカウント削除を申請後、ご利用プランに応じた猶予期間を設けます（無料プラン: 即時削除 / スタンダードプラン: 7日間 / ファミリープラン: 30日間）。猶予期間中は削除の取消しが可能です。</li><li><strong>バックアップからの完全消去</strong>: アカウント削除後90日以内に、バックアップデータからも完全に消去されます。</li></ol>',
	section6_2:
...
export const LP_LEGAL_TERMS_LABELS = {
	...
	section13:
		'<h2>第13条（利用者データの取扱い）</h2><ol><li>利用者は、自己のコンテンツについて、いつでも削除を申請することができます。</li><li>アカウント削除を申請した場合、ご利用プランに応じた猶予期間（無料プラン: 即時削除 / スタンダードプラン: 7日間 / ファミリープラン: 30日間）の後、全データが完全に削除されます。猶予期間中は削除の取消しが可能です。</li><li>運営者はデータのバックアップを実施していますが、データの復旧を保証するものではありません。</li></ol>',
	section14:
