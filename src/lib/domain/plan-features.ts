...
export const PRICING_PAGE_FEATURES: Record<PlanKey, readonly string[]> = {
	free: [
		...
		// #1912 (F-6): ギャンブル系語彙「ログインボーナス・連続達成ボーナス」→「毎日のごほうび・続けるごほうび」へ
		...
		// 検討時点で見えている必要がある。止まるのはオリジナルの登録だけで、プリセットは全プラン可
		// (実ゲートは isCustomRewardUnlocked、#4584)。
		`${REWARD_TERMS.originalRegistration}は${PLAN_TERMS.standard}以上`,
	],
	standard: [
		'お子さまの登録人数：無制限',
		'オリジナル活動の作成：無制限',
		...
		// #4705 / #4928: 行名を実ゲート (isCustomRewardUnlocked、#4584) が止めている機能に揃える
		REWARD_TERMS.originalRegistration,
	...
	family: [
		...
		// #1912 (F-8): 「クラウド保管枠」→「家族のデータ預かり枠（自分でダウンロード可）」へ日本語化
		'家族のデータ預かり枠（同時保管 10 件・自分でダウンロード可）',
...
> = {
	...
	standard: [
		...
		{ text: REWARD_TERMS.originalRegistration, icon: '✅' },
