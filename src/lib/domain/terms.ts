...
export const AI_TRANSFER_TERMS = {
	...
	/** 領収書画像に印字されうる個人情報の例示 (#4598、プライバシーポリシー第9条④ と同じ事実) */
	receiptPrintedInfo: '宛名の氏名・住所',
	/**
	 * 送信先の別 (#4598 QM レビュー指摘)。`getAiProvider()` は `AI_PROVIDER` で送信先を切り替えるため、
	 * **同じ画面文言でも配布形態によって送信先が変わる**。第9条④ はその 2 ケースを書き分けており、
	 * 画面だけ「生成 AI」で丸めると条文が区別している事実を画面が消してしまう。
	 * 文言は条文と同じ粒度 (運営者の環境の内か外か) に留め、個別の製品名・モデル名は書かない (#4370)。
	 */
	destinationCloud: 'クラウド版では運営者が管理する環境内',
	destinationSelfHosted: 'セルフホスト版では設定により運営者の環境外',
} as const;
...
export const CROSS_BORDER_TERMS = {
	 ...
	 * privacy.html 第 10 条 (LEGAL_LABELS.section10) が本文を引用しているため、
	 * ここを変えると法務文書の実質変更 = 改定日 bump = 全利用者の再同意発火になる (#4944)。
	 */
	consentLabel: 'サービス提供に必要な範囲でのデータ保存・処理に同意します',

	// --- 第一層: 何が起きる / 起きない (#4944) ---
	//
	// 事業者名・国名・法律の条番号は第一層に置かない。読み手はそれを「何をされるのか」に
	// 変換できず、リスクだけが想起される (Nouwens 2020: ベンダ名リストは 68.6% の機会で無視 /
	// Balash 2024: カテゴリ語は理解されず、結果ベースの記述だけがリスク知覚を動かす)。
	// 法定記載 (移転先国 / 当該国の制度 / 移転先が講ずる措置、施行規則 17 条 2 項) は
	// 第二層 = privacy.html 第 10 条に置き、detailLink から 1 クリックで到達させる
	// (WP29 WP260 ¶36: 第一層には「本人に最も影響がある処理」を平易語で置く)。
	//
	// 各文は実装で裏が取れる事実に限る。守れない約束を書かない (ICO AADC Standard 4:
	// "does not promise protections or standards that are not routinely upheld")。
	//
	// **常時表示は 2 行に抑え、残りは折りたたむ** (PO 判断 2026-09-12)。同意画面は
	// 利用規約 / プライバシーポリシー / 本項の 3 ブロックが縦に並ぶため、本項だけ本文が長いと
	// **ブロックの大きさの差が重要度の差に見えてしまう**。3 ブロックの見た目の重さを揃えたうえで、
	// 詳しく読みたい人は 1 クリックで全文に到達できる形にする (ICO の layered approach)。

	/** 画面見出し。法律用語ではなく読み手の問いにする */
	sectionTitle: 'お預かりするデータの取り扱い',
	/** 常時表示 1 行目。起きることを短く肯定形で述べる (否定形を先頭に置かない) */
	summaryPositive: 'お子さまの記録とお写真は、暗号化してお預かりします。',
	/** 折りたたみの見出し。中に何があるかを述べる (「詳細」のような中立語にしない) */
	detailsSummary: 'ほかのご家庭との分離・お支払い・退会したときについて',
	/** 折りたたみ内。根拠: テナント分離 (ADR-0063) */
	whatHappens: 'ほかのご家庭から、お子さまの記録やお写真が見えることはありません。',
	/**
	 * 折りたたみ内。決済会社に渡る範囲。根拠: stripe-service.ts の Checkout Session は
	 * metadata (tenantId / planId) とプラン名だけを送り、customer_email を設定しない。
	 * メールアドレスとカード番号は利用者が Stripe のページへ直接入力する。
	 */
	paymentScope:
		'有料プランをお申し込みの場合、決済会社にお渡しするのは、プラン名と、ご家族を表す記号だけです。お名前・メールアドレス・カード番号は、当方のサーバーを通りません。',
	/**
	 * 常時表示 2 行目。起きないこと。
	 * privacy.html 第 10 条が「この説明とともに表示する」と書いている実体なので、
	 * **折りたたみの中に入れない**（畳まれた状態で「表示している」とは言いにくい）。
	 */
	noNoUse: '広告への利用・第三者への販売・AI の学習への流用は行いません。',
	/** 折りたたみ内。消せること。抽象的な保証ではなく操作と結果で書く。根拠: 退会時の物理削除 (DPIA §2.6) */
	deletion: '退会すると、お預かりしたデータはサーバーから削除されます。',
	/**
	 * 第二層への導線。何が読めるのかを書いたうえで、**同意の前に確認することを明示的に求める**。
	 *
	 * 施行規則 17 条 2 項の 3 情報を URL で提供する場合、PPC Q12-10 は
	 * 「当該 URL を本人にとって分かりやすい場所に掲載した上で、同意の可否の判断の前提として、
	 * 本人に対して当該情報の確認を明示的に求めるなど、本人が当該 URL に掲載された情報を
	 * 閲覧すると合理的に考えられる形で、情報提供を行う必要がある」としている。
	 * 「詳細」のような何が読めるか分からないラベルでは、この要件を満たさない。
	 */
	detailLink: 'ご同意の前に、保管している国と講じている対策をご確認ください',
} as const;
...
export const REWARD_TERMS = {
	...
	canonical: 'ごほうび',
	/**
	 * 有料プラン (スタンダード以上) で解放される機能の名前 (#4705 / #4915 / #4928 / #4992)。
	 *
	 * 実ゲート `isCustomRewardUnlocked` (#4584) が止めているのは次の 2 つ:
	 ...
	 * 値は PO 決裁 (#4992) の文言そのもの。変えるときは決裁を取り直す。
	 */
	originalCreateEdit: 'オリジナルのごほうびの作成・編集（ポイントの調整を含む）',
} as const;

// ============================================================
// BACKUP_TERMS — データのバックアップ/復元 用語 atom
// ============================================================
//
// アプリが書き出す「家族データのバックアップ」では内部フォーマット (JSON / ZIP) を
// 露出しない。「データをファイルに書き出して保存し、必要なときに戻す」体験を
// 「バックアップ」で統一する (家族データのエクスポート/インポート画面・LP・FAQ で共通参照)。
//
// 設計指針 (ADR-0045 atom / SSOT):
//   - canonical    : 'バックアップデータ' ( 書き出されるデータの呼称 )
//   - file         : 'バックアップファイル' ( 選択/復元するファイルの呼称 )
...
//
// ⚠️ CSV は「バックアップ」ではない (ADR-0013 LP truth):
...
// ============================================================
// ACTIVITY_QUOTA_TERMS — 無料プランの活動上限が数える対象の atom (#4693)
// ============================================================
//
// PO 回答 (2026-09-03、#4693): 上限の母集団は **親が手で作った活動だけ** で、プリセット
// (みんなのテンプレート) からの取込は上限を消費しない (`site/pricing.html` の
// 「プリセット活動の利用 = 無料 ✓」「オリジナル活動の作成 = 3 個まで」が契約文)。
// 旧文言「カスタム活動は最大 N 個まで」は「テンプレも入らない」と読めたため、上限メッセージは
// 「何が数えられ、何が数えられないか」を両方言う。数値は FREE_PLAN_QUOTA (plan-quota.ts) から引く。

export const ACTIVITY_QUOTA_TERMS = {
	/** 上限に数える対象 = 親が自分で作った活動 (LP 料金表の行名と同語) */
	original: 'オリジナル活動',
	/** 上限に数えない経路 = みんなのテンプレート / プリセットからの取込 */
	presetImport: 'プリセットからの取込',
} as const;
