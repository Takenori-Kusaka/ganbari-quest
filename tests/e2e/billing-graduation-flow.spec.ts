...
//
// 検証観点:
// - /admin/subscription/cancel/graduation が直接アクセス可能（解約フロー後の redirect 着地点）
...
test.describe('#1603 Graduation flow', () => {
	test('/admin/subscription/cancel/graduation が 200 で表示される', async ({ page }) => {
		const response = await page.goto('/admin/subscription/cancel/graduation');
		expect(response?.status()).not.toBe(500);
	});

	test('卒業ページに祝福見出し + 説明文が描画される', async ({ page }) => {
		await page.goto('/admin/subscription/cancel/graduation', { waitUntil: 'domcontentloaded' });
