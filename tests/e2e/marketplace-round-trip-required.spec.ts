...
test.describe('#2374 marketplace round-trip 必須化 (export → import、PO 指摘 ④ 構造的回帰防止)', () => {
	...
	test('activity-pack: 空 activities 配列を import すると failure 応答 (#2389 Copilot AC4)', async ({
		request,
	}) => {
		// SvelteKit form action 仕様 (option B):
		//   - `fail(400, { error: ... })` は HTTP **200** で返り、body に
		//     `{ type: 'failure', status: 400, data: { error: '...' } }` を埋め込む
		//     (Playwright `request.post` は browser fetch ではなく action POST 扱い)
		//   - body の error メッセージで「validation failure」を判定するのが正攻法
		//
		...
		//
		// (旧 spec は `[200, 400].includes(...)` で曖昧化していたが、コード経路と
		// SvelteKit 仕様の両確認で 200 + failure body が正解と判明したため #2389 で
		// 明確化、ADR-0006 アサーション弱体化禁止整合)
		...
		// fail() の error メッセージが body に埋め込まれる (`file-source.ts` の throw 文言)
		expect(body).toContain('インポートする活動がありません');
		// body は SvelteKit form action の failure 構造 (`type:"failure"` または status:400)
		expect(body).toMatch(/failure|"status":400/);
	});
});
