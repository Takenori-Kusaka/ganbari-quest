...
test.describe('#0051: 複数回実行', () => {
	...
	test('複数回実行可能な活動のUIにバッジが表示される', async ({ page }) => {
		...
		// #2471 (本 PR): 子供 home の load() を `getChildActivities(child.id, ...)` に
		// 切替えたため、たろうくん画面では たろうくん の per-child instance のみが render され、
		// 同名 activity の duplicate がゼロになった。strict-mode assertion (`.first()` なし)
		// で「同名 card が 1 件のみ visible」を ADR-0006 適合の正面検証に戻す。
		const multiButton = page
			.locator('[data-testid^="activity-card-"]')
			.filter({ hasText: 'おさらあらい' });
		await expect(multiButton).toHaveCount(1);
...
test.describe('#0122: アバターアップロード', () => {
	...
	test('PNG形式のアバターもテナントパスで保存される', async ({ request }) => {
		// Sharp 互換の有効な 1x1 PNG (sharp で生成済み)
		const pngData = Buffer.from([
			0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44,
			0x52, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x02, 0x00, 0x00, 0x00, 0x90,
			0x77, 0x53, 0xde, 0x00, 0x00, 0x00, 0x09, 0x70, 0x48, 0x59, 0x73, 0x00, 0x00, 0x03, 0xe8,
			0x00, 0x00, 0x03, 0xe8, 0x01, 0xb5, 0x7b, 0x52, 0x6b, 0x00, 0x00, 0x00, 0x0c, 0x49, 0x44,
			0x41, 0x54, 0x78, 0x9c, 0x63, 0xf8, 0xff, 0xff, 0x3f, 0x00, 0x05, 0xfe, 0x02, 0xfe, 0x0d,
