...
//   かつ「残りの step (requiredTier 無しの `challenges-create`) で
//   ガイドが成立する」ことは tier 別ケースが無く未検証だった
//   (filterGuideStepsByTier の enforcement 回帰が無い)。
//
//   challenges guide は #2896 で 2 部構成
//   (`challenges-intro` [requiredTier:'family'] / `challenges-create`) に整理されている
//   (marketplace 陳列を 3 type に絞った際、チャレンジ取込 step `challenges-marketplace` を撤去)。
//   free tier では family 限定の `challenges-intro` のみ除外され、非 gate の 1 step
//   (`challenges-create`) が残る。
//
...
//     - 非 gate step (`challenges-create`) が **出る** (ガイド自体は dead-end にならず成立する)
//     - 進捗の総数 (total) が 1 になり (「1 / 1」)、family 時 (2 step) から family 限定 1 件のみ
//       正しく除外されている
//   を assert する。filter が壊れて family step が漏れれば family 限定 step の検出 or 進捗の total (2) で
//   必ず fail する。
//
...
if (process.env.DEBUG_PLAN === 'free') {
	test.describe('#2919 page guide の requiredTier フィルタ (free tier で family 限定 step が除外される)', () => {
		...
		test('/admin/challenges: family 限定 step は free で非表示・非 gate step でガイドは成立する', async ({
			...
			// 進捗の総数 (`current / total` の total) が 1 に絞られている
			// (family 時の 2 step から family 限定 1 件 `challenges-intro` のみ除外)。
			// 唯一の step では progress は「1 / 1」。filter が壊れて family step が漏れれば total が 2 になり必ず fail する。
			await expect(page.locator('.guide-header-progress')).toHaveText('1 / 1');

			// family 限定 step (`challenges-intro`) は free のガイドに含まれない (filter enforcement の回帰検出)。
