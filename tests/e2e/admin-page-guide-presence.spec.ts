...
//   DEBUG_PLAN は process.env 駆動 (shared webServer 固定) のため、free 指定が無い既定実行では
//   skip する (cron / cognito 系 spec と同じ env-conditional skip 慣習、tests/CLAUDE.md)。
test.describe('#2919 page guide の requiredTier フィルタ (free tier で family 限定 step が除外される)', () => {
	test.setTimeout(60_000);

	test.skip(
		process.env.DEBUG_PLAN !== 'free',
		'DEBUG_PLAN=free 指定時のみ実行 (shared webServer は process.env 駆動のため、tier 別ケースは env 指定が前提)',
	);
	...
	test('/admin/challenges: family 限定 step は free で非表示・非 gate step でガイドは成立する', async ({
		...
		await expect(page.locator('.guide-bubble[data-step-id="challenges-overview"]')).toHaveCount(0);
