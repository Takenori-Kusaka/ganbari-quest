...
//   (challenges guide の `challenges-intro`) が下位 tier で「ガイドから除外される」こと、
//   かつ「残りの step (requiredTier 無しの `challenges-create` / `challenges-marketplace`) で
//   ガイドが成立する」ことは tier 別ケースが無く未検証だった
//   (filterGuideStepsByTier の enforcement 回帰が無い)。
//
//   challenges guide は #2962 で 3 部構成
//   (`challenges-intro` [requiredTier:'family'] / `challenges-create` / `challenges-marketplace`)
//   に統一されている。free tier では family 限定の `challenges-intro` のみ除外され、
//   非 gate の 2 step (`challenges-create` / `challenges-marketplace`) が残る。
//
//   本 test は free tier (`DEBUG_PLAN=free`、#758) で challenges ページのガイドを開き:
//     - family 限定 step `challenges-intro` が **全ステップを通して 1 度も出ない**
//       (上位プラン限定手順を free に見せない / filter enforcement)
//     - 非 gate step (`challenges-create` を起点に `challenges-marketplace` まで) が **出る**
//       (ガイド自体は dead-end にならず成立する)
//     - 進捗の総数 (total) が 2 になり (起点「1 / 2」→ 末尾「2 / 2」)、family 時 (3 step) から
//       family 限定 1 件のみ正しく除外されている
//   を assert する。filter が壊れて family step が漏れれば family 限定 step の検出 or 進捗の total (3) で
//   必ず fail する。
//
...
if (process.env.DEBUG_PLAN === 'free') {
	test.describe('#2919 page guide の requiredTier フィルタ (free tier で family 限定 step が除外される)', () => {
		...
		test('/admin/challenges: family 限定 step は free で非表示・非 gate step でガイドは成立する', async ({
			...
			// PageGuideBubble は常に「現在 step」1 件だけを DOM に描画する。free では family 限定の
			// `challenges-intro` が先頭から除外されるため、起点 step は非 gate の `challenges-create`。
			await expect(page.locator('.guide-bubble[data-step-id="challenges-create"]')).toBeVisible();

			// 進捗の総数 (`current / total` の total) が 2 に絞られている
			// (family 時の 3 step から family 限定 1 件 `challenges-intro` のみ除外)。
			// 起点 step では progress は「1 / 2」。filter が壊れて family step が漏れれば total が 3 になり必ず fail する。
			await expect(page.locator('.guide-header-progress')).toHaveText('1 / 2');

			// family 限定 step (`challenges-intro`) は free のガイドに含まれない。
			// 起点 (`challenges-create`) でも、Next で末尾 (`challenges-marketplace`) まで進めても
			// 1 度も出ないことを全ステップ通して確認する (filter enforcement の回帰検出)。
			await expect(page.locator('.guide-bubble[data-step-id="challenges-intro"]')).toHaveCount(0);

			await page.locator('.guide-nav-next').click();

			// 末尾の非 gate step (`challenges-marketplace`) が表示される (ガイドは成立する)。
			await expect(
				page.locator('.guide-bubble[data-step-id="challenges-marketplace"]'),
			).toBeVisible();
			// 末尾 (2 step 目) の進捗は「2 / 2」。total は依然 2 のまま (family step 漏れなし)。
			await expect(page.locator('.guide-header-progress')).toHaveText('2 / 2');
			// 末尾でも family 限定 step は依然として 1 件も漏れていない。
			await expect(page.locator('.guide-bubble[data-step-id="challenges-intro"]')).toHaveCount(0);
