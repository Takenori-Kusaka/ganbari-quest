...
test.describe('#2365 marketplace -> activity-pack -> import (PO 指摘 ① 直接解決)', () => {
	...
	test('#2745: activity-pack 取込完了時に Toast success feedback (件数動的表示) が表示される', async ({
		...
		// imported > 0 → 「○ 件の活動を追加しました」 (件数を含む数字 + 「活動」)
		// imported = 0 → 「すでに追加済みです」 (重複時の info toast)
		const hasSuccessPattern = /\d+\s*件の活動を追加しました/.test(toastText);
		const hasDuplicatePattern = /すでに追加済み/.test(toastText);
		expect(
			hasSuccessPattern || hasDuplicatePattern,
			`Toast 本文に success (件数 + 「活動を追加しました」) もしくは重複 message が含まれる必要あり: 実際は "${toastText}"`,
		).toBe(true);
	});
});
