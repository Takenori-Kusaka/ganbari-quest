...
test.describe('#2365 marketplace -> activity-pack -> import (PO 指摘 ① 直接解決)', () => {
	...
	test('#2745: activity-pack 取込完了時に Toast success feedback (件数動的表示) が表示される', async ({
		...
		// Step 3: success feedback 表示検証 (DESIGN.md §5 「操作完了の成功フィードバック」)
		//
		// 2 重防御 (#2745 fix Round 2 CI artifact 解析根拠):
		//   (A) Toast.svelte は `role="alert"` で出現する (`showToast()` 経由、3 秒自動消滅)。
		//   (B) in-page banner `data-testid="admin-activities-action-message"` は同期 set
		//        され handler return 直後の re-render で表示される (#2748 Round 1/2 で
		//        Toast 単独だと micro-task race + invalidateAll 後の DOM 反映遅延で 5s
		//        timeout する fail を CI artifact `error-context.md` で確認したため、
		//        banner を 2 重防御として追加した)。banner には `role="status"` を付与し
		//        ARIA live region としても feedback を届ける。
		//
		// imported > 0 の case (新規 child seed では grew) と imported = 0 (dedupe 全 skip)
		// の case 両方をカバーするため「○ 件の活動を追加しました」 / 「すでに追加済みです」
		// のいずれかが visible であることを assert する。両系統 (Toast / banner) のうち
		// 先に visible になった方の textContent が regex にマッチすれば PASS。
		const feedback = page
			.locator('[role="alert"], [data-testid="admin-activities-action-message"]')
			.first();
		await expect(feedback).toContainText(/\d+\s*件の活動を追加しました|すでに追加済み/, {
			timeout: 10_000,
		});
	});
});
