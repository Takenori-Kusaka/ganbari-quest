...
test.describe('#2362 PR-3 Phase 5 marketplace activity-pack 取込で childId 露出 0 件', () => {
	...
	test('AC4: response body / URL に child 情報露出 0 件 (CWE-598 privacy)', async ({ page }) => {
		...
		// #2455 Round 6: breadcrumb root は MARKETPLACE_LABELS.breadcrumbRoot = TEMPLATE_TERMS.short
		// = 'テンプレート' (atom #2276)。「みんなのテンプレート」は LP の userFacing 表記のため
		// detail page breadcrumb には現れない。
		const breadcrumb = page.locator('nav', { hasText: 'テンプレート' });
