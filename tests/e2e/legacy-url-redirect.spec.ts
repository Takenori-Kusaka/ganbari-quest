...
test.describe('#578 旧 URL の中央リダイレクト', () => {
	// ============================================================
	// PIN 忘れ救済の方式置換 (#2993): email magic link → パスワード再入力
	// ============================================================
	test('/auth/forgot-pin → /auth/reset-pin (308) — #2993 パスワード再入力方式へ置換', async ({
		request,
	}) => {
		await expectRedirect(request, '/auth/forgot-pin', '/auth/reset-pin');
	});
