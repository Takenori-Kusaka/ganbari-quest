...
test.describe('#2148 子供画面 カテゴリヘッダー誤タップで活動グリッドが消えない (γ 採用)', () => {
	...
	test('baby: ヘッダー tap で活動グリッドが消えない (準備モード、ADR-0011)', async ({ page }) => {
		...
		// baby モードは ADR-0011 で親準備モード扱い。`ProdDashboardSections` を経由する場合は
		// カテゴリヘッダーが描画されるが、baby 専用 layout でカテゴリ非表示の場合もある。
		// いずれの構造でも「ヘッダー tap で活動グリッドが全消失する不具合」が再発しないこと
		// を構造的に保証するため、ヘッダー有無で分岐した上で必ず assertion を実行する
		// （ADR-0006 assertion erosion 回避: runtime skip ではなく実 assertion を行う）。
		const headers = page.locator('[data-testid^="category-header-"]');
		const headerCount = await headers.count();
		if (headerCount > 0) {
			// baby でもカテゴリが描画される構成: ヘッダー tap → 活動グリッド残存を検証
			await assertCategoryHeaderTapDoesNotHideGrid(page);
		} else {
			// baby 専用 layout でカテゴリ非表示の構成: 不具合の再発条件 (誤タップで消える
			// カテゴリヘッダー <button>) が物理的に存在しないことを assert
			const buttonHeaders = page.locator('button[data-testid^="category-header-"]');
			await expect(
				buttonHeaders,
				'baby モードに button タグのカテゴリヘッダーが存在しない (#2148 γ 採用、構造的排除)',
			).toHaveCount(0);
		}
	});
});
