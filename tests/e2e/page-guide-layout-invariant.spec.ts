 ...
 * allowlist 以外 hard-fail) を全 step に適用しつつ、`requiredSpotlight` に列挙した step は
 * さらに「ガイドに出現すること」まで要求する (#4650 のページ単位 ratchet)。
 */
	...
	for (const { label: vpLabel, width, height } of VIEWPORTS) {
		for (const path of ADMIN_GUIDE_PAGES) {
			test(`[${vpLabel}] ${path}: 全 step でバブル非重複 / viewport 収容 / spotlight 表示`, async ({
				...
				await walkAllSteps(page, bubble, path, vpLabel, REQUIRED_SPOTLIGHT_STEPS[path] ?? []);
			});
		}
	}
});
