...
async function walkAllSteps(
	...
	for (let i = 0; i < MAX_STEPS; i++) {
		...
		// (d) #4653: selector を持つ step は (列挙の有無によらず) 実要素に spotlight していること。
		// AdminLayout が起動直前に filterGuideStepsByTargetPresence を適用するため、残った
		// selector 付き step は必ず描画済要素を指す (EPIC #4650 PO 判断 4)。
