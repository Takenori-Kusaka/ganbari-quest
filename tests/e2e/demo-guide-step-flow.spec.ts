...
// #2097 PR-B3 (#2188) skip 理由 (ADR-0006 適合):
//
// `/demo` landing + `demo-guide-start-link` 起動ボタンが本 PR で撤去されたため、
// 本 spec の起動経路 `page.goto('/demo')` → `demo-guide-start-link` click が不可能になった。
// 起動経路を `/preschool/home` 直接アクセス + `startGuide()` script 注入で代替する案は、
// `getGuideState()` が `$state` rune で client-side 限定のため Playwright `page.evaluate()`
// から呼べない (Svelte 5 runes は SSR/CSR boundary を跨いで操作できない)。
//
// 構造的解決: PR-B4 (#2189) で hooks.server.ts demo 検出を env-only 化する際に、
// demo Lambda 環境で root `/` に guide 起動ボタンを表示する UI を追加し、本 spec を
