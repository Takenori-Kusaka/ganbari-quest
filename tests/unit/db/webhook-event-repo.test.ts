...
describe('demoWebhookEventRepo (in-memory)', () => {
	...
	describe('insert', () => {
		it('同一 event.id の上書き insert で 2 度目の record が勝つ (Map.set 挙動)', async () => {
			...
			// 注: sqlite/dsql 実装では `INSERT OR IGNORE` / `ON CONFLICT DO NOTHING` で
			// 2 度目を弾く設計 (race condition 検出)。本 demo 実装は最小限の Map.set のため
			// 上書き挙動になるが、dispatcher 側で findByEventId 経由の dedup 後に呼ぶ前提で問題ない。
		});
	});
