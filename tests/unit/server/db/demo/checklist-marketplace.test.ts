...
describe('demo/checklist-repo — marketplace integration (#2097 B-7)', () => {
	...
	describe('per-child marketplace checklists (legacy fixture helper)', () => {
		it('903 (elementary M): event-pool checklist が含まれる', () => {
			const templates = getDemoMarketplaceChecklistTemplatesByChild(asChildId(903));
		...
		it('901/902/906: marketplace checklist 対象外', () => {
			expect(getDemoMarketplaceChecklistTemplatesByChild(asChildId(901))).toHaveLength(0);
			expect(getDemoMarketplaceChecklistTemplatesByChild(asChildId(902))).toHaveLength(0);
			expect(getDemoMarketplaceChecklistTemplatesByChild(asChildId(906))).toHaveLength(0);
		});
	});

	describe('Synthetic ID 衝突回避', () => {
		it('marketplace templates は id >= 5000 (DEMO_CHECKLIST_TEMPLATES と衝突しない)', () => {
			expect(DEMO_MARKETPLACE_CHECKLIST_TEMPLATES.every((t) => Number(t.id) >= 5000)).toBe(true);
		});

		it('marketplace items は id >= 6000', () => {
			expect(DEMO_MARKETPLACE_CHECKLIST_ITEMS.every((i) => Number(i.id) >= 6000)).toBe(true);
		});
	});

	describe('Repository read API', () => {
		it('findTemplatesByChild(asChildId(903)) は event-pool template を返す', async () => {
			const templates = await checklistRepo.findTemplatesByChild(asChildId(903), 'demo');
		...
		it('findTemplatesByChild(asChildId(904)) は event-school-start + hand-curated を返す', async () => {
			const templates = await checklistRepo.findTemplatesByChild(asChildId(904), 'demo');
