...
describe('demo/battle-repo (Phase B-5b)', () => {
	...
	it('findTodayBattle は 903 (elementary) で pending battle を返す', async () => {
		const battle = await battleRepo.findTodayBattle(asChildId(903), TODAY, 'demo');
	...
	it('findTodayBattle は 904 (junior) で当日 active battle を返す', async () => {
		const battle = await battleRepo.findTodayBattle(asChildId(904), TODAY, 'demo');
		expect(battle).toBeDefined();
		expect(battle?.status).toBe('pending');
	});

	it('findTodayBattle は 906 (senior) で当日 active battle を返す', async () => {
		const battle = await battleRepo.findTodayBattle(asChildId(906), TODAY, 'demo');
		expect(battle).toBeDefined();
		expect(battle?.status).toBe('pending');
	});

	it('findTodayBattle は battle 対象外 child (902 preschool) で undefined', async () => {
		expect(await battleRepo.findTodayBattle(asChildId(902), TODAY, 'demo')).toBeUndefined();
	});

	it('findTodayBattle は別日付で undefined', async () => {
		expect(await battleRepo.findTodayBattle(asChildId(903), '2099-01-01', 'demo')).toBeUndefined();
	});

	it('findRecentBattles は 903 で limit 件分を新しい順に返す', async () => {
		const battles = await battleRepo.findRecentBattles(asChildId(903), 5, 'demo');
	...
	it('findRecentBattles は 904 で過去 + 当日合計 5 件以上', async () => {
		const battles = await battleRepo.findRecentBattles(asChildId(904), 10, 'demo');
		expect(battles.length).toBeGreaterThanOrEqual(5);
	});

	it('findRecentBattles は battle 対象外 child (902) で空配列', async () => {
		const battles = await battleRepo.findRecentBattles(asChildId(902), 5, 'demo');
	...
	it('countConsecutiveLosses は 0 (集計 stub)', async () => {
		expect(await battleRepo.countConsecutiveLosses(asChildId(903), 'demo')).toBe(0);
	});
});
