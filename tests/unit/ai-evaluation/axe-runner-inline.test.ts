 ...
 *      (Stagehand v3 inline runner は AxeBuilder 非依存を維持)
 *   5. anti-regression: scripts/ai-evaluation 配下に AxeBuilder pattern (`new AxeBuilder(...)`) が残っていない
 *   6. @axe-core/playwright は runtime dependencies に無い (本番 bundle 非搭載)。
 *      devDependencies には存在してよい — PR #2800 で tests/e2e/ の a11y gate が消費 (real Playwright Page なら
 *      AxeBuilder 動作可、Stagehand v3 非互換制約の対象外)。
 *
 * SSOT:
...
describe('Anti-regression — @axe-core/playwright / AxeBuilder pattern 撤去確認', () => {
	...
	// #2695 (本 file 初版) は「@axe-core/playwright が package.json から完全に無い」ことを invariant
	// としていた。理由は Stagehand v3 understudy/Page が AxeBuilder 非互換だったため scripts/ai-evaluation
	// から撤去した点にあった。
	//
	// PR #2800 (CX-DoR #10、Round 18 PR-A11Y-2) で **通常 Playwright E2E (tests/e2e/)** に
	// @axe-core/playwright を再導入した。tests/e2e/ は real Playwright Page を使うため AxeBuilder が
	// そのまま動作し、Stagehand v3 非互換の制約を受けない。よって「package.json に存在してはいけない」
	// という旧 invariant は新要件と矛盾するため、真の安全意図 (= inline runner を AxeBuilder に依存させない
	// + 本番 bundle に載せない) を保持する形へ invariant を更新する (ADR-0006 — assertion 弱体化ではなく
	// 要件変更に伴う invariant 再定義)。
	it('@axe-core/playwright は runtime dependencies ではなく devDependencies にのみ存在する (本番 bundle 非搭載)', async () => {
		const pkg = JSON.parse(await readFile(resolve('package.json'), 'utf-8')) as {
			devDependencies?: Record<string, string>;
			dependencies?: Record<string, string>;
		};
		// 本番 runtime dependencies には入っていてはならない (E2E 専用 dev tool)
		expect(pkg.dependencies?.['@axe-core/playwright']).toBeUndefined();
		// devDependencies には存在してよい (tests/e2e/ の a11y gate が消費、PR #2800)。
		// 存在する場合は semver 文字列であることを確認する。
		const devEntry = pkg.devDependencies?.['@axe-core/playwright'];
		if (devEntry !== undefined) {
			expect(typeof devEntry).toBe('string');
			expect(devEntry.length).toBeGreaterThan(0);
		}
	});
