...
describe('purchaseItem', () => {
	it('ポイントが足りていれば購入成功', async () => {
		...
		const result = await avatarService.purchaseItem(1, sakura?.id ?? 0, 'test-tenant');
	...
	it('既に所持済みでエラー', async () => {
		...
		await avatarService.purchaseItem(1, sakura?.id ?? 0, 'test-tenant');
		const result = await avatarService.purchaseItem(1, sakura?.id ?? 0, 'test-tenant');
...
describe('equipItem', () => {
	it('所持済みアイテムを装備できる', async () => {
		...
		const result = await avatarService.equipItem(1, 'background', sakura?.id ?? 0, 'test-tenant');
		expect(result).toEqual({ success: true });
