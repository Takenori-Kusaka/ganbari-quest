...
describe('#3230 invariant: URL エンコード traversal はデコード差異で tenant 境界を迂回しない', () => {
	// #3349: payload 網羅を 14-セキュリティ設計書が言及する Unicode 正規化 / null byte / 過長 UTF-8 まで拡張。
	// アプリ層は raw 文字列を扱いデコード/正規化しないため、いずれも literal な 1 セグメントとして扱われ
	// 親ディレクトリ参照にならない (構造的封じ込めは全 payload で成立する。representative を増やし over-claim を解消)。
	const ENCODED_TRAVERSALS = [
		...
		'/tenants/t-old/avatars/5/．．/．．/t-other/avatars/9/secret.png', // #3349: Unicode 全角ドット U+FF0E
		'/tenants/t-old/avatars/5/%2e%2e%00/t-other/avatars/9/secret.png', // #3349: null byte (%00) 切詰め
		'/tenants/t-old/avatars/5/%c0%ae%c0%ae/t-other/avatars/9/secret.png', // #3349: 過長 UTF-8 (%c0%ae = overlong .)
	];
	...
	// #3349 item 1: fileExists=true の live case。上記ループは fileExists=false 固定のため
	// `restored?url:null` が常に null で `if (v !== null) expect(...)` が vacuously-true な dead branch だった。
	// 復元ファイルが実在する場合に **実際に永続化される avatarUrl が取込先 tenant prefix 内に閉じる**ことを
	// 非 vacuous に assert する (encoded traversal でも別 tenant key を生成せず t-new に remap される)。
	it('復元ファイル実在時 (fileExists=true) も persist される avatarUrl は t-new tenant prefix 内に閉じる', async () => {
		fileExistsMock.mockResolvedValue(true);
		await remapChildAvatarUrls(
			makeState('/tenants/t-old/avatars/5/%2e%2e/%2e%2e/t-other/avatars/9/secret.png'),
			TENANT,
			makeResult(),
		);
		// vacuous 回避: update が非 null 値で実際に呼ばれたことを保証したうえで t-new 境界を assert する。
		const nonNullUpdates = updateMock.mock.calls.filter((c) => c[1] !== null);
		expect(nonNullUpdates.length).toBeGreaterThan(0);
		for (const call of nonNullUpdates) {
			expect(String(call[1])).toMatch(/^\/tenants\/t-new\//);
			// 別 tenant セグメント (t-other) を prefix に昇格させない
			expect(String(call[1])).not.toMatch(/^\/tenants\/t-other\//);
		}
		for (const call of fileExistsMock.mock.calls) {
			expect(String(call[0])).toMatch(/^tenants\/t-new\//);
		}
