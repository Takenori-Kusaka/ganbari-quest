...
describe('sendCheer', () => {
	it('正常にスタンプを送信', async () => {
		...
		const result = await sendCheer(asChildId(1), asChildId(2), 'ganbare', TENANT);
		expect('success' in result && result.success).toBe(true);
		expect(mockInsertCheer).toHaveBeenCalledWith(
			{ fromChildId: asChildId(1), toChildId: asChildId(2), stampCode: 'ganbare' },
			TENANT,
		);
	});

	it('自分への送信は拒否', async () => {
		const result = await sendCheer(asChildId(1), asChildId(1), 'ganbare', TENANT);
	...
	it('無効なスタンプコードは拒否', async () => {
		const result = await sendCheer(asChildId(1), asChildId(2), 'invalid', TENANT);
		expect('error' in result).toBe(true);
	});
	...
	it('4回目までは送信可能', async () => {
		...
		const result = await sendCheer(asChildId(1), asChildId(2), 'nice', TENANT);
		expect('success' in result && result.success).toBe(true);
	});
});
...
describe('getUnshownCheers', () => {
	it('未表示のおうえんを名前付きで返す', async () => {
		...
		const result = await getUnshownCheers(asChildId(2), TENANT);
