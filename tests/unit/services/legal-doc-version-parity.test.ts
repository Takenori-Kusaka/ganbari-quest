...
const BODY_PINS: Record<string, BodyPin> = {
	privacy: {
		...
		// 顧客の再同意は 1 回にまとめる)。
		//
		// #4516 (develop) の merge に伴う再計算 (PR #4778 conflict 解決):
		//   - §3 = #4516 の越境移転リード文 (第10条への参照) + #4778 の生成 AI 用途訂正
		//     (廃止済みの「活動アイコン生成」を落とし、AI 提案 / 領収書読み取りと
		//      推論リージョン us-east-1 / us-east-2 / us-west-2 を明記) の合成
		//   - §10 = #4516 の §28 4 点開示 (制度 / 措置 / 範囲 / 同意記録) を土台に、
		//     移転先国へ #4778 の「保存先 us-east-1 / 推論のみ 3 リージョン」を注入
		// 本文が両者の合成になったため hash が変わる。改定日は同一リリース cut のため据え置き。
		//
		// #4844 follow-up (PO 決定 2026-09-04): 第1条の「これらの情報はご契約期間中保存されます」3 箇所を
		// 実装の事実に合わせた (アカウント / お子さまの情報 = アカウント削除まで保存 / 活動データ =
		// プラン別の履歴保持期間で順次削除)。本文の実質改定なので改定日を 2026-09-04 へ動かし、
		// CURRENT_PRIVACY_VERSION も同じ日付へ上げている (再同意が発火する)。
		// #4866 系 / PO 決裁 2026-09-10 (横断決定): Cookie 列挙の追補 3 件 / 呼称の統一 /
		// AI 応援提案の開示撤去 — この 3 件を **1 本の PR** にまとめ、版を 1 回だけ動かす
		// (別々に出すとそのたびに全顧客が /consent へ飛ぶ)。#4598 / #4516 / #4778 と同じ型。
		// #4944 (PO 判断 2026-09-12、最終リリース後の hotfix): 第10条「移転される情報の範囲」が
		// 実態より狭かった (保護者のメールアドレス / 子供のアバター画像 / プロフィールが抜けて
		// いた) ため DPIA §2.3 の全カテゴリに合わせ、Stripe の措置 (グローバル CBPR 認証) と
		// 運営者から Stripe へ送る範囲を明記した。同意した範囲が変わるので改定日を 2026-09-12 へ
		// 動かし CURRENT_PRIVACY_VERSION も上げる。2026-09-10 版への同意直後の 2 度目の再同意に
		// なるが、実態より狭い法務文書を本番に残す方が悪い (PO が 2 度目を受け入れた)。
		revision: '2026-09-12',
		sha256: 'eccfb549498877b2f3f0113bfb92c5c13daa9fbdd69ebde8ba4f4d4f7ff05598',
	},
	terms: {
		// #4503 (develop) が第7条・第8条ほかを改定し、改定日を 2026-08-13 へ動かした。
		// 本 branch との merge 時に上記 3 点セット (改定日 / CURRENT_TERMS_VERSION / pin) で追随している。
		// 同上。利用規約は **第 1 条の定義語を含めて** honorific
		// (お子さま / 保護者) に揃えた (PO 決定 2)。権利義務は 1 つも変わらない。
		revision: '2026-09-10',
		sha256: '2ef4196e2e8b5cf3c6436ebcccc0bbf3e5c3acbbb6f4787d9488513123bbea5f',
	},
};
