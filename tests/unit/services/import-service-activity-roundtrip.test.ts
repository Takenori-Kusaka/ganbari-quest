...
describe('#3327/#3328 backup round-trip: replace で per-child 活動が復元されるか (failing-test 再現)', () => {
	...
	// #3358 (第10回監査 data-5): childActivity round-trip が isVisible / sortOrder /
	// isArchived (+ archivedReason) を取りこぼし、archived 済み活動が import 後に active へ
	// 復活する silent なデータ破綻の failing-test (ADR-0061 failing-test-first)。
	it('#3358 childActivity の表示状態/並び順/アーカイブ状態が round-trip で完全一致する (archived→active 復活防止)', async () => {
		testDb.insert(schema.children).values({ nickname: 'ゆうき', age: 8, theme: 'blue' }).run(); // id=1

		// 非デフォルト値で seed: 非表示 + 並び順 7 + アーカイブ済 + reason
		testDb
		...
		// --- 復元後: 3 (+1) field が seed と完全一致するか ---
		// 再 import 後の child id は autoincrement で元の値と異なるため動的に解決する。
		const restoredChild = testDb.select().from(schema.children).all()[0];
		if (!restoredChild) throw new Error('child が復元されませんでした');
		const rows = testDb
			.select()
			.from(schema.childActivities)
			.where(eq(schema.childActivities.childId, restoredChild.id))
			.all();
		const archived = rows.find((r) => r.name === 'かくれA');
		const active = rows.find((r) => r.name === 'みえるB');

		expect(archived, 'archived 活動が復元される').toBeDefined();
		expect(archived?.isVisible, 'isVisible round-trip').toBe(0);
		expect(archived?.sortOrder, 'sortOrder round-trip').toBe(7);
		expect(archived?.isArchived, 'isArchived round-trip (active へ復活しない)').toBe(1);
		expect(archived?.archivedReason, 'archivedReason round-trip').toBe('trial_expired');

		expect(active?.isVisible, 'active isVisible').toBe(1);
		expect(active?.sortOrder, 'active sortOrder').toBe(3);
		expect(active?.isArchived, 'active isArchived').toBe(0);
	});
});
