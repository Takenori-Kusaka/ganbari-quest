...
function makeActivity(
	overrides: Partial<Omit<Activity, 'id'>> & { id: ActivityId | number | string },
): Activity {
	return {
...
describe('sortActivitiesWithPreferences', () => {
	...
	it('使用頻度が高い活動が上位に来る', async () => {
		vi.mocked(findPinnedByChild).mockResolvedValue([]);
		vi.mocked(getUsageCounts).mockResolvedValue([
			{ activityId: asActivityId(2), usageCount: 10 },
	...
	it('空の活動リストを処理できる', async () => {
		...
		const result = await sortActivitiesWithPreferences([], asChildId(1), 'test-tenant');
		expect(result).toHaveLength(0);
	});

	it('usageCountが付与される', async () => {
		vi.mocked(findPinnedByChild).mockResolvedValue([]);
		vi.mocked(getUsageCounts).mockResolvedValue([{ activityId: asActivityId(1), usageCount: 7 }]);
...
describe('toggleActivityPin', () => {
	...
	it('ピン留め上限内なら成功', async () => {
		...
		const result = await toggleActivityPin(asChildId(1), asActivityId(1), true, 'test-tenant');
	...
	it('ピン留め解除は上限チェックなし', async () => {
		...
		const result = await toggleActivityPin(asChildId(1), asActivityId(1), false, 'test-tenant');
