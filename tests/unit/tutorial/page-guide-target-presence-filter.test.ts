...
//
// ガイド step の selector が「条件付き UI (承認待ちバナー / お子さま 0 人で出ない子供タブ) /
// viewport 別 nav (desktop: header 下 / mobile: 画面下部)」を指すとき、対象が描画されていない step を
// そのまま driver.js に渡すと中央 dummy (0×0) か display:none 要素の 0×0 spotlight になる
// (/admin desktop step 3 の実害)。本フィルタは起動直前に「描画済の対象を持つ step だけ」を残す。
