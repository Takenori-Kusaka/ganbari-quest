...
describe('#4709 privacy の生成 AI 用途が実送信経路と一致する', () => {
	...
	// #4866 系 / PO 決裁 2026-09-10 (決定 9): **AI 応援提案 (#2273) を畳んだ**ので
	// 4 種 → 3 種になった。「応援メッセージ」を開示していたのに
	// **UI が無いまま LLM を叩く課金 endpoint が生きていた** — privacy だけが
	// 機能の存在を顧客に約束する形だったので、実装と開示を同じ PR で落としている。
	//
	// **逆方向をここで pin する**: 応援メッセージを開示に戻すなら、
	// 実送信経路 (route + service) を同じ PR で戻すことを強制する。
	it('AI 提案 3 種（活動・ごほうび・チェックリスト）を開示している', () => {
		for (const c of clauses) {
	...
	it('畳んだ AI 応援提案 (#2273) を開示に残していない', () => {
		for (const c of clauses) {
			expect(c).not.toContain('応援メッセージ');
		}
	});
