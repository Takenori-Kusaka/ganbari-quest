...
| 8 | 5 mode visual baseline + primitives 準拠 (独自 UI 逸脱なし) | 見た目 | 既存 pixelmatch (`scripts/check-lp-visual-regression.mjs`) を 3 層適用: LP (`lp-screenshot-baseline/`、#2401) + child home 4 mode + battle (`child-home-baseline/`、#2520) + **app baby home + admin 2 画面 (`app-screenshot-baseline/`、`capture-app-baseline.mjs` + `app-visual-regression.yml`、CX-DoR #8)** で 5 age mode home + admin critical 画面を網羅 + Storybook play (#2544 AC4) | **既存 + #2520 + CX-DoR #8 (実装済)** |
