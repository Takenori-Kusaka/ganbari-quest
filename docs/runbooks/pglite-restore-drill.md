...
そこで `HOST_BACKUP_DIR` を設定すると、compose が `BACKUP_OFFSITE_EXPECTED=true` を導出して
コンテナへ渡し (運用者が別途設定する項目ではない)、バックアップ後に
**保存先が稼働中 DB と同じファイルシステム上に無いか**を `stat().dev` で検査する。

| 判定 | 意味 | 挙動 |
|---|---|---|
| 別デバイス | 期待どおり | 何も出さない |
| **同一デバイス** | 差し替えたつもりがローカルに落ちている | **alert** |
| device が読めない | 判定不能 | alert (「問題なし」に丸めない) |

**この alert は失敗 alert とは別**である。取得自体は成功しているため、失敗として通知すると
運用者が実在する控えを無いものとして扱ってしまう。伝えるべきは「取れたが置き場が想定と違う」。
連続失敗カウンタ (#4129) にも載せない。

判定は `src/lib/domain/backup-offsite.ts` (純粋関数) が持ち、`tests/unit/domain/backup-offsite.test.ts`
が固定している。**path 文字列ではなく device を見る**のは、上記 fallback がまさに
「NAS らしい path なのに実体はローカル」になるためである。
