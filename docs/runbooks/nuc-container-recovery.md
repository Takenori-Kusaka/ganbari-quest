...
ssh kusaka-server@192.168.68.79 "docker exec ganbari-quest-app-1 wget -qO- http://127.0.0.1:3000/api/health"

# 2. host に port が publish されているか
ssh kusaka-server@192.168.68.79 "docker port ganbari-quest-app-1"
#    空 → docker-proxy 未確立 (下記 recreate で復旧)

# 3. host 自身から localhost (302 なら host までは到達)
ssh kusaka-server@192.168.68.79 "curl.exe -s -m 8 -o NUL -w \"%{http_code}\n\" http://localhost:3000/"
```

> **注意**: host 自身から自分の LAN IP (`http://192.168.68.79:3000`) を curl すると NAT hairpinning で `000` になることがあるが、**別端末からは到達できている**。LAN IP の最終確認は必ず別端末のブラウザで行う。

### 復旧 (WAL 安全順序、#0099)

```bash
# graceful stop で SQLite WAL flush → force-recreate で docker-proxy 貼り直し
ssh kusaka-server@192.168.68.79 "cd /d C:\Docker\ganbari-quest && docker compose stop app"
ssh kusaka-server@192.168.68.79 "cd /d C:\Docker\ganbari-quest && docker compose --profile backup up -d --force-recreate --no-build app"
