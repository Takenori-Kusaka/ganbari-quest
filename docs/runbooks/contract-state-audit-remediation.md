...
同カードの「基準不明の継続月キー」は、`loyalty_last_increment_month` が prefix（`jst:`）無しで
保存されているテナント数（件数のみ・母数付き）。**契約 4 列とは別の在庫**であり、X1-X4 の是正手順は適用しない。

| 項目 | 内容 |
|---|---|
| 何が起きている状態か | 保存値の基準（JST か UTC か）が判別できないため、その家族の継続月数の加算が**安全側に見送られ続ける**（`loyalty-service` の `classifyMonthKeyMatch` = `ambiguous-legacy`）。skip 分岐は値を再 write しないため、以後 `invoice.paid` が来ないテナントでは滞留したままになる |
| 顧客への影響 | 継続月数が実際より 1 少なく計算されうる（画面にも通知にも出ない）。過剰加算（未達ティア特典の配布 = 回収不能）を避けるための意図的な非対称選択 |
| 0 件のとき | **その旨がそのまま結論**。行が出ていること自体が「監査が動いている」証拠であり、追加操作は不要 |
| 1 件以上のとき | 次の `invoice.paid` を prefix 付きで受ければ自然に解消する。解消を待たずに継続月数のずれを是正するかは**オーナー判断**（本番データの書き換えのため §4 と同じ扱い） |
| skip が起きた瞬間の記録 | サーバーログ（`context.kind = 'loyalty-increment-skipped-ambiguous'`）。本カードは「滞留の在庫」、ログは「skip の発生」で役割が違う（両方要る） |
