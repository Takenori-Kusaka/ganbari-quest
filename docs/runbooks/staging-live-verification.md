...
## 9.6 クラウド共有の PIN 往復（生成待ち → 生成完了、#4717）

クラウド共有は非同期 build（#3504）で、AWS の build cron は 5 分毎に走る。**発行〜5 分の窓は必ず生成待ち**になるため、この往復は cron を跨ぐ staging でしか通しで確認できない（local は cron を手で叩けば代替できるが、AWS の実 S3 / 実 cron 間隔は再現しない）。

手順（所要 10 分程度、read-only ではなく共有データを 1 件作る）:

1. **発行側**（staging の owner アカウント）: 設定 > データ > 「クラウドに保管」→ 種別を選んで実行。一覧に「受付済み・生成待ち」と PIN が出る（PIN を控える）
2. **受け取り側**（別ブラウザ / 別アカウント）: 設定 > データ > 「PINコードでインポート」に **すぐ**その PIN を入れて確認を押す
   - 期待: **「このデータはまだ準備中です。数分後にもう一度お試しください。」**（HTTP 409 / `EXPORT_NOT_READY`）
   - 「システムに問題が発生しました」（500）が出たら #4717 の回帰
3. **5〜10 分待つ**（build cron 起動 → 一覧の表示が「生成中…」→ ダウンロード可能に変わる）
4. **受け取り側**: 同じ PIN でもう一度確認 → プレビューが表示され、取込を実行できる
   - 期待: 取込結果が表示され、対象データ（活動 / チェックリスト等）が増える
5. **後始末**: 発行側の一覧から共有データを削除する（保管したままにしない）

確認結果は実行日と HTTP status を PR / Issue にそのまま記録する（「確認した」だけの記述は証跡にしない）。
