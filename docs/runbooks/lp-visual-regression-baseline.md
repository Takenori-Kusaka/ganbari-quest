...
## §5. app 層 baseline 更新 (child-home / app / ガイド open、同型運用)

visual regression 3 層 (`docs/CLAUDE.md` §「visual regression 3 層」) のうち **app 層** (`scripts/app-screenshot-baseline/`、`capture-app-baseline.mjs` / `app-visual-regression.yml`) も同じ `check-lp-visual-regression.mjs --update-baseline` 機構で更新する。LP 層 (§3) と原則・triage は同型で、撮影 script と baseline dir のみ異なる。

### (A) app 層 baseline 更新コマンド

意図的にアプリ画面 / ページガイドの見た目を変えた後、demo fixture env で preview server を起動した状態で実行する。

```bash
# 1. preview server を demo fixture env で起動 (別ターミナル)
AWS_LICENSE_SECRET=dummy PARENT_GATE_COOKIE_SECRET=dummy-secret-do-not-use \
  AUTH_MODE=anonymous DATA_SOURCE=demo npm run preview -- --port 5173

# 2. current SS を撮影 (全 group: baby home + admin 2 画面 + ガイド open 6 枚)
BASE_URL=http://localhost:5173 node scripts/capture-app-baseline.mjs --webp
#    ガイド open のみ撮り直す場合: --only guide
#    age / admin / guide の group 単位で絞り込める

# 3. current → baseline へ上書き
node scripts/check-lp-visual-regression.mjs \
  --baseline-dir scripts/app-screenshot-baseline \
  --current-dir tmp/app-baseline-current \
  --update-baseline
#    または capture-app-baseline.mjs --webp --update-baseline (撮影 + 上書きを一括)

# 4. baseline を commit (git-tracked SSOT)
git add scripts/app-screenshot-baseline/
git commit -m "chore: app visual baseline 更新 (意図的なアプリ / ガイド UI 変更を反映)"
```
...
### (C) app 層は warn 段階 (hard-fail 昇格は別 Issue)

`app-visual-regression.yml` は `continue-on-error: true` の warn 段階 (child-home 層と同方針)。diff > 10% でも CI を止めず、安定後に hard-fail 昇格を別途判断する。`missing` (撮影漏れ) の triage は §3 (D) 3 と同じ。
