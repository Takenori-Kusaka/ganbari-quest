...
- **判定は文字列マッチ**: セグメント (`&&` / `;` / `|` / 改行) 単位で判定するため無害な前置きでは回避できないが、新しい重量コマンドを足したら `HEAVY_PATTERNS` の更新が要る
- **判定対象は「実行される部分」だけ** (#4401): heredoc の本文 (`<<'EOF' … EOF`) と、テキストを運ぶ flag の引用符付き引数 (`--body` / `--message` / `-m` 等) は判定前に落とす。PR 本文 / Issue 本文 / commit message に検証コマンドを書くのは「言及」であって実行ではない。`PULL_REQUEST_TEMPLATE.md` は「コマンドと結果」を書くことを要求しているので、ここを落とさないと**規約を守るほど block される**。<br>**引用符付き文字列を一律に落とすことはしない** — `sh -c "npx vitest run"` の中身は実行されるため、落とすと排他が回避できる
