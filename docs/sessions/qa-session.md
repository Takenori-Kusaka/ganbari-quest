...
### SOLID 準拠確認（サービス/リポジトリ層の変更時）

diff にサービス層・リポジトリ層・ドメインモデルの変更が含まれる場合、以下を確認する:

- **SRP（単一責任）**: 変更したクラス/関数が単一の責務に限定されているか
- **DIP（依存性逆転）**: 具体的なリポジトリ実装に直接依存せず、インターフェースや DI を経由しているか（`$lib/server/services` 経由の原則と整合）
- **ISP（インターフェース分離）**: 必要最小限の interface/型に依存しているか（巨大な interface 全体への依存になっていないか）

違反例:
- `+server.ts` が ORM クライアントを直接呼び出している（DIP 違反、CLAUDE.md 禁止事項）
- サービスクラスが認証・DB・通知を 1 関数内で全て処理している（SRP 違反）
- 使用しないメソッドを多数含む大きな interface に型依存している（ISP 違反）
