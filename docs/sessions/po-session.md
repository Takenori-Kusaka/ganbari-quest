...
**整った報告ほど実測を省きたくなる。** 論理が整っていることと、事実がそうであることは別である。

> **2026-08-05 の実例**: `#3970` は 8/1 に「保存先は NUC 内のまま」とオーナー決裁済みだったが、最新コメント 1〜2 件しか読まずに「受け入れ条件 1 は未達」と報告し、**既に受容済みの決定を蒸し返した**。実物（SSH の実測値）は正しく、**過去の決定を読まなかった**ことが誤りだった。
