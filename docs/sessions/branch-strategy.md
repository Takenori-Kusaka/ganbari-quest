...
| `--force-with-lease` が stale info で reject | worktree では `origin/<branch>` tracking ref が自動更新されないことがある。`git fetch origin <branch>` で当該 branch ref を明示更新してから再 push する |
