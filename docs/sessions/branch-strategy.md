...
- **develop merge 時点で Issue を close し、`status:awaiting-release` label を付与する（2026-07-30 改訂）**。main 到達時にラベルを外す。GitHub の auto-close は default branch 到達時にしか発火しないため、この close は **統合 PR 側の `Closes #N` 集約とは独立に、merge 実施者が明示的に行う**。
  ...
  - **実装は `integration-pr-body.mjs` の `extractClosedIssues` を再利用する**（二重実装しない）。close 対象の抽出規約は統合 PR 集約と同一関数を共有する。
  - `epic` label 付き tracking issue は従来どおり除外する（AC 未検証 force-close 防止）。
...
- **`issue-close-gate.yml`（手動 close 時の AC 検証 gate）は #4322 で削除済み**。以前は close 経路（auto-close / 手動 close）で挙動が分かれたが、現在は機械 gate が存在しないため経路によらず close 後の reopen は発生しない。
- **close は例外パスでも行う**: wontfix / duplicate / PO 保留など意図的 close、および含有 PR が closing keyword を付け忘れた Issue の close は、Issue body の `- [x]` 化（証跡コメント付き）または `wontfix` / `duplicate` label を付けて手動で close する（機械 gate なし、目視で AC 未達がないか確認すること。ADR-0004 §4）。

> 補足: 本節は「個別 PR の develop merge では auto-close が張られず、統合 PR の `Closes #N` 集約で main 反映時に一括 close する」という GitHub 仕様起因の運用方針を SSOT 化したもの。手動 close 時の AC 検証機械 gate（旧 `issue-close-gate.yml`）は #4322 で削除されたため、現在は両者とも機械 gate なしで運用する。
...
| `dependabot-auto-merge.yml` | dependabot（`pull_request`、actor=`dependabot[bot]`、**base=`develop` のみ auto-merge 武装**、#4544） | — | なし（`BOT_ACTORS` SSOT を参照し inline 判定。composite action は overhead 回避で不採用、#2947） | 軽量 |
...
| `integration-attest.yml` | N/A（push[main] / dispatch、統合 merge を契機に SARIF + in-toto Release predicate を Sigstore 署名し GH attestations API へ永続化） | — | なし（生成は `scripts/audit/to-sarif.mjs` / `scripts/audit/generate-release-predicate.mjs` SSOT。統合 merge でない push は no-op skip。deploy 系と needs 関係なし = 独立 job、attestation 失敗は deploy を阻害しない） | 補助（§3.5 マージ判定エビデンス永続化、#2876） |
...
- **必要な変更（2 点）**:
  1. **main 向け PR の base 制限**: 「main へ PR を出せるのは develop と hotfix branch のみ」を Ruleset 単体で表現するのは困難（Branch Ruleset は head branch の base 制限を直接持たない）。**補完案**として、`main` 向け PR の base / head を workflow gate で検査し、`develop` / `fix/*` 以外を head とする main 向け PR を fail させる方式を採る — **#2931 で `ci.yml` の `main-pr-base-guard` job として実装済み**。有効化は repository variable **`BRANCH_STRATEGY_CUTOVER_AT`**（ISO 8601 UTC 日時、ユーザー手動設定）。未設定なら **head 判定は** inactive、設定後は設定日時より後に作成された main 向け PR のみ enforce（既存 open PR は grandfather 免除 = §8 step 6）。**bot（dependabot / renovate）の main 向け PR は exempt ではなく fail-close する**（#3922）。dependabot の security 更新は `dependabot.yml` の `target-branch: develop`（#3072）を無視して default branch = main に PR を作る仕様で、設定では防げないため検出して止める。この bot 判定は **`BRANCH_STRATEGY_CUTOVER_AT` の設定有無に依存しない**（variable を外すと bot の main 直行検出まで道連れで消える fail-open を #4544 で解消）。retarget（`gh pr edit <N> --base develop`）で通常レーンに載せる（#4273）。
...
- **GitHub App 認証の設定（ユーザー手動、#3067 / ADR-0022 Amendment 5）**: `hotfix-back-merge.yml`（§5 back-merge 機械強制）と B-3 統合 PR 自動発行（#2871）は、`secrets.GITHUB_TOKEN` で PR を作ると author が `github-actions[bot]` になり `pr-author-guard.yml` に auto-close され、かつ下流 CI を発火させない。これを回避するためプロジェクト専用の **GitHub App** を作成し、その install トークン（`actions/create-github-app-token` が実行毎に短命発行・自動失効）で PR を発行する。長命個人 PAT（旧 `BACK_MERGE_PAT`）は撤廃済み（GitHub 非推奨の常駐 credential を排除）。一度きりの設定手順:
  ...
  2. **install + 秘密鍵**: App を本 repo に install し、App の Private key（PEM）を生成・ダウンロード。App ID を控える。
  3. **repository secret / variable 登録**（secret 値はユーザー専権、AI からは設定不可）:
...
## §10 自動化段階移管（S0→S4、tracker #3399）

develop→main 統合の運用を **手動（audit-manager が release ブランチ cut + 監査 + approve/merge）から段階的に自動化**する移管経路。#3067 で GitHub App 化を実装し、secret 設定済み環境で `integration-pr.yml` が bot 名義の standing 統合 PR を自動 upsert する経路が稼働している（初回 #3397）。#3067 は残作業のため OPEN。一気に全自動化せず、各段の gate を観測してから次段へ進む。

| 段階 | 自動化する範囲 | 残す判断（AI audit-manager / human） | 次段へ進む gate |
|---|---|---|---|
| **S0（現在）** | 統合 PR upsert のみ | release cut・audit・起票・approve/merge すべて手動 | — |
| **S1** | + standing 統合 PR を vehicle 化（手動 cut 廃止） | audit・起票・approve/merge | 自動 body / 含有 PR 列挙が N サイクル連続で正確 |
| **S2** | + audit run の自動 dispatch（schedule で audit agent 起動） | 起票(AI)・approve/merge(human) | audit agent の false-positive 率 < 閾値（M サイクル観測） |
| **S3** | + clean run（実害直結 sev3+/CRITICAL=0）は auto-approve/merge | 起票(AI)・**HOLD 時のみ human** | rules-based merge gate が安定 |
| **S4** | green は full auto。audit-manager は findings 時のみ起動 | HOLD escalation のみ | — |
...
- **構造化 evidence schema**（severity / policy_compliant / file:line）を起票判断の入力に強制（根拠接地）
- **policy-compliance filter** で誤起票抑制（意図的設計を弾く）
- **起票前 dedup 必須**（`gh issue list --search` で既存照合 → 既存は新規でなくエスカレーション comment 化）
- **実害直結 sev3+ は HOLD→PO escalation**（判断が重い所だけ human）

要点: S3/S4 でも「起票」は AI agent に残し、**merge 自動化と起票判断を分離**する。merge gate だけ機械化し、起票判断は止めない。
