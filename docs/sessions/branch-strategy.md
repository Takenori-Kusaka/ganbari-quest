...
全 `.github/workflows/*.yml`（35 本）の lane 帰属・required 化・lane 分岐の網羅表。新規 workflow を追加する人はこの表に必ず 1 行追加する（差分検出 gate `scripts/check-internal-terms.mjs` の workflow-coverage group が「`.github/workflows/*.yml` 一覧 ⊆ 本表記載一覧」を機械検証し、未記載は CI fail、#2948 AC4）。
