...
### §3.2 develop 二層での Issue close 運用（`closes #N` 非連動、#2938 項目 4）

develop 二層では feature/fix/docs PR の base が `develop`（非 default branch）であり、かつ本リポジトリの commit 規約が conventional-commit prefix（`fix: #N` / `feat: #N` / `docs: #N`）であるため、**Issue は merge 時に GitHub の auto-close で閉じない**。GitHub の closing-keyword 連動は (1) `Closes #N` / `Fixes #N` のような closing keyword（keyword の直後に `#番号`）が、(2) default branch（= `main`）に到達した時にのみ発火する。`fix: #N` 等のコロンを挟む形は Issue **参照**であって closing keyword ではない。このため運用を以下に統一する。

- **develop merge 時点では Issue を手動 close しない**。`closes #N` を付けた PR を develop へ merge しても Issue は **OPEN のまま**で正常。これを「未対応」と誤認しない。
- **main 反映でも自動 close は当てにしない**。GitHub の auto-close が発火するのは `Closes #N` / `Fixes #N` のような **closing keyword（keyword の直後に `#番号`）が default branch（= `main`）に到達した時のみ**。本リポジトリの commit 規約は conventional-commit prefix（`fix: #N` / `feat: #N` / `docs: #N`）で、コロンを挟む `fix: #N` 等は Issue **参照**であって closing keyword ではないため、統合 PR の merge commit が main に到達しても **auto-close は発火しない**（統合 PR 自体も単一 Issue に紐づく `Closes #<single>` を持たない、[INTEGRATION_PR_TEMPLATE.md](../../.github/INTEGRATION_PR_TEMPLATE.md)）。したがって現規約では Issue は基本的に main 反映で自動 close されず、下記の例外パスで close する。
- **close は例外パスで行う**: wontfix / duplicate / PO 保留など意図的 close、および対応完了済 Issue の close は、Issue body の `- [x]` 化（証跡コメント付き）または `wontfix` / `duplicate` label で `issue-close-gate.yml` の AC 検証 gate を通す（ADR-0038 / [.github/CLAUDE.md](../../.github/CLAUDE.md) §「issue-close-gate auto-reopen の挙動」）。Issue body の `- [ ]` 残存で gate が reopen ループを起こすため、`- [x]` 化を先に済ませること。main 反映による auto-close を期待して放置しない。
  - main 反映時の auto-close をチームが望む場合は、feature commit / PR body に明示的な `Closes #N` keyword を含める必要がある（本節 scope 外の別判断）。

> 機械層の補足: `issue-close-gate.yml` が扱うのは「手動 close 時の AC 検証」であり、本節は「現 commit 規約（`fix:` / `feat:` / `docs:` は closing keyword でなく Issue 参照）では merge で auto-close が張られない」という GitHub 仕様起因の運用方針を SSOT 化したもの（両者は別レイヤー）。
