...
develop→main 統合の運用を **手動（audit-manager が release ブランチ cut + 監査 + approve/merge）から段階的に自動化**する移管経路。#3067 で GitHub App 化が完了し `integration-pr.yml` が bot 名義の standing 統合 PR を自動 upsert する経路が実働している（初回 #3397）。一気に全自動化せず、各段の gate を観測してから次段へ進む。
