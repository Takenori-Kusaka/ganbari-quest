...
develop 二層では feature/fix/docs PR の base が `develop`（非 default branch）であるため、**PR body / commit message の `closes #N` は GitHub 仕様上 auto-close linkage を張らない**（GitHub の closing-keyword 連動は **default branch（= `main`）向け PR / default branch への push でのみ発火**する）。このため運用を以下に統一する。

- **develop merge 時点では Issue を手動 close しない**。`closes #N` を付けた PR を develop へ merge しても Issue は **OPEN のまま**で正常。これを「未対応」と誤認しない。
- **Issue は統合 PR（`release/* → main`、§3.1）が main へ reflect した時に一斉 auto-close される**。develop 上の各 feature commit が持つ `closes #N` keyword が、統合 PR の merge commit で main（default branch）に到達した時点で GitHub が auto-close を発火させる（統合 PR 自体は単一 Issue に紐づかない＝`closes #<single>` を持たない、[INTEGRATION_PR_TEMPLATE.md](../../.github/INTEGRATION_PR_TEMPLATE.md)）。
- **手動 close を避ける理由**: `gh issue close` / GitHub UI ボタンでの手動 close は `issue-close-gate.yml` の AC 検証 gate を通し、Issue body の `- [ ]` 残存で reopen ループに陥る（[.github/CLAUDE.md](../../.github/CLAUDE.md) §「issue-close-gate auto-reopen の挙動」）。PR 経由 auto-close（main 反映時）は同 gate を skip するため、**main 反映の auto-close に委ねるのが正準**。
- **例外（main 反映前に明示 close したい場合）**: wontfix / duplicate / PO 保留など意図的 close は、Issue body の `- [x]` 化（証跡コメント付き）または `wontfix` / `duplicate` label で gate を通す（ADR-0038 / [.github/CLAUDE.md](../../.github/CLAUDE.md)）。

> 機械層の補足: `issue-close-gate.yml` が扱うのは「手動 close 時の AC 検証」であり、本節は「develop 二層で `closes #N` がそもそも auto-close を張らない」という GitHub 仕様起因の運用方針を SSOT 化したもの（両者は別レイヤー）。
