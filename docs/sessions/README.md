...
複数ロールが**別クローン・別セッション**で動く本リポジトリには、ロール間の境界を定義したファイルが無かった。本ファイルはその**チーム憲章**にあたる。「Sprint Zero」ではなく、**恒常的に参照されるルール**として置く（Scrum.org は準備専用スプリントを批判するが、チーム憲章 / ワーキングアグリーメントは正当な事前作業として認める）。

装置（`check-*` / workflow / PR テンプレート / `CLAUDE.md` 等）が増える一方だったのは、削減を引き受ける役割が居なかったため。この役割を §3.4 で新設する。装置の削減は po-session.md の起票基準（装置起因の不具合は Issue 化しない）とは区別する（§4.5）。

装置の現在数（実測 SSOT。`platform-session.md` §「装置の現在数」がこの表を参照する。数値の書き写しはここ 1 箇所に限定する）:

| 装置 | 数 |
|---|---|
| `scripts/check-*` 検査スクリプト | **45** |
| `scripts/` 全体 | 84 |
| GitHub Actions workflow | **32** |
| PR テンプレート | **25 行** |
| Issue テンプレート | 5 |
| `CLAUDE.md`（6 ファイル合計） | **815 行** |
| Skill 定義 | 20 |
