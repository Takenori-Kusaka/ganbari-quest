...
| # | ルール |
|---|---|
| 1 | **装置は凍結。** 品質ゲート / guard / fitness / lint / テンプレート / hook を**増やさない・良くしない・減らさない**。`pre-ready` の構成も変えない |
...
**🔒 凍結中です（§0 ルール 1）。** 装置に関する決定は、凍結解除まで発生しません。

- **装置の Issue は起票しない**（増やす / 直す / 減らす / 統合する、すべて）
- **既存の `state:needs-platform` は `status:on-hold`**。Platform ロールは凍結中、装置作業を持たない
