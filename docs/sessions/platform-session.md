...
**[チーム憲章 §0](README.md) ルール 1**: 品質ゲート / guard / fitness / lint / テンプレート / hook を **増やさない・良くしない**。`pre-ready` にも step を増やしません。**進めてよいのは A（削減）と B（リリースプロセスへの移管）だけ**で、それが Platform の唯一の仕事です（[§4.5](README.md#45-装置開発基盤に関する決定)）。

- **装置の Issue は起票しない**（[§0](README.md) ルール 7 / §4.5）。A / B も Issue を経由せず、気づいた時点でその場で PR を出す
- **増やす / 直す作業だけが凍結対象**。既存の「増やす / 直す」`state:needs-platform` は `status:on-hold`
- **例外は「顧客の金かデータに現に届いている装置不具合」だけ**。判定は **QM** が行う（PO 決裁は要らない）
- **再開トリガー**: E1（#4117）が staging で checkout → webhook → plan 反映 → 実画面 を 1 周した時点（そのとき「増やさない」を続けるかを再判断する）

**なぜ**: 直近 14 日の Issue は装置・プロセスが 56%、顧客に届く変更が 24%。`scripts/check-*` は 61 本あり、「8 本に絞る」E5 は CLOSED なのに 1 本も減っていません。**装置が顧客を止めた実例**として、本番 NUC が 3.5 時間停止しています（#4275、バックアップの沈黙を防ぐ検査が原因）。以下の手順のうち「増やす / 直す」に当たるものは、凍結解除後に使います。
...
> **ratchet が縛るのは「新しい検査の追加」だけ。置換・統合は増加に当たらない**（2026-08-02、#4199 で明確化）。散在した同じ記述を SSOT 1 本に寄せる作業（例: `node-version: 22` × 52 箇所 → `node-version-file: .nvmrc`）は行数がほぼ変わらず、**減らす原資を必要としない**。この区別が無いと、**制約が正しい作業を止める** — 実際に #4199 本体（置換）が「ratchet の原資が無い」を理由に着手不能と判断されかけた。
...
| 種類 | label |
|---|---|
| **本番データの削除 / 本番 deploy / 課金書込 / スキーマ変更**を含む（不可逆 4 操作、[§4.4](README.md#44-不可逆-4-操作--常にオーナー)） | `state:needs-owner` |
| **gate / guard / test を残すか消すかの方針** / 優先度 / 語彙・ルールの改訂 / 受容判断 | `state:needs-po`（**削除の実行はオーナー決裁ではない** — 方針が決まったら通常の PR + QM レビュー、[§5.2](README.md#52-判断を仰ぐとき)） |
...
- **本番データの削除 / 本番 deploy / 課金書込 / スキーマ変更を自分で実行しない。** 不可逆 4 操作は**常にオーナー**（[§4.4](README.md#44-不可逆-4-操作--常にオーナー)）。案と影響範囲を用意して `state:needs-owner` で渡す。**`gate` / `guard` / `test` の削除は 4 操作に含まれない** — 方針は PO（上表）、実行は通常の PR + QM レビュー
