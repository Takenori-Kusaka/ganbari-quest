...
### UI 課題の一般化原則（A〜D 仕分け、[webui-review-process.md](webui-review-process.md) §4）

UI 系の fix / design PR では、レビューや CI 失敗で見つけた問題を**一段抽象度を上げて還元**する（レビュー = モデル改善、対症療法禁止 ADR-0003）。発見した問題を A〜D に仕分け、**仕分け結果を PR body（「レビュー依頼事項」等）に記載**する:

| 仕分け | 還元先 | 同 PR で行うこと |
|---|---|---|
| **A** 画面固有 | 当該 `*.svelte` / route | 画面修正のみ（還元不要） |
| **B** コンポーネント設計 | `src/lib/ui/primitives/` API + `app.css` 3 層トークン（DESIGN.md §5/§2） | primitive 修正 + Storybook story（play）追加 + 必要ならトークン更新 |
| **C** 配置・振る舞いアルゴリズム | Ark UI `positioning` 設定 / 共通ロジック | ロジック修正 + Playwright geometry assertion（Layer 2）追加 |
| **D** ガイドライン欠落 | DESIGN.md / `tests/CLAUDE.md` CX-DoR / `qa-checklist-ui-quality.md` / 該当 skill | 観点追補 + 本 PR or 後続 issue 起票 |

**運用**: B / C / D に該当する問題を A（その場修正）で済ませることは禁止。B / C は同 PR で還元先まで実装する（段階的リリース禁止）。D の追補は本 PR で行うか、scope が大きい場合は 後続 issue を起票したうえで Done とする。詳細・4 層自動化モデル・既存資産対応表は SSOT [webui-review-process.md](webui-review-process.md) を参照。
