...
### Issue 完了後の `/clear` ルール（2026-04-26 制定）

**1 Issue が PR Ready まで完了したら `/clear` を実行してセッションをリセットすること。**

理由：
- 長いセッションはコンテキスト圧縮が進み、「自分が開発リーダーである」という認識が薄れる
- Issue 間の文脈がセッション内に混在すると判断が誤った前提に依存しやすい
- `/clear` は新しい Issue に向かう前の「役割の再確認」の儀式として機能する

例外：
- CI 待ちなど「同一 Issue の継続作業が未完了」の場合はクリア不要
- PO との判断が必要な事項を話し合い中の場合は会話を保持する
