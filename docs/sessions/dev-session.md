...
Dev セッションの**開発責任者である Claude 本体**が Issue を **1 件ずつ直列に**進める。**複数 Issue 並列着手禁止**（ルール見落とし・手戻りの主因）。

| 操作 | 担当 | 備考 |
|---|---|---|
| Issue 着手順 | - | 1 件ずつ直列。前 Issue PR が Ready (CI 全緑) → 次へ |
...
#### やってはいけないこと
- 複数 Issue を別 Agent / セッションに振り分けて並列進行
...
7. Draft PR 作成: `gh pr create --draft --body-file tmp/pr-bodies/<slug>.md`（HEREDOC 禁止 #1172）
...
- 用語は `$lib/domain/labels.ts` 経由（ADR-0009）
