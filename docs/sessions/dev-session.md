...
### PR body draft の Write tool 例外（#1804）

`gh pr create / gh pr edit` で本文を渡すときは、**長文は `--body-file` 必須**（HEREDOC 禁止 — ADR-0003 / #1172、PR 本文に必要な PR テンプレート全節 + AC 検証マップ + SS リンクを含む長文はバッククォート / コードブロックで HEREDOC が壊れやすい）。
そのため Write tool / `cat > ... << 'EOF'` で **一時ファイル `tmp/pr-bodies/<slug>.md` を作成することが許容される**。
これは sub-agent ハーネスの「report files / summary を書くな」一般原則の **明示的例外**。

```bash
# 1. PR body 全文を Write tool または cat で tmp/pr-bodies/ に保存
#    例: tmp/pr-bodies/1804.md
# 2. PR 作成 / 編集
gh pr create --draft --title "..." --body-file tmp/pr-bodies/<slug>.md
# または
gh pr edit <pr-num> --body-file tmp/pr-bodies/<slug>.md
# 3. 完了後に削除（古い draft が混ざらないように）
