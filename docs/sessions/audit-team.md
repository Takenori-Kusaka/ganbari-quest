...
## §0 セッション起動時の必須手順: mailbox cron を作る

**SSOT**: [label-mailbox.md](label-mailbox.md)

監査セッションも別クローン・別セッションで動くため、**起動直後に mailbox を polling する cron を 1 本作る**。
...
- **merge 方式（#2938 / [branch-strategy.md](branch-strategy.md) §3.1）**: 統合 PR の merge は **merge commit 必須（`gh pr merge --merge`）・squash 禁止**（根拠 SSOT = [branch-strategy.md](branch-strategy.md) §2「統合 PR の自動発行 + merge 戦略 = merge commit（#2871）」）。develop は恒久 branch のため merge 後も削除しない（[qm-session.md](qm-session.md) §レビュー対象レーン と同一規定）。
...
[2] 重複統合
    audit-manager が同一原因・同一箇所の finding をマージし重複を 1 件化
        ...
        ▼
[3] severity 閾値
    severity 1-2（軽微）= accepted-residual（PR 本文記録、起票せず）※[5b] へ
...
[4] ポリシー準拠判定 filter
    「あえてそうしているプロダクトポリシー」由来の挙動か判定
      ...
      └─ 真の問題         → 次段へ
        │
        ▼
[5] 受容/起票/棄却 の 3 区分（accepted-residual gate、#3487 / ADR-0061 原則 5）
    残った真の問題を {blocking / class-lock 対象 / accepted-residual} に強制分類する。
      ├─ blocking          → 統合 PR を止め、その場で修正 or 即 Issue（severity≥3 必須）
      ├─ class-lock 対象   → 同 root class が 2 instance 目（同 run 内含む）→ instance Issue を
      │                       repeat せず **class 全体を 1 件の fitness function/property test で lock**
      │                       （ADR-0061 原則 2 shift-left。class-lock Issue は class 単位で 1 件まで）
      └─ accepted-residual → Pre-PMF で受容する marginal な残課題。**Issue 化せず** [5b] へ
[5b] accepted-residual の記録
    統合 PR 本文「Accepted residual (Pre-PMF)」section に {finding / severity / 受容理由 / 関連 class}
    を記録（trail は残るが open issue カウントを汚さない）。棄却分は従来通り evidence に記録。
```

- **accepted-residual gate（#3487、最重要）**: adversarial は echoing 抑止（arXiv:2511.09710）のため 3 件の反対理由を**必ず生産**するが、「**生産**」と「**Issue 化**」を分離する。filter 通過 finding のうち Pre-PMF で受容可能な marginal は Issue 化せず PR 本文に記録する。これにより「1 PR ≈ 1 follow-up」の比例（merge 行為が generator）を発生源で断つ。**ガード**: severity ≥ high（3-4）は accepted-residual 化禁止（必ず blocking か Issue）。誤って genuine bug を residual に落とすのを防ぐ。根拠: ISTQB pesticide-paradox / absence-of-errors-fallacy、Bach 停止ヒューリスティクス、Lehman Law VI（open 増は正常、brake 不在が病理）。
- **class-lock 優先（#3487）**: 同 root class（TOCTOU / import 値検証 / reset 完全性 / supply-chain pin 等）が 2 回目に露見したら、instance Issue を N 件起票せず class 全体を 1 件の機械 guard で lock する（ADR-0061 原則 2）。
- **無限棄却ループの回避**: severity 閾値で打ち切り、閾値未満は accepted-residual（記録のみ）。各 run は固定時間 box で完了する（EPIC 失敗シナリオ⑥）。
...
**gate を直す変更ほど、その gate 自身の検査を通す。** 直した gate が走らないまま入ると、「直したつもり」が本番まで届く（Draft ゆえ skip、等）。
