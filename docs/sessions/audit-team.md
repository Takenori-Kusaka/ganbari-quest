...
| 7 | 統合 PR の全 CI 成功を確認。fail は **1 件で止めず全件洗い出し**、各々 deepresearch で真因特定・なぜなぜ分析・横展開（影響範囲）まで行い **Issue 起票**。監査中の修正は release branch への commit（append）で対応する | audit-manager（起票） + 各領域 agent | **可** |
