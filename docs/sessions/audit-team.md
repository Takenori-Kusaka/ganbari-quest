...
- **merge commit への attestation 永続化（#2876）**: 統合 PR の merge（main push）を契機に `integration-attest.yml` が SARIF + in-toto Release predicate v0.2（`scripts/audit/generate-release-predicate.mjs`、subject = merge commit SHA / predicate = 含有 PR 一覧 + テスト結果 + NG-0 宣言）を生成し、`actions/attest` で Sigstore 署名 → GH attestations API へ永続化する。これにより `tmp/audit-evidence/` の揮発に依らず、merge 後も `gh attestation verify --repo Takenori-Kusaka/ganbari-quest --predicate-type https://in-toto.io/attestation/release/v0.2 <merge commit>` で audit trail（含有 PR 群 × テスト結果 × NG-0 宣言）を改ざん検知付きで遡れる。attestation 発行失敗は deploy を阻害せず（独立 job）、predicate / SARIF は artifact `integration-attestation-<run_id>` に degrade backup として残る。
- **merge 方式（#2938 項目 2）**: 統合 PR の merge は **merge commit 必須（`gh pr merge --merge`）・squash 禁止**。squash すると develop / main の履歴接続が切れ、次回統合 PR の diff が汚染される。develop は恒久 branch のため merge 後も削除しない（[qa-session.md](qa-session.md) §レビュー対象レーン と同一規定）。
...
| 8 | 全緑なら統合 PR を merge → 本番 deploy actions を watch し成功確認 → **main → develop back-merge sync PR** で squash commit を develop へ取り込む（§5 / #2951・#3061） | audit-manager（merge） | **可** |
