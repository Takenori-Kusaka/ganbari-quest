...
- 段階適用 (Issue #1839 / #1851 / #2395):
  ...
  - **Phase 3 (#2395 PR、完了 2026-05-22)**: `pamphlet.html` / `faq.html` / `selfhost.html` / `graduation.html` へ同パターンで波及 (4 HTML で baseline 72 → 0、削減率 100%)。content (faq / graduation / selfhost) 共通 hero / CTA / section spacing を `--lp-content-*` トークンに集約 + 4 HTML 個別の構造的 padding/margin (`--lp-faq-*` / `--lp-graduation-*` / `--lp-selfhost-*` / `--lp-pamphlet-*`) を Semantic 化。`scripts/lp-inline-style-baseline.json` は index / pricing / privacy / tokushoho の Phase 1/2 残値のみ pin。pamphlet.html の印刷用 mm 単位は対象外 (`check-lp-inline-style.mjs` が `px|em|rem|%` のみ検出)
...
| `PIN_RESET_LABELS` | const |  |
| `PIN_GATE_ONBOARDING_LABELS` | const |  |
| `IMPORT_LABELS` | const |  |
| `SETTINGS_LABELS` | const |  |
| `SETTINGS_NAV_LABELS` | const |  |
| `SUBSCRIPTION_PAGE_LABELS` | const |  |
| `LICENSE_PAGE_LABELS` | const |  |
| `UPGRADE_FLOW_LABELS` | const |  |
| `NUC_LICENSE_LABELS` | const |  |
| `REPORTS_LABELS` | const |  |
| `OPS_LABELS` | const |  |
| `POINTS_LABELS` | const |  |
| `SIGNUP_LABELS` | const |  |
| `BILLING_LABELS` | const |  |
| `IMMEDIATE_DOWNGRADE_CREDIT_BANNER_LABELS` | const |  |
| `PHASE4_REACTIVATION_FLOW_LABELS` | const |  |
...
| `ADMIN_ACTIVITIES_PAGE_LABELS` | const |  |
| `ADMIN_REWARDS_PAGE_LABELS` | const |  |
...
| `ADMIN_CHALLENGES_PAGE_LABELS` | const |  |
...
| `SETUP_ACTIVITIES_DEFAULTS_LABELS` | const |  |
| `SETUP_CHALLENGES_LABELS` | const |  |
...
| `OVERFLOW_MENU_LABELS` | const |  |
| `CHILD_SELECTION_LABELS` | const |  |
| `VISIBILITY_CHIP_LABELS` | const |  |
...
| `CHECKOUT_LABELS` | const |  |
| `LP_LEGAL_SLA_LABELS` | const |  |
| `LP_LEGAL_TOKUSHOHO_LABELS` | const |  |
| `UNIFIED_IMPORT_HUB_LABELS` | const |  |
| `UNIFIED_EMPTY_STATE_LABELS` | const |  |
...
**#2778 横展開事例 (admin/checklists、Cluster D)**: 同パターンを admin/checklists にも適用。旧「`+ テンプレート作成`」(primary) と「`📅 ワンオフ追加`」(outline) の 2 並列 button が User 指摘 #1「ボタンの重複」を生んだため、`Menu` primitive で「`+ 追加`」dropdown menu に集約 (menu 項目 = `+ テンプレート作成` / `📅 ワンオフ追加`)。同一リソース (持ち物チェックリスト) の add 経路を 1 入口に統合。
