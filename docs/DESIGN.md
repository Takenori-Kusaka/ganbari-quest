...
UI に表示されるラベル・用語は **`src/lib/domain/terms.ts` (atom) → `src/lib/domain/labels.ts` (compound)** の 2 階層 SSOT で管理する (#1916)。
...
│   PLAN_TERMS / PLAN_FULL_TERMS / PRICE_TERMS / TRIAL_TERMS /   │
│   CANCEL_TERMS / FREE_TERMS                                    │
...
- atom (terms.ts): 単一の用語。1 行修正で全 LP・アプリ本体・法務文書に伝播
- compound (labels.ts): 複数 atom を文に組み立てた表示文字列。`${TERMS.foo}` 経由で参照
- LP 側 (`site/shared-labels.js`) は `scripts/generate-lp-labels.mjs` で template literal を解決した後、文字列値として配信
