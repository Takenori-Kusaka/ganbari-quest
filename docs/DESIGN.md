...
- **OVERFLOW_MENU_TERMS**: `openLabel`=`'メニューを開く'` / `itemMarketplace`=`'みんなのテンプレから取込'` / `itemAiSuggest`=`'AI で提案してもらう'` / `itemRestore`=`'バックアップから復元'` / `itemRestoreIcon`=`'⬇'` / `itemExport`=`'エクスポート'` / `itemExportIcon`=`'⬆'` / `itemHelp`=`'このページのヘルプ'` / `itemFeedback`=`'ご意見を送る'` / `itemFeedbackIcon`=`'💬'`
...
- **admin layout に常設 FAB は置かない (現状 0 個、#2904)**。フィードバック導線は 設定 > サポート (`/admin/settings/support`) を SSOT とし、各リソース画面の `︙` overflow menu 末尾「ご意見を送る」(`AdminResourceHeader` が自動 append) から 1 hop で到達する (ADR-0012 anti-engagement / Clippy 型常時露出の回避)
...
| **︙ overflow menu** | `AdminResourceHeader` の `overflowItems` (内蔵 Menu) または `overflowSnippet`。**末尾に標準「ご意見を送る」item (`OVERFLOW_MENU_TERMS.itemFeedback` → `/admin/settings/support`) を `AdminResourceHeader` が自動 append する (#2904)**。snippet 経路のページは snippet 引数で受けた標準 item を自前 items 末尾に spread する | header 右端 |
