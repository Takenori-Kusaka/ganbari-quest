...
|--------|---------|-----------|---------|------|
| `baby` | 準備モード (0-2歳) | 1.5 | 120px | 保護者向け・子供向けゲーミフィケーション非適用 |
...
| 内部コード UI 露出 | ユーザーーに意味不明 | `eslint` + レビュー |
...
- **MilestoneBanner と MonthlyRewardDialog のシーケンス（#1722 AC2）**:
  ...
  - ユーザーが reward を受け取って閉じると `showOpening = false` となり modal が即座に消える
