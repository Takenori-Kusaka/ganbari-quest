...
- [x] ライセンスキー体系の設計（形式・発行・検証フロー）
- [x] サインアップフロー設計（Email/Password → ライセンスキー入力 → テナント作成）
- [x] ライセンスとテナント・ユーザーの紐付けモデル設計
- [x] DynamoDB ライセンス管理スキーマ設計
- [x] 買い切り / サブスク両対応の課金状態管理設計
- [x] Stripe Webhook → ライセンス発行・更新の連携設計
...
- [x] **Email/Password 認証の有効化** → **#0123**
- [x] **サインアップフロー** → **#0124**
- [x] **ログインフロー動作検証** → **#0125**
- [x] デバイストークン発行・検証 API → **#0129**
- [x] 運用 API（`/api/internal/*`）の API キー + IP 許可リスト認証実装（将来）
- [x] JWT 検証失敗時の統一エラーレスポンス実装（401/403）→ **#0127**

**Phase C: 家族共有機能**
- [x] 家族共有メカニズム設計・実装 → **#0128**
- [x] 共有招待（QRコード + 認証コード）→ **#0128**
- [x] 共有解除フロー → **#0128**
- [x] テナントメンバー管理画面 → **#0128**
- [x] デバイストークン（共用タブレット）→ **#0129**
- [x] 子供プロフィールのテナント紐付け → **#0132**
...
- [x] 二層セッションモデルのプロトタイプ実装（ローカル版で検証可能）
- [x] マルチテナント設計（#0065）との統合 — DynamoDB メンバーシップスキーマの確定
- [x] 利用規約（#0073）に解約・データ削除ポリシーを明記
- [x] プライバシーポリシー（#0074）にデータ保持期間を明記
- [x] Cognito Hosted UI のカスタマイズ（子供向けアプリに合ったデザイン）
- [x] Apple Sign-In の実装要件調査（App Store 審査要件との整合性）
- [x] デバイストークンの暗号化方式選定（JWT署名 vs DB紐付け）
- [x] Context Cookie 寿命のユーザビリティテスト（親モード30分は短すぎないか）
- [x] ライセンスキー再発行フローの UI/UX 設計（/billing/reissue）
- [x] SES による Email 一時コード送信の実装・テンプレート設計
- [x] AuthProvider インターフェース設計と NoneAuthProvider の実装（現行動作の抽象化）
- [x] BasicAuthProvider の設計 — 環境変数によるID/PW設定、PIN認証との統合
- [x] AUTH_MODE 環境変数の Docker / docker-compose.yml への組み込み
- [x] DATA_SOURCE（#0111）と AUTH_MODE の組み合わせマトリクス検証
- [x] 各 AUTH_MODE ごとの E2E テストシナリオ設計
- [x] JWT 検証ミドルウェアの実装 — JWKS キャッシュ戦略、キーローテーション対応
- [x] API エンドポイントの認証バイパス一覧の明示的管理（ホワイトリスト方式）
- [x] 運用 API（`/api/internal/*`）の API キー発行・ローテーション運用設計
- [x] Stripe Webhook 署名検証の実装（`constructEvent` + シークレットローテーション）
- [x] ティーン向け OAuth フローの未成年保護対応（Google Family Link 等との干渉確認）
