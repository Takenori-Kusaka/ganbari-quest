...
1. **国内アプリのプリセット数は概ね不足**。日本語圏のお手伝い系アプリ（ファミポイ / ママペイ / OurHome 日本語版 / ピグっち / AllowanceQuest）は「白紙ベース＋保護者が手入力」が基本で、業界標準とも言える「即使えるプリセット集」という概念自体が薄い。海外勢 (Joon: 500+ tasks, S'moresUp: 数十のビルトイン chore, Habitica: 自由度極大) と比較して 1〜2 桁の差がある。
...
|---|-----------|------|----|------------|-------|---------|------|--------------------------|
| 1 | **Habitica** | 家庭タスク（汎用） | 米 | 0（白紙＋ユーザーー作成） | − | 中立 | 個人無料 / グループ $9/月+$3/メンバー | "Tasks" / "Challenges"（公開チャレンジ集は "Tavern" 内 Challenges Tab） |
...
| 10 | **ファミポイ（AppSeed）** | お手伝いポイント | 日 | **0（白紙＋保護者が登録）** | − | 中立 | 無料 | （該当機能なし） |
| 11 | **ママペイ（kouki yuza）** | お手伝い + お小遣い | 日 | **0（白紙＋保護者が登録）** | − | 中立 | 無料 | （該当機能なし） |
...
**弱み**: プリセット活動が **0**。完全に「白紙からユーザーーが作る」前提。年齢配慮はなく英語 UI（公式 i18n あるが日本語化は限定的）。利用規約上 13 歳以上が基本で、低年齢層は完全に対象外。
...
**強み**: 数十の "Built-in Chores"（laundry, raking leaves, vacuuming, drinking water, reading a book, washing hands 等）。`ChoreAI` という機械学習システムで割当・リマインド・報酬を最適化、「保護者の時間を週平均 8 時間節約」と謳う。Family Campfires での保護者同士のコミュニティ。
...
- ChoreAI 的な「保護者の時間節約」訴求は、**共働き家庭ペルソナ** (`marketplace-persona-research.md` の `dual-income`) に強い説得力を持つ。
...
**弱み**: ゲーミフィケーション要素は皆無。子供向けというより保護者向けのオーガナイザー。
...
- "List Library" という呼称は、子供にとっても「としょかん」のメタファーで保護者しみやすい候補。
...
**強み (FamZoo)**: $2.50-$5.99/月で最も "in-depth" な chore system、chore の保護者同士分割払い、子供が "dibs" を主張可能、保護者レビュー後の支払い保留可能。

**弱み**: 米国市場前提（プリペイドデビットカードが基本）。日本の子供向け金融文化（子供銀行口座、お年玉、お小遣い帳）と整合せず、そのまま転用は不可。

**がんばりクエスト視点での示唆**:
- 金融教育の本格機能は **Pre-PMF では明示的に対象外**（ADR-0010 / ADR-0010 系の Pre-PMF 防衛設計禁止と整合）。ただし、「お小遣い相当の何か」（スクリーンタイム交換 / 特権交換）は本アプリ既存実装と整合し、Greenlight/FamZoo の「保護者レビュー後の付与」UI パターンは参考にできる。
...
**がんばりクエスト視点での示唆**:
- **「ポイント数の段階別景品」は日本の保護者子に既に十分浸透**。本アプリのごほうび設計（`reward-sets/`）に「進研ゼミ風 7 段階」のテンプレを 1 つ用意する価値がある。
...
**がんばりクエスト視点での示唆**:
- **「マイキャラショップ」**は「マーケットプレイス」より子供に保護者しみやすい呼称の参考事例。ただし、本アプリは「無料配布の活動パック」が中核なので「ショップ」という売買ニュアンスは過剰。**「マイキャラ → マイがんばり」「ショップ → おみせ / おうち」** などの語尾アレンジが候補。
...
**強み**: 完全無料、保護者用 / 子用ページ分離、グラフ機能（週/月/6 ヶ月）、履歴機能、ポイント通知機能。「メンバー編集」「リスト編集」「交換アイテム編集」のシンプルな 3 軸構成。

**弱み**: プリセット活動が **0**（保護者が手で登録）。報酬設計は「ポイント蓄積→交換アイテム」のシンプルな 2 段階のみ。年齢別の動機構造設計なし。
