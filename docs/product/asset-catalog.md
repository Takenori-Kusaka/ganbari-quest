...
| 条件 | 理由 |
|------|------|
| ユーザーーが「コレクション」するもの | 収集体験の満足度は視覚品質に直結する |
...
| 活動アイコン（ユーザーーがカスタマイズする前提） | 🏃 うんどう、📚 べんきょう 等 |
...
- **家庭視点ベネフィット（1 行）併記**（#1793）: 各 scrshot 直下に文脈別の 1 行ベネフィット（例: 「家庭で起きること」「家庭で楽になること」「家庭ごとにカスタマイズできること」「家族で実感できること」）を必ず併記し、機能の効果を「保護者が監視する」のではなく「家庭で実感する」視点で言語化する。旧表記「保護者が観測できること」は 2026-05 に刷新済み（PO 指針: 監視連想で permission marketing が毀損するため不採用）
...
| 要素 | 理由 |
|------|------|
| 活動アイコン（🤲💧 等） | ユーザーーが自由にカスタマイズする設計。絵文字の多様性が利点 |
