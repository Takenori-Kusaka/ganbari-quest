...
- [ ] **移管実行時の家族影響台帳（利用家族への影響を明示評価してから実行する）**: 本節は移管メリット（二重実装税解消）側の評価に偏っていたため、利用家族側のリスク 3 点を台帳化する。(1) **cutover ダウンタイム** — 切替中に活動記録・ポイント反映が止まる時間の見積と告知方法、(2) **データ移行の完全性** — 件数・ポイント残高・履歴の round-trip 検証（移行前後の突合で欠損ゼロを機械確認）、(3) **参照整合性（FK なし）の喪失リスク** — DSQL は外部キー制約非対応のため、移行データの orphan 行検出と以後のアプリ層整合の担保。再評価トリガが発火しても、この 3 点の影響評価を経ずに移管を実行しない
- **既定路線化しない注記**: 本 rationale が DSQL を「将来の有力候補」と記録することは、**将来移管を既定路線とする決定ではない**。移管の実行可否は再評価時点で上記の家族影響台帳を含めて改めて判断する（実際の実行判断は EPIC #3424 が別途担った — 冒頭 supersede 注記参照）
...
- **影響を受ける設計書**: `docs/design/08-データベース設計書.md` / `docs/design/parallel-implementations.md`「並行実装ペア一覧」の「7. シードデータ vs マイグレーション」
