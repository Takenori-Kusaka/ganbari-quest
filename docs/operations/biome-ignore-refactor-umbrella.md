...
biome の complexity rule (`noExcessiveCognitiveComplexity` / `useMaxParams`) を **45 箇所 / 39 ファイル** で `biome-ignore` 指定で suppress しており、コメントの 8 割が「複雑なビジネスロジックのため、別 Issue でリファクタ予定」のままになっている。コードレビュー時の毎回の説明コスト・新規開発者の理解障害・テスト困難化が累積している。
...
`grep -rn "biome-ignore.*complexity" src/` で **45 箇所**ヒット。ユニークファイル数 **39 件**。
...
| Rule | 件数 |
|------|------|
| `lint/complexity/noExcessiveCognitiveComplexity` | 31 |
