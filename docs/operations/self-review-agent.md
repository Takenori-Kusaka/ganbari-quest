...
### §2.1 qa-session.md 5 手順 (Self-Review でも実施)
...
| 5 | **過去 QA 指摘事前回避 (7 件)** | ADR 番号衝突 / ADR deprecation chain / copilot-instructions 同期 / todo trap / label / SS / discriminated union | `feedback_check_qa_merge_fixes.md` 参照 |
...
5. memory `feedback_check_qa_merge_fixes.md` (過去 QA 指摘 学び)
...
- 1 Agent = 1 PR (qa-session.md と同じ)
- 700 行以内で報告
```
...
| 4+ 件 | Fix Agent spawn (qa-session.md CI Fix Agent パターン踏襲) → 再 spawn |
...
- 該当 BLOCK 種類を本 docs §2.2 観点表に追加
- memory `feedback_check_qa_merge_fixes.md` に反映
