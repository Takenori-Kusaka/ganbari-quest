...
トラブルシューティング:
- Webhook が 400 エラー: signing secret 配備ミス → ステップ 5-1 / 5-2 / 5-3 を再確認
- Webhook が 401 / 403: アプリ側の `verifyStripeWebhook()` で署名検証失敗 → secret の改行混入 / 文字化けチェック
- Webhook が 500: アプリ側の例外 → CloudWatch Logs `/aws/lambda/ganbari-quest-app` 参照
...
| 日付 | 版数 | 内容 |
|------|------|------|
| 2026-05-18 | 1.0 | #2101 初版作成（Phase 1/2/3 + 配布証跡 + Phase F research D-4 / D-5 転記）|
