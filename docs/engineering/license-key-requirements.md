...
| # | 課題 | 影響 | 対応 Issue |
|---|------|------|----------|
| 1 | `consumeLicenseKey` が tenant の plan を更新しない | 決済完了してもユーザーーは無料プランのまま | #796 |
| 2 | `/admin/license` に既存ユーザーー向けの適用 UI・action が存在しない | 購入後にキーを入力する画面がない | #796 |

### 1.2 高優先度（High）

| # | 課題 | 影響 | 対応 Issue |
|---|------|------|----------|
| 3 | owner 制約がない | 子供アカウントでも保護者のライセンスキーを消費できてしまう | #798 |
| 4 | 一回限り使用の警告ダイアログがない | 入力ミスでキーが消費され、ユーザーーサポート負荷増 | #799 |
