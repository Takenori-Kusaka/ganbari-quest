...
ADR-0010 (Pre-PMF Issue 優先度判断基準) では、Pre-PMF 期の最優先課題は **「最初の 100 ユーザーーの獲得と継続」** と定義されている。本アプリの Free → 有料転換ファネルは以下:
...
- LP (`site/index.html`) では既に **「持ち物チェックリスト機能」** を訴求している (#1164 で LP スクショ対応済)。
- しかしマーケットプレイス側には持ち物パックが **15 個全て (morning/evening/weekend × baby/kinder/elementary/junior/senior)** あるが、これは **「日課ルーティン」** であって LP の訴求である **「遠足・修学旅行・季節行事の持ち物」** ではない。
...
5. **国内お手伝い系アプリは pre-loaded 0**。「白紙＋保護者手入力」が業界常識 → 本アプリの「200+ プリセット」は構造的差別化
...
→ **「ポイント数の段階別景品」と「アバター進化」は日本の保護者子に既に十分浸透**しており、本アプリの報酬設計はこれらに **学ぶべき**。一方、ママペイの「ポイントマイナス機能」は Erikson「勤勉性 vs 劣等感」の劣等感を強化するため **意図的に採用しない**。
...
|------|---------|-------|---------|--------|---------|------|---------|
| baby (0-2) | 6 | 4 | 2 | 2 | 2 | **16** | 厚労省睡眠ガイド / 子供家庭庁発育調査 |
...
**互換性**: 性別 ID は維持されるため `LEGACY_PACK_ID_MAP` 不要。既存ユーザーーの `pack_id='baby-boy'` 等はそのまま動作継続。
...
|------|------------|---------|---------|
| baby (0-2) | 基本的信頼 / 自律性 vs 恥 | **感覚刺激 / 共感応答** | 保護者の笑顔・拍手・効果音 + 大きなアニメ。シール画像不使用、振動・色変化のみ |
...
| junior (13-15) | 同一性 vs 役割の混乱 | **自己効力感 / 数値目標** | 数値ダッシュボード + 自己ベスト更新 + 保護者介入最小化 |
...
| 世代 | 補強活動 (一部抜粋) | 出典 |
|------|------------------|------|
| baby | ねがえりした / つかまりだちした / ベビーリトミック / 五感あそびした | たまひよ / 子供ちゃれんじ baby |
| kinder | こんだてをいっしょにきめた / おみせやさんごっこ / ペットのおせわ | 子供ちゃれんじぽけっと / 食生活支援ガイド / 10 の姿 |
...
| **ページタイトル** | **みんなのテンプレート** | 「みんなの〜」(NHK みんなのうた等) で安心感 + 保護者しみ |
...
- 既存 JSON は **すべて optional フィールド追加のみ** で、後方互換性は保たれる (ADR-0006 準拠)
...
- **デフォルト (中立)** は「性別を回答しない / 未設定」ユーザーへの自動適用パック。3 性別ではない
...
| ユーザーー平均インポートパック数 | (未計測) | 3+ パック / ユーザーー |
...
| カスタム活動を作成するユーザーー比率 (custom-need の発火率) | 40%+ |
| 持ち物テンプレ import 数 (LP 訴求の効果計測) | 30%+ ユーザーーが 1 回以上 import |
...
本 Epic は ADR-0008 (設計ポリシー先行確認フロー) に従い、以下 2 件の ADR を **実装前に PO 合意取得** する:
...
- 結果: ADR-0009 (labels.ts SSOT) と整合
...
|------|---------|------------|
| toddler-rewards (0-3) | 感覚刺激 / 保護者子共感 | 10 |
...
| F2 | アバター進化 (たまご → ひよこ → 子供) UI | asset-catalog 画像化前提 |
| F3 | 進研ゼミ風 7 段階景品交換 | 金銭交換は Pre-PMF 範囲外 (ADR-0010) |
| F4 | 月次「すごいキミ」表彰 (家族内ランキング) | 既存 `child-stats` に集計データはあり、UI 未実装 |
| F5 | 簡易ユーザーーテスト (社内モニター 5-10 名 × 30 分) で命名最終確定 | naming-recommendation.md §5 補足記載の通り、定性インタビュー未実施 |
...
1. **`docs/design/marketplace-content-audit.md`** §7 (34 出典)
   - 文科省: 幼稚園教育要領 / 小・中・高学習指導要領 / 「早寝早起き朝ごはん」運動 / 子供の読書活動推進
   - 厚労省: 健康づくり睡眠ガイド 2023 / 身体活動・運動ガイド 2023 / 保育所保育指針対照表
   - スポーツ庁: 全国体力・運動能力等調査 / 体力・運動能力調査
   - 子供家庭庁: 令和 5 年乳幼児身体発育調査 / 未就学児の睡眠指針
...
3. **`docs/design/family-discipline-research.md`** §5 (27 新規 + 第 1 フェーズ再掲)
   - 公的: 子供家庭庁未就学児睡眠指針 / 厚労省乳幼児生活習慣 / 国立保健医療科学院栄養ガイド / 文科白書 / 小児科学会成人移行支援 / スポーツ庁部活ガイドライン
   - 育児雑誌: たまひよ / 子供ちゃれんじ baby/ぷち/ぽけっと/ほっぷ/すてっぷ/じゃんぷ / kodomoe / Baby-mo
   - 民間: HugKum / 子供まなび☆ラボ
