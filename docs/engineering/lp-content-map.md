...
- LP のコンテンツ更新が場当たり的になり、類似機能が重複訴求される（例: 「5つのチカラチャート」と「成長記録 & 管理画面」が両方とも「保護者向けの成長可視化」）
...
- **対象外**: アプリ内 (`src/routes/`) の画面設計、SEO 文言詳細、広告クリエイティブ、`site/help/license-key.html`（既存ユーザーーへの操作ガイドであり LP スコープ外。#1736 m-MIN-11）
...
|---|---|---|---|---|
| **P1** | 3 歳児の保護者 | 28-35 歳 | トイトレ・生活習慣、スマホ依存への不安 | 「うちの子でもタップできる」「シール帳の代替」と確信 |
| **P2** | 小学生の保護者 | 30-45 歳 | 宿題・お手伝い習慣化、教育関心高 | 「ポイント・ウィークリーチャレンジで自走する」「保護者が褒める UI がある」と納得 |
...
※ baby (0-2 歳) は ADR-0011 で「保護者の準備モード」として別軸扱い。
...
- **SLA の詳細**: 意思決定に使うユーザーーは少数。footer からの参照で十分
...
### 3.5.1 規範 A: [02b] age-panel — 大 scrshot 1 + 保護者主語短文

| 項目 | 規範値 |
|------|--------|
| 構造 | 大 scrshot 1 + h3 保護者主語 1 行 + UI 軸特徴 1 行 + 学年訴求 1 文 + CTA |
| 情報密度 | **low** |
| scrshot 占有率 | **高** (≥ 35%) |
| 主語 | 保護者 |
...
| 主語統一 | h3 / p で「保護者が」「お子さま」の主語が一貫しているか | 保護者主語必須 | 保護者主語必須 |
...
| 05 | ソフト機能 | 保護者の安心（手間軽減、ゲームっぽさの中和） (#1623 R19 で責務分割) | 1300 | 800 |
...
- **H1**: 「やりなさい」を「やりたい！」に変える家族の冒険アプリ（#1899: IT リテラシーなし保護者対応で旧「家族 RPG」から書換え。RPG はサブ訴求 / meta description で SEO 維持）
...
2. **4 行に圧縮、5 行以上は禁止**: ADR-0023 §5 I5 で示された 4-5 個の優位点を **4 行** に確定する（自動集計 / 3-18 歳継続 / 卒業設計 / 場所自由）。「保護者の負担軽減」は他セクションで重複訴求されるため本セクションでは外し、認知負荷を最小化する（離脱の臨界点で 30 秒読みに 4 行が上限）。
...
|----|---------------------------|--------------------------------|
| 1 | お手伝いの種類が増えたり貼る場所がなくなっちゃう | **自由に子供の活動をカスタマイズ** — 子供のフェーズに合わせた活動を予めご用意 |
| 2 | どれだけ頑張ってきたか振り返るのが大変！ | **日々の活動実績をポイントでわかりやすく** — 3 歳から 18 歳まで同じアプリで継続 |
...
- **構造**: L1 アクション層 / L2 メタ習慣層 / L3 経済層 の 3 層縦積み + 保護者子両視点バナー (#1343、`LP_CORELOOP_LABELS`)
  - **L1 アクション層**: 活動を 2 タップで記録 → ポイント獲得
  - **L2 メタ習慣層**: 毎朝おみくじスタンプ（日 1 回 cap）→ スタンプカード完成（週次）
  - **L3 経済層**: ポイント蓄積 → ごほうびショップ（唯一の出口）で実物・お小遣い・特権と交換
- **訴求**: 保護者視点（プリセット設定 2 分・定量把握）と子供視点（活動→ポイント→ショップ交換）を同一セクション内で両面訴求
...
#### [05] ソフト機能（保護者の安心）— 3 カード構成（#1720 R4 で 4→3 圧縮、#1847 PO-N-5 で featured 強調完全撤去・3 cards 視覚均質化）

1. **成長の記録（月次レポート）** (`data-testid="feature-monthly-report"`): 月次レポートで活動・ポイント推移を可視化（`/admin/reports` 実装済み）
2. **家庭に寄り添う運用補助** (`data-testid="feature-auto-sleep"`): 自動スリープ + おうえんメッセージを統合した 1 カード。設定時間超過で自動スリープ（`src/lib/features/auto-sleep.ts` 実装済み, #1292）+ 必要時に保護者→子のおうえんメッセージ（Family プランで家族全員から送信可）
3. **設定の自由度** (`data-testid="feature-customization"`): 活動・ポイント配分・ごほうびをカスタマイズ可能
- → **意図的にゲーム要素を挟まない**。保護者が「これは遊びだけでは終わらない」と判断するセクション
...
- 旧カード「みんなのテンプレート」「保護者の作業は 1 日 5 分」は削除（[04] / 設定自由度カードに統合）。旧カード「おうえんメッセージ」は #1720 で「家庭に寄り添う運用補助」に統合
...
3. **ADR-0011 整合**: 4 コア年齢モード (preschool / elementary / junior / senior) を全て stage に登場させ、コアターゲット 3-18 歳を物語上で網羅する。0-2 歳 baby（準備モード）は LP の成長物語に含めない（baby は保護者の準備モードで、子供 UI ゲーミフィケーションが非適用のため）。
...
- **5 stage 内訳（graduation.html 詳細）** (#1712 R5 で H3 を「保護者主語ベネフィット」にリフレーム):

| data-stage | 年齢ラベル | h2（保護者主語ベネフィット） | scrshot サムネ | 保護者が観測できること | 子供が体験すること |
|-----------|-----------|----------------------|--------------|------------------|------------------|
...
- **ラベル管理**: 全 H2 / 訴求文 / 年齢ラベル / 保護者視点 / 子供視点ベネフィットは `LP_GROWTH_ROADMAP_LABELS`（labels.ts）→ `shared-labels.js` 経由で `data-lp-key="growthRoadmap.*"` で注入（並行実装チェックリストに準拠）。#1848 で追加された `linkLabel` / `pageTitle` / `pageHeroTitle` / `pageHeroLead` / `pageMetaDescription` / `breadcrumbHome` / `breadcrumbCurrent` / `ctaBottomTitle` / `ctaBottomDesc` も同チャネル
- **背景**: graduation.html では `bg-gray` 相当の `.graduation-detail` セクション（ソフト機能 [05] と同系統で「保護者の安心」文脈を継承）
...
- **LP 内では 1 バンドのみ**: 「基本無料 ・ 有料は月 ¥500〜 ・ 7 日間無料トライアル ・ いつでも解約できます（契約期間の縛りなし）」（#1904 PERS-CRT-5: 旧「いつでも解約 OK」を動詞ベース + 契約期間明示に書換え。サブスク被害連想を断つ）
...
- LP 本文には「OSS / ソースコード / サーバーー / 自前運用」を出さない（#1286、IT 非リテラシ保護者への伝達性を優先）。同語を記載したいページは `selfhost.html` 等の詳細ページに閉じ込める
...
| 3 | 運用負荷（毎日設定が必要？） | `#soft-features`「保護者の作業は 1 日 5 分」 | テキスト | High | ✅ #1289 で追加 |
...
- 懸念 3 (運用負荷): GIF/動画（「保護者が毎朝 30 秒で何をするか」フロー）は #1292 実装後に追加予定
...
| **(iii) 役割識別** | core-loop `🧑‍💻` 保護者の視点 / `🧒` 子供の視点 | **保持**: 短時間で役割を識別する目的に限る（数を増やさない） |
...
hamburger `☰` (1) + 準備モード注釈 `👶` (1) + 保護者子視点 `🧑‍💻` `🧒` (2) + lightbox close `✕` (1) = 計 5 件
...
| 活動パック | [05] ソフト機能 | 保護者向けオンボ |
| おうえんメッセージ | [05] ソフト機能 | 保護者 → 子 コミュニケーション |
...
- `ログイン` を `btn-primary` として装飾しない（primary CTA はサインアップ = 新規獲得専用。ログインは既存ユーザーー復帰のみで役割が違う）
...
**語彙ポリシー（#1286）**: `OSS` / `ソースコード` / `サーバー` / `自前運用` は「運営の不安を減らす安心訴求」のために LP 本体で直接書かないこと。代わりに「仕組みを全て公開」「運営が止まっても続けられる準備」のような非 IT 保護者にも伝わる日本語で置き換え、開発者向け語彙は `selfhost.html` 等の詳細ページに閉じ込める。`scripts/measure-lp-dimensions.mjs` が `site/index.html` (TARGET_HTML) をスキャンし、カウント 1 以上で CI を fail させる。
...
| 2026-04-21 | §7.2 の「ログインは ボタン化しない」ルールを supersede。ヘッダー右上の `ログイン` を `nav-login` ghost button として復帰（shared.css に追加 / index.html + selfhost.html に適用）。既存ユーザーーの自分のマイページ復帰導線の明示化。 | #1285 (umbrella #1262 派生 A5) |
...
| 2026-04-27 | [05] ソフト機能を 3 カード → 4 カードに拡張（保護者ペルソナ訴求強化）。「みんなのテンプレート」「保護者の作業は 1 日 5 分」カードを削除し、「成長の記録（月次レポート）」「時間管理（使いすぎ防止）」「おうえんメッセージ」「設定の自由度」4 カードに再編。ADR-0013 準拠（月次レポート: `/admin/reports`、自動スリープ: `src/lib/features/auto-sleep.ts` #1292、おうえんメッセージ: 既存、設定カスタマイズ: 既存実装済み）。LP-LANDMARK コメントを更新。 | #1287 (umbrella #1262 派生 A7) |
