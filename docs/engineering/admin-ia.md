...
| `settings` | アカウント | 子供 / 設定 / プラン / 請求管理 / メンバー |
...
- **「子供」が「アカウント」に入っている問題**: 子供の追加・設定変更は週次操作（活動設定と同文脈）だが、設定・プランと同じ「稀」文脈のカテゴリに格納されている
...
| **活動** | `activity` | 活動 | 活動管理 / チェックリスト / イベント / チャレンジ / テンプレート / **子供** | 週次 |
...
|------|------------|-----------|------|
| 子供 | `settings`（アカウント） | `activity`（活動） | 子供追加・設定変更は活動登録と同じ週次操作文脈 |
