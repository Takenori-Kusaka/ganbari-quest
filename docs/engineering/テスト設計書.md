...
- **4歳児視点**: UIテストは「読めない」「タップのみ」を前提にユーザービリティ観点を含める
...
#### E2E-08: 保護者が活動の表示/非表示を切り替える（UC-08）
...
通常の機能テストに加え、以下のユーザービリティ観点を確認する。
...
不具合（バグ）を修正する際は、**再発防止のためのリグレッションテストを必ず追加する**。

#### 基本ルール

1. **バグ修正にはテストを伴うこと**: 不具合チケットの完了条件に「再発防止テストの追加」を含める
2. **テストレベルの選択**: 原則としてユニットテストで十分。API境界の不具合はAPIテスト（統合テスト）で対応
3. **テストは修正前に失敗することを確認**: 可能であれば、修正適用前にテストが失敗する（Red）ことを確認してから修正（Green）を行う

#### テスト対象の判断基準

| 不具合の種類 | テストレベル | 例 |
|------------|-----------|---|
| サービス層のロジック不具合 | ユニットテスト | ポイント計算ミス、重複表示、状態遷移の不整合 |
| DB スキーマ・クエリの不具合 | ユニットテスト（インメモリDB） | カラム不足、クエリ条件ミス |
| API レスポンスの不具合 | 統合テスト（APIテスト） | ステータスコード誤り、レスポンス形式不正 |
| UI 表示の不具合（ロジック起因） | ユニットテスト（純粋ロジック） | 正規化計算ミス、座標計算ミス |
| UI 表示の不具合（CSS/レイアウト起因） | 目視確認のみ（テスト不要） | 余白調整、フォントサイズ変更 |

#### 命名規約

リグレッションテストは既存のテストファイルに追加し、テスト名に不具合の内容を明記する：

```typescript
it('表示済みの報酬は再表示されない (#0089)', () => { ... });
it('偏差値ベースの正規化で星評価と整合する (#0082)', () => { ... });
```
...
| チケット | 不具合内容 | 追加テスト |
|---------|----------|----------|
| #0082 | レーダーチャートプロット/星評価の乖離 | `tests/unit/ui/RadarChart.test.ts` — 正規化・座標計算 |
| #0088 | レーダーチャートラベル見切れ | `tests/unit/ui/RadarChart.test.ts` — viewBox計算 |
| #0089 | ごほうび表示の重複 | `tests/unit/services/special-reward-service.test.ts` — 未表示/表示済み管理 |
