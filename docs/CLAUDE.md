...
| 6 type データモデル scope (ADR-0053) | `data-model-resource-scope.md` (per-child / family master 選択 SSOT、PR-3〜7 で参照) |
| Marketplace 取込フロー (ADR-0053 §3.2) | `marketplace-import-flow.md` (child binding sequence + CWE-598 privacy SSOT) |
...
| `lp-visual-regression` (#2401) | per-image diff ≤ 10% | `scripts/lp-screenshot-baseline/*.webp` (git tracked) と CI 撮影 `site/screenshots/*.webp` を pixelmatch 比較。diff > 10% で fail。意図的変更時は `node scripts/check-lp-visual-regression.mjs --update-baseline` で更新 (PR #1893 Phase 2) |
...
### prh / textlint `--fix` 一括禁止運用 (#2243 / #2223 教訓)

PR #2223 Epic 2 (142 files) BLOCK の構造的原因は **textlint preset-ja-spacing / prh の一括 `--fix` 適用で機械的誤変換 (`ユーザーローカル` → `ユーザーーローカル` の長音重複) と CommonMark 仕様外の `- - text` ネストが混入し、142 files の手動レビューが事実上不可能になった**ことにある。

#2243 (Re-Issue) では以下を運用ルール化:

- **`--fix` 一括適用は禁止**。dry-run でルール提案を確認し、ファイルごとに目視レビューしてから手動修正する
- **新規 prh rule 追加時**: (a) dry-run で全 docs/zenn/ への影響件数を計測、(b) サンプル 5 件目視、(c) 短い pattern (例: 「ユーザー」) は他語彙への誤マッチに注意
- 詳細は `docs/zenn/README.md` §3.7 / `docs/zenn/prh.yml` 冒頭コメント参照

#2243 の本体 PR は **config 整理のみで実 docs 変更を伴わない予防策** として実施。冗長文章 rewrite (旧 Split C) は Pre-PMF 過剰防衛 (ADR-0010 §3) として skip。
