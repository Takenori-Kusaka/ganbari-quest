...
## §10 PO 判断結果 (2026-05-23、`tmp/user-question/2026-05-23-import-hub-ux-redesign-v2-qa.md`)

### Q&A 確定事項

| Q | 判断 | 補足 |
|---|---|---|
| Q1 戦略方針 | **v2 採用** | Pre-PMF でも離脱体験 = クリティカル不具合扱い、徹底回収 |
| Q2 PR 分割 | **8 PR 構造** (抽象 → 具象 OOP 落とし込み) | 旧 7 PR 案は OOP 階層化が欠落 |
| Q3 既存データ | **廃棄** (本番 user 0、α tester 0) + データモデル設計原則 PR で根本是正 | per-child instance 採用の再発防止が PR-1 のメインゴール |
| Q4 age filter | **A. activities と完全同型** | 顧客の学習コスト共通化、anti-engagement (ADR-0012) |
| Q5 chip primitive | **A. 新 primitive 化** | DRY + Storybook visual test + SSOT |
| Q6 demo 同期 | **N/A (削除済)** | 旧 demo 並行実装は #2188 (PR-B3) で撤去済 (ADR-0048 env-driven demo に置換) |
| Q7 v1 PR #2436 | **merge + ヘッダ注記** | QA team merge 依頼、v1 docs ヘッダで「v2 で覆った」明記 |
| Q8 着手タイミング | **B. 全並行 PR merge 後** | file 競合回避 |

### 補足 5 認識 — User 同意事項

| 認識 | User 回答 |
|---|---|
| v2 paradigm shift は方向性として正しい | 方向性として正しい |
| privacy 懸念 (OWASP CWE-598 + COPPA) は重要 | 重要 |
| 「activities と他 type のデータモデル不整合」は技術負債 | **技術負債解消も今回スコープに含めたい** |
| Phase 2/3 (per-child schema 縮退) は PMF 後で良い | **よくない、やり切り** |
| 本 redesign は PMF 直結 | 同意 |

→ **「やり切り」モード**: 旧 4 Phase (1a/1b/2/3) を全 PR スコープに統合、PMF 後送りなし。

### 8 PR 統合 plan (本 docs §7/§8 を本セクションが supersede)

| Layer | PR | 内容 |
|---|---|---|
| Abstract | **PR-1** | データモデル設計原則 ADR + 旧 schema drop + 新 master + preferences 適用 (やりきり: `special_rewards` / `checklist_templates` 等廃棄) |
| Framework | **PR-2** | `VisibilityChipGroup` / `OverflowMenu` primitive + Pattern Z/Y 規約 docs + DESIGN.md §5 / アーキ設計書同期 + Storybook |
| Concrete | **PR-3** | activity-pack: service refactor + admin/activities + E2E + SS |
| Concrete | **PR-4** | reward-set: service refactor + admin/rewards + E2E + SS |
| Concrete | **PR-5** | checklist: service refactor + admin/checklists + E2E + SS |
| Concrete | **PR-6** | rule-preset: service refactor + admin/settings/rules + E2E + SS |
| Concrete | **PR-7** | challenge-set: service refactor + admin/challenges + E2E + SS |
| Impact | **PR-8** | demo Lambda 動作確認 + LP SS 再撮影 + 設計書最終同期 + 棚卸 |

**依存順**: PR-1 → PR-2 → PR-3〜7 並列 (file 競合なし) → PR-8

### Phase 旧 → 8 PR mapping

| 旧 Phase | 旧範囲 | 新 PR 統合 |
|---|---|---|
| 1a | activity-pack 整理 | PR-3 (実体) |
| 1b | reward / checklist / rule / challenge master 化 | PR-4 / 5 / 6 / 7 |
| 2 | 既存 per-child data バックフィル | PR-1 (廃棄で吸収) |
| 3 | per-child schema 縮退 | PR-1 (drop 同時実行で吸収) |

### 着手 schedule

- **本日 (並行 PR merge 待ち中)**: PR-1 事前ドラフト準備 (ADR 起草 + 新旧 schema 比較表)
- **全並行 PR merge 完了後**: PR-1 着手 → PR-2 → PR-3〜7 並列 → PR-8
