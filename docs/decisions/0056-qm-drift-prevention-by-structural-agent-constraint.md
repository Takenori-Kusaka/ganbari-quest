...
- Review / Re-Review agent の責務は **V-0〜V-6 (semantic verify + evidence 生成・報告) で完結**。V-7 approve action (account switch → approve → merge → 復帰 → cleanup) は **QM Orchestrator 本体が完了報告受領後に直接実行** (qm-session.md §「全手順 Pass → approve & merge」)
...
本 §E と並行で Dev Agent 側にも `Ready 化前 5 項目 SSOT` を明示 (`docs/sessions/dev-session.md` + `.claude/skills/dev-open-pr/SKILL.md`)。Persona Drift が QM 側で発生してもなお Dev 側で `pre-ready -- --pr <N>` PASS が verify されていれば、QM Tier 2 Review (BLOCK 列挙工程) の発生回数自体が減り QM 本質判定時間が確保される (defense in depth 第 3 層)。
...
| 結果 | hook 自身が QA 機構毀損 (User 明示 priority「QM チーム時間取り戻し」と真逆動作) |
