...
| Phase 3 (別 Issue) | `pamphlet.html` / `faq.html` / `selfhost.html` / `graduation.html` へ同パターンで波及 + baseline 縮小 | Dev (別 Issue 起票予定) | 未着手 |
...
| トークン | 値 | 用途 |
|---------|----|------|
| `--lp-tour-card-padding-y` / `-x` | `var(--space-4)` / `14px` | `.tour-card` 既定 (16px / 14px) |
| `--lp-tour-card-padding-y-md` / `-x-md` | `var(--space-6)` / `22px` | `.tour-card` @≥1024px (24px / 22px) |
| `--lp-tour-card-padding-y-lg` / `-x-lg` | `var(--space-7)` / `var(--space-6)` | `.tour-card` @≥1440px (28px / 24px) |
| `--lp-soft-card-padding-y` / `-x` | `var(--space-5)` / `18px` | `.soft-card` (20px / 18px) |
...
| `--lp-core-loop-card-padding` | `18px` | `.core-loop-card` 上下左右 |
