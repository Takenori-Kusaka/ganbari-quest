...
- ADR-0050（Parent-Gate Session Cookie 署名方式）— 本 ADR の OSS 先調査原則を `cookie-signature` 採用で実適用
- #1346（本 Issue）/ #1350（OSS 先調査プロセス化）
- #566 / #1126（LP SSOT パイプラインの半完成の原点）

## 機能別追補: OYAKAGI_LABELS PIN gate modal 系（EPIC #2310 / 2026-05-20）

EPIC #2310 Parent-Gate で `OYAKAGI_LABELS` に下記キーを追加した。compound (labels.ts) として `terms.ts` atom を直書きしない原則 (ADR-0045) を守りながら、PIN gate modal / lockout 通知 / banner の UX 用語を SSOT 化:

| key | 用途 |
|---|---|
| `gateModalTitle` | PIN modal タイトル「おやカギコードを入力してください」 |
| `gateModalDescription` | modal 説明文 (ご家族の見守り画面に入る前の説明) |
| `gateModalSubmitting` | 認証中表示 |
| `gateLockoutNotice(sec)` | lockout 残り秒表示 (関数形式) |
| `gateFormatNotice` | 4〜6 桁数字フォーマットエラー |
| `gateGenericError` | 通信失敗等のフォールバック |
| `gateDefaultHint` | 初期値ヒント (5086) |
| `gatePinRequiredBanner` | `/switch?pinRequired=1` 到達時の info banner 文言 |

既存 `OYAKAGI_LABELS.invalidError` / `lockedError` は再利用（重複追加せず compound 互換性を維持）。
