...
| 日付 | 2026-05-23 |
| 起票者 | Dev (Claude) |
| 関連 Issue | #2401 / PR #2435 / PR #1893 (Phase 1) / #2520 (child-home 拡張) / CX-DoR #8 (app 拡張) |
...
- **運用手順**: baseline 更新コマンド / 判断基準 / CI fail triage は [`docs/runbooks/lp-visual-regression-baseline.md`](../runbooks/lp-visual-regression-baseline.md) (#2452) を参照

### app / child-home 拡張 (2026-06-03 補追、#2520 / CX-DoR #8)

本 ADR の汎用 pixelmatch ロジック (`scripts/check-lp-visual-regression.mjs` の `--baseline-dir` / `--current-dir`) は LP 専用に設計したが、**汎用化されているため LP 以外の baseline にもそのまま適用できる** (使い捨て script 禁止 #1442 整合)。これを利用し 2 件の拡張を追加した:

- **#2520 (child-home 拡張)**: `scripts/child-home-baseline/` (child home 4 mode + battle) + `child-home-visual-regression.yml`。per-child refactor (ADR-0055) で頻発する「子供 UI 破綻」を検出。
- **CX-DoR #8 (app 拡張)**: `scripts/app-screenshot-baseline/` (baby home + admin/activities + admin/checklists) + `capture-app-baseline.mjs` + `app-visual-regression.yml`。#2520 未カバーの 5 番目 age mode (baby、ADR-0011 準備モード) + admin critical 画面 (marketplace 取込 CUJ 受領先) を補完。

3 層合算で **5 age mode home + admin critical 画面** の見た目回帰を機械検出する (`tests/CLAUDE.md` 条件 8、`docs/CLAUDE.md` §visual regression 3 層)。child-home / app 層はいずれも初回 warn (continue-on-error)、安定後に hard-fail 昇格判断 (別 Issue)。新規 OSS / 機構の追加はなく、本 ADR の pixelmatch 採用判断をそのまま継承する (新規 ADR 不要、本 ADR 拡張で完結 = 1-in-1-out 回避)。
