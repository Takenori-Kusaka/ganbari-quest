...
- アカウントを削除したユーザーーに**翌月以降も課金が続く**
- ユーザーーからのクレーム → チャージバック → Stripe アカウントのリスク
- 信頼失墜・炎上の温床
...
**ユーザーー起因の「削除」は、常にデータと課金の両方に同期して適用する。**
...
2. **Stripe 失敗時は DB 削除を中断**
   ...
   - ユーザーーは削除に失敗したことを認識し、サポートに問い合わせできる
3. **冪等性を保つ**
...
- Stripe 失敗時にユーザーーが状態を認識できる（500 応答）
- チャージバックリスクの低減
