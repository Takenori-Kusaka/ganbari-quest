...
4. **ADR-0036 の閲覧パブリック設計と矛盾**: 未ログインユーザーーの第一印象が「販売プラットフォーム」になることは、ADR-0036 で意図した「テンプレート閲覧でサービス価値を伝える」導線と齟齬がある。
...
- 将来 UGC 化した際に「みんなの」が自然に意味拡張可能 (現状はキュレーション、将来はユーザーー投稿も含む)

### Negative

- ラベル変更による既存ユーザーーの一時的混乱 (ただし URL 不変のためブックマーク・URL 共有は影響なし)
