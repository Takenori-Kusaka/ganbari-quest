# 0030. Cognito E2E テストユーザーーのライフサイクル基盤
...
| 要素 | 実装 | 場所 |
|------|------|------|
| 固定ユーザーー E2E | あり | `tests/e2e/cognito-auth.spec.ts` |
...
`AdminSetUserPassword(Permanent: true)` を直接実行し、CONFIRMED 状態のユーザーーを即作る。
...
**Admin API で作ったユーザーーは Cognito 本物プールに対する sign-in / tenant 作成 /
...
- sha 入りで「どのコミットが作ったユーザーーか」が DB 残骸からも追跡可能
...
- `@ganbari-quest.com` (プロダクションドメイン) の email を E2E テストユーザーーに使ってはならない
