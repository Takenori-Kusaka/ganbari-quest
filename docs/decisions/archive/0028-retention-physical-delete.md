...
今のうちに削除パイプラインを通しておかないと、ユーザーーが増えてから実装する方が技術的負債になる。
...
ユーザーー体験上最も繊細な点: point_ledger を削除しても **現在残高は変わらない**。
...
- 再 upgrade しても消えたデータは**戻ってこない**（ADR-0027 からの後退点、ユーザーーへの明示が必要）
...
- [ ] ユーザーーへの仕様変更通知（再 upgrade で過去データが戻らない点）
