...
ADR-0019 (CDK Replacement Detection Gate) と同パターン。`scripts/check-lp-innerhtml-tags.mjs` (#1747) で
`site/shared-labels.js` の `SANITIZE_CONFIG.ALLOWED_TAGS` が必須タグ
(`h1` / `h2` / `h3` / `h4` / `p` / `ul` / `ol` / `li` / `div` / `table` / `tr` / `th` / `td` / `header` / `section` / `dl` / `dt` / `dd` 等) を**全て含むこと**を静的検査する。
