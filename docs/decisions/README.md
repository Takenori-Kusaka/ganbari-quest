...
|------|---------|-------|------------|------------|-----------|------|
| コードベース探索性 (knowledge graph 化) | [Graphify](https://github.com/Graphify-Labs/graphify) (Apache-2.0) | 2026-09-16 | 一度採用 (#4343) したが撤去 (#4975)。30MB の生成物を git で追跡し、再生成のための bot PR・依存 pin・hook が道具本体より大きくなった。query はハブノイズが多く grep より当たらなかった | grep 探索のトークン消費が再び作業を妨げたとき（まず軽量なコード索引と docs/codebase-map.md の拡充を比べる） | `.github/workflows/graphify-refresh.yml` | [rationale/16](../rationale/16-graphify-evaluation-rationale.md) |
