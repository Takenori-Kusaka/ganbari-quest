...
> #1262 sub-A（#1265）完了時点で新体系に更新。旧採番ファイルは sub-B（#1262）で archive 行きまたは削除予定（過渡期として `docs/decisions/` 直下に残存）。
...
## 旧採番ファイル（sub-B 待ち、過渡期）

> 以下の旧採番ファイルは現時点で `docs/decisions/` 直下に残存するが、#1262 sub-B で archive 行き（25 件）または削除（5 件: 0002, 0008, 0009, 0016, 0027）される。新採番との file prefix 衝突は sub-B 完了まで解消されない。sub-B 完了後に本セクションは削除される。

| 旧 # | 処理 | 新採番への対応 |
|------|-----|-------------|
| 旧 0001 rename-backward-compat | archive | — |
| 旧 0002 dialog-queue-required | 削除（supersede チェーン終結） | — |
| 旧 0004 stamp-card-spec | archive | — |
| 旧 0007 image-asset-protection | archive | — |
| 旧 0008 age-mode-duplication-risk | 削除（supersede チェーン終結） | — |
| 旧 0009 server-client-type-contract | 削除（supersede チェーン終結） | — |
| 旧 0011-0015 (技術選定) | archive | — |
| 旧 0016 dialog-overlay-management | 削除（supersede チェーン終結） | — |
| 旧 0019 dialog-fsm | archive | — |
| 旧 0021 deploy-verification | archive | — |
| 旧 0022 billing-data-lifecycle | archive | — |
| 旧 0024-0026 (license 関連) | archive | — |
| 旧 0027 retention-policy | 削除（supersede チェーン終結） | — |
| 旧 0028 retention-physical-delete | archive | — |
| 旧 0030 cognito-e2e-user-lifecycle | archive | — |
| 旧 0031 schema-change-compat-testing | archive | — |
| 旧 0033 ops-dashboard-cognito-authz | archive | — |
| 旧 0036 marketplace-public-access | archive | — |
| 旧 0039 demo-mode-app-execution-mode | archive | — |
| 旧 0040 runtime-mode-license-unified | archive | — |
| 旧 0041 marketplace-naming-template | archive | — |
| 旧 0042 marketplace-gender-variant-policy | archive | — |
| 旧 0043 native-select-primitive | archive | — |
| 旧 0044 admin-bypass-evidence | archive | — |

## archive 一覧

> `docs/decisions/archive/` 配下。#1262 の sub-B 完了時点で 25 件が移動される予定。現時点では空。
