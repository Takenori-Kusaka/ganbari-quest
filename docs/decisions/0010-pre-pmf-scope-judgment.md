...
### 0. 適用範囲と誤用禁止（2026-07-17 PO 指示で明文化）

本 ADR の Pre-PMF スコープ判断は **「新規の過剰な機能追加・過剰防衛設計を退ける」ためだけに適用する**。「まだ作らない / まだ導入しない」の判断であって、**既存実装の品質を下げる根拠ではない**。以下の誤用を明確に禁止する:

**禁止する誤用（Pre-PMF と呼んではならない）**:

- **はりぼて実装 / 一時的な対処 (band-aid) を「Pre-PMF だから」で正当化する** — これは Pre-PMF ではなく単なる**未完成・技術的負債**であり、ADR-0061（band-aid サイクル打破）違反。「最小実装」を選ぶ場合も **"正しく完成した最小"** が条件であり、はりぼて / 未完成 / 動くだけを許容する意味ではない。
- **既存実装の refactoring / 完成度向上 / 品質向上 / 移管の完遂 を「Pre-PMF だから」で妨げる・後回しにする** — これらは Pre-PMF の対象外であり、**常に正当な作業**。
- **「完遂していないのに Pre-PMF を理由に中断して次に進まない / 着地する」** — 完遂を妨げる口実に使わない。

**Pre-PMF が適用されるもの（のみ）**: 新規機能の追加是非（§1 3 バケット）/ 過剰防衛設計の採否（§4）/ 新規 OSS・基盤の導入コスト（§7.1）。

**Pre-PMF が適用されないもの**: 既存機能の完成度・品質・refactoring・移管の finalize（例: DSQL 移管の完遂）・bug fix・データ整合・security 是正。これらは ADR-0006（既存 safety 維持）/ ADR-0061（shift-left 根本対処）に従い、Pre-PMF を理由に劣化・deferral させない。

> **背景**: AI エージェントが「Pre-PMF 最小」を、はりぼて実装・一時対処・品質作業の deferral の言い訳に繰り返し誤用した（2026-07-17 PO 指摘）。本 §0 はその誤用を構造的に止めるための scope guard。
...
ナビ / 情報アーキテクチャ系 (NAV_CATEGORIES / admin-ia.md / 親管理画面ナビ / 子供画面ナビ / 設定サブナビ / 運営者ナビ 等の階層変更) を扱う Issue は以下 **2 必須項目** を AC に複製必須。Phase Admin-Nav-Restructure (#2176 EPIC) で発覚した「過去設計 SSOT 確認漏れ / ナビゲーション面の同期変更漏れ」の構造的再発防止:
...
| **ナビゲーション面の SSOT 確認** | ナビを描画する面を grep で全件洗い出し、一部だけ変更する漏れがないか確認する。面は固定の集合として数えない（数え上げると必ず陳腐化する。**存在しない `AdminMobileNav.svelte` を「3 種」の 1 つとして数え続けた**のが本項の元の壊れ方）。主な面は `AdminLayout.svelte`（管理画面。Desktop ドロップダウンと Mobile ボトムナビが同一ファイルに同居）/ `BottomNav.svelte`（子供）/ `admin/settings/+layout.svelte`（設定サブナビ）/ `ops/+layout.svelte`（運営者）で、ページ内タブ・パンくずも `<nav>` を持つため grep 結果で判断する。labels.ts SSOT 経由で全面に同期反映されることを担保 | PR 本文に grep 結果 (下記コマンド) + 変更が及ぶ全面への反映確認 |


ナビゲーション面の洗い出しコマンド (表セル内に置くと GFM が code span の `\|` を `|` に落として動かない形になるため、ここに置く):

```bash
grep -rnE '<nav\b|role="navigation"' src/
```

`<nav ` や `<nav>` で区切ると複数行にまたがる宣言を落とす (実測: 4 面が漏れた)。単語境界 `\b` で取ること。
