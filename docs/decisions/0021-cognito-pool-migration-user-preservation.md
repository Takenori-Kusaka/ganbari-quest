# ADR-0021: Cognito Pool 移行におけるユーザーー保全戦略
...
2026-04-21 の ADR-0018（Cognito User Pool 論理 ID 変更による明示的 Replacement）で Pool を再作成した際、**既存 federated ユーザーーが全員消失した**。Pre-PMF 段階で PO テストアカウントのみだったため許容できたが、Post-PMF では再発不可。
...
| ユーザーー種別 | 特徴 | インポート方法 |
|------------|------|-------------|
| email/password | Cognito native ユーザーー | `AdminCreateUser` (SUPPRESS) + パスワードリセット案内 |
| Google OAuth | federated identity | `AdminCreateUser` + `AdminLinkProviderForUser` |

パスワードハッシュは AWS の仕様上エクスポート不可。email/password ユーザーーには移行後にパスワードリセット案内が必要（MFA の TOTP シークレットは再スキャン不要）。

### 3. スクリプト整備

- `scripts/cognito/export-users.mjs` — Pool 全ユーザーーを JSON エクスポート
...
**却下理由**: Pre-PMF ユーザーー数では実装コストが見合わない。Admin API で一括インポートで十分。
