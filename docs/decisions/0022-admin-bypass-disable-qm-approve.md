...
PR #1875 (1 回目) / PR #1982 (2 回目) で実際に違反が再発したため、Issue #1994 で **server side gate (`.github/workflows/pr-author-guard.yml`)** を追加。`pull_request: opened/reopened/ready_for_review` イベントで発火し、author が許可リスト (`Takenori-Kusaka`) 外であれば PR を自動 close + 違反コメント投稿 + workflow fail。client / 経路を問わず必ず発火するため、L1/L2 で抜けた全経路を server side で捕捉する 3 層防御に拡張された。

---

## Amendment 4 (#2863, 2026-06-04): lab アカウント merge の 2 role 区別 + 統合 PR 作成者ルール

### 背景

EPIC #2861 で branch モデルを develop 二層 (`feature/* → develop → 統合 PR → main`、`docs/sessions/branch-strategy.md` §6) に拡張すると、`ganbariquestsupport-lab` の merge 行為が **2 つの異なる role** を持つ。区別を ADR で明文化しないと merge 権限の audit trail が曖昧になり、#2768 (auto-merge bypass + audit trail) の懸念が再燃する。

### 決定（追加）

1. **lab アカウント merge の 2 role 区別**: 同一 gh アカウント `ganbariquestsupport-lab` を担当 PR の base branch で role 区別する。
   - **QM role**: `feature/* → develop` の per-PR approve + merge (毎時、機能正しさ判定、軽量レーン)。本 ADR 本文「決定 2」の QM Approve 体制をそのまま継承する。
   - **外部品質監査チーム role**: `develop → main` 統合 PR の approve + merge (1 日 1 回、CUJ 横断・客観的第三者判定、最重厚レーン)。
2. **統合 PR 作成者ルール**: `develop → main` 統合 PR は `Takenori-Kusaka` 名義 (監査マネージャ session) が作成 → `ganbariquestsupport-lab` (外部品質監査チーム role) が approve + merge。本 ADR 本文「決定 2」の **作成者 ≠ 承認者** 分離を統合 PR でも維持する。
3. **pr-author-guard の変更は不要**: `.github/workflows/pr-author-guard.yml` は author が `Takenori-Kusaka` 以外を fail させる。統合 PR も `Takenori-Kusaka` 作成のため許可リストに合致し、現行 guard のまま整合する (許可リスト拡張・base 別分岐は不要)。
4. **各 role の audit trail 要件** (#2768 懸念解消):
   - **QM role**: approve body に `qa-session.md §「QM approve 前の必須実行手順」` (SS 実視認所見 / AC 照合 / 禁忌確認) を記述 (本 ADR 本文「決定 3」)。
   - **外部品質監査チーム role**: 統合 PR の approve は **ADR-0056 structured evidence** に基づく。adversarial reviewer の反対理由 3 件を `tmp/adversarial-evidence/<pr>.json` (main repo 直下、`must_object_count: 3` schema 必須、TTL 30 分) に保存し、PreToolUse hook `.claude/hooks/gate-approve.mjs` が approve 系コマンド実行前に存在 + schema を物理検証する (schema 検証ロジックは `scripts/verify-adversarial-output.mjs`)。誰が・どの gate で・どのエビデンスに基づき merge したかが evidence file として残る。

### 二役の限界の明示 (AC5)

本 amendment の役割分離は **論理上の 2 role** であり、lab approve の実運用は同一オペレータ (`Takenori`) 配下の AI session による二役で、物理的に別人格ではない。この限界を本 amendment で明示的に認める。「別人格でない」リスクは、self-report 単独信頼を禁止し (EPIC #2861 PO 判断 5 / ADR-0056)、approve 判定根拠を以下の **構造的エビデンス (audit trail)** に固定することで補償する:

- **保存先**: `tmp/adversarial-evidence/<pr>.json` (main repo 直下必須、ADR-0056 §D / §E)。
- **検証手順**: `scripts/verify-adversarial-output.mjs` が `--pr <number>` で evidence file を読込み、存在 + `must_object_count: 3` field + 30 分 TTL を検証。PreToolUse hook `.claude/hooks/gate-approve.mjs` が不通過時に approve 系 Bash command を `permissionDecision: "deny"` で物理 block する。
- これにより「同一オペレータの二役」であっても、approve は AI session の自覚に依存せず evidence の物理存在で gate される。

### 効力発生条件

本 amendment の 2 role 区別は **develop cutover (#2870、branch-strategy.md §8) 完了後** に発効する。cutover 前は現行運用 (全 PR が main 向け、QM role が毎時 approve + merge) が継続する。

### 受入基準（#2863 AC）

- [x] AC1: lab merge の 2 role 区別 (QM = feature→develop / 監査 = develop→main) を明記 (§決定 1)
- [x] AC2: 統合 PR 作成者ルール (Takenori-Kusaka 作成 → lab approve+merge、作成者≠承認者維持) を明記 (§決定 2)
- [x] AC3: pr-author-guard 変更不要を根拠付きで明記 (§決定 3)
- [x] AC4: 各 role の audit trail 要件を #2768 懸念解消の形で規定 (§決定 4)
- [x] AC5: 同一オペレータ二役の限界 + ADR-0056 structured evidence による補償を明記 (§二役の限界の明示)
- [x] AC6: ボリューム上限超過への対応 — 詳細は `docs/sessions/branch-strategy.md` §6 / `docs/sessions/audit-team.md` (A1 #2862) に分離、本 amendment は規約核のみ記述

### 関連

- EPIC #2861 (外部品質監査チーム体制) — PO 判断 1 / 3 / 5 / 6
- #2768 (auto-merge bypass + audit trail) / ADR-0056 (structured evidence) / ADR-0010 (Pre-PMF Bucket A)
- `docs/sessions/branch-strategy.md` §6 (役割分担 2 層、本 amendment の上流 SSOT)
