...
- ~~**法的文書**（`site/tokushoho.html` の条項番号・社名等） — 法務要件で固定~~ — **#1703 で supersede（ADR-0025 LP SSOT innerHTML 機構の対象拡大）**: 法的文書も SSOT 化対象。 `LP_LEGAL_PRIVACY_LABELS` / `LP_LEGAL_TERMS_LABELS` / `LP_LEGAL_SLA_LABELS` / `LP_LEGAL_TOKUSHOHO_LABELS` を `src/lib/domain/labels.ts` に追加し、`data-lp-key="legalPrivacy.section8" 等` の属性経由で innerHTML + DOMPurify sanitize で注入する。法務要件の固定文言は `labels.ts` を SSOT として人間が管理し、用語ドリフトを防ぐ
