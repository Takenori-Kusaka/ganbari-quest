...
**注**: テーマ型 (勉強特化・運動特化等) はバンドル同梱せず、将来の「追加インポート機能」(既存プリセットを置換せず追加) で扱う。初期パッケージの十全感は **15 パック × 中身の質** で達成する。
...
ADR-0010 (Pre-PMF Issue 優先度判断基準) では、Pre-PMF 期の最優先課題は **「最初の 100 ユーザーの獲得と継続」** と定義されている。本アプリの Free → 有料転換ファネルは以下:
