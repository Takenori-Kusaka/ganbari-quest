...
| child-facing POST action の Cookie 入口（`(child)/checklist`・`shop`・`home/initial-points`・`(character)/battle` の 4 action、#3581 ②） | `requireValidChildCookieFormat`（`src/lib/server/auth/child-cookie-guard.ts`、DSQL backend 時のみ） | Cookie delete + `/switch` redirect（layout GET gate と同契約に正規化）。空 Cookie も uuid 列で 22P02 を誘発するため redirect 対象 |
| mutation / lookup repo 入口（`child-repo.updateChild` / `deleteChild`、`checklist-repo.findAssignmentsByChild`、#3581 ②） | `isUuidFormat`（DSQL backend 時のみ） | 各メソッド既存の not-found shape（`undefined` / no-op / `[]`）を維持。signature 不変 |
...
- **fitness function**: `tests/unit/routes/child-cookie-guard.test.ts` が DSQL で非 uuid → Cookie delete + `/switch` redirect / 有効 uuid → service 到達 / 非 DSQL で数値 id・空 Cookie を素通しを機械検証する。`tests/unit/db/dsql-child-repo.test.ts` / `dsql-checklist-repo.test.ts` が repo 入口の not-found 正規化を、`dsql-pg-uuid-guard.test.ts` が warn の rate-limit を固定する。
