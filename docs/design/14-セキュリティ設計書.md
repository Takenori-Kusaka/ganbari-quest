...
| **Cookie**（`selectedChildId`） | child-facing POST action の Cookie 入口（`selectedChildId` を直読する全 child POST action。`(child)/checklist`・`shop`・`home/initial-points`・`(character)/battle` に加え、`home` の record / cancelRecord / claimBonus / loginStamp / togglePin / stampCard / redeemStampCard / claimBirthday / claimChallengeReward / sendCheer の 10 action） | あり | `requireValidChildCookieFormat`（`src/lib/server/auth/child-cookie-guard.ts`） | Cookie delete + `/switch` redirect（layout GET gate と同契約）。空 Cookie も 22P02 を誘発するため redirect 対象。非 DSQL では空文字を返し各 action 既存の空 Cookie 処理（`loginStamp` の retry-storm 回避 no-op 等）を保持 |
...
### 11.5 本番 / staging の防御層マトリクス (#4266)

staging (AWS staging 4 stack、`deploy-aws-staging.yml`) は本番と**同一アカウント・同一リージョン**に物理名 prefix `ganbari-quest-staging` で並置する。防御層のうち **意図的に外しているものと、外していないもの**を 1 箇所で読めるようにする。staging の構成を触るときは本表を先に読むこと。

| 層 | 本番 | staging | staging がそうである理由 |
|---|---|---|---|
| CloudFront geo 制限 | JP allowlist | **無し (全世界公開)** | post-deploy smoke を回す GitHub Actions runner が日本国外にあり、JP allowlist だと 403 で smoke が回らない (#4204)。**staging に本番データを入れる運用が生まれたら JP allowlist を戻す** |
| `/admin` `/api/v1/admin` `/ops` の IP allowlist | CloudFront Function (`secrets.ADMIN_ALLOWED_IPS`) | **本番と同じ** (同一 secret) | context 未指定なら `network-stack.ts` が synth を throw する (#4266)。**適用範囲は CloudFront 経由のリクエストのみ**（下記の但し書き） |
| Cognito 認証 | 本番 User Pool | staging 専用 User Pool (`AuthStaging`) | 本番ユーザーは staging に存在しない。登録メールドメインは `STAGING_ALLOWED_EMAIL_DOMAINS` (workflow env が SSOT) で allowlist し、外れたら Discord incident に通知する (deploy 自体は止めない) |
| 親 PIN gate | 有効 | 有効 (同一実装、staging 専用 `PARENT_GATE_COOKIE_SECRET`) | — |
| DB | 本番 DSQL cluster | staging DSQL cluster (`GanbariQuestDsqlStaging`、DP 無し) | Lambda 実行 role の `dsql:DbConnect` は staging cluster ARN に resource 限定。**staging から本番 cluster へは接続できない** |
| 投入データ | 本番顧客データ | 合成 seed / 手動検証データのみ | 本番 → staging のデータ複製経路は `scripts/snapshot-prod-db.cjs` のみで、これは NUC staging (`deploy-nuc-staging.yml`) 専用。`deploy-aws-staging.yml` は snapshot / restore step を持たない。合成 seed の仕様は [staging-synthetic-seed.md](staging-synthetic-seed.md) |
| Stripe | live key | **test mode key のみ** | live prefix は workflow の secret 検査 + ComputeStack の allowlist で 2 段停止する (#4104) |
| Discord / Gemini / SES | 注入 | **非注入** | staging アプリが動いた副作用で実チャネルを汚さない (CI が人間に出す運用アラートは別) |
| Ops stack (アラーム / SNS) | 有り | 無し | staging は 4 stack (Storage / Auth / Compute / Network) のみ |

**admin IP allowlist の適用範囲（本番 / staging 共通）**: CloudFront Function は viewer request にしか介在しないため、**Lambda Function URL を直接叩く経路には掛からない**。Function URL は `authType: FunctionUrlAuthType.NONE`（`infra/lib/compute-stack.ts`）で公開されており、CloudFront → origin 間の shared secret header 検証も行っていない。したがって Function URL のホスト名を知る者は IP allowlist を迂回して `/admin` `/ops` に到達できる（アプリ側の Cognito 認証と親 PIN gate は依然として効く）。これは本 allowlist 固有の話ではなく Function URL origin 構成に由来する既存の性質で、埋めるには origin custom header 検証か OAC 相当の origin 制限が要る。**「ネットワーク層で守られている」を前提にした判断をしないこと。**
