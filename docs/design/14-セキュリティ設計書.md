...
| `/admin` `/api/v1/admin` `/ops` の IP allowlist | CloudFront Function (`secrets.ADMIN_ALLOWED_IPS`) | **本番と同じ** (同一 secret) | geo 制限を外した分、この 3 path のネットワーク層防御はこれが唯一。context 未指定なら `network-stack.ts` が synth を throw する (#4266) |
