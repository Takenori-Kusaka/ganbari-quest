...
#### 5.2.7 selectedChildId Cookie の形式検証（DSQL uuid 列への stale id 直達防止、CWE-20、#3709 / #3581）

`selectedChildId` Cookie は client に保持される外部供給値であり、uuid 列を持つ DSQL / PGlite backend にとって trust 境界の入口となる。旧 SQLite の数値 id（`'903'` 等）が残存した Cookie を持つ既存ユーザーが DSQL backend に到達すると、非 uuid 文字列が uuid 列の WHERE に渡り `22P02 (invalid input syntax for type uuid)` を throw して **500** になる（CWE-20 入力検証欠如）。この入口を DSQL backend 限定で形式検証し、fail-closed に正規化する。

| trust 境界の入口 | 検証 | 非 uuid（stale）id への正規化 |
|---|---|---|
| child 配下 GET load の gatekeeper read（`findChildById` 4 経路: child / point / status / login-bonus repo、#3709） | `isUuidFormat`（DSQL backend 時のみ） | not-found 相当（`undefined` → layout gate が Cookie delete + `/switch` redirect） |
| child-facing POST action の Cookie 入口（`selectedChildId` を直読する全 child POST action。`(child)/checklist`・`shop`・`home/initial-points`・`(character)/battle` に加え、`home` の record / cancelRecord / claimBonus / loginStamp / togglePin / stampCard / redeemStampCard / claimBirthday / claimChallengeReward / sendCheer の 10 action、#3581 ②） | `requireValidChildCookieFormat`（`src/lib/server/auth/child-cookie-guard.ts`、DSQL backend 時のみ） | Cookie delete + `/switch` redirect（layout GET gate と同契約に正規化）。空 Cookie も uuid 列で 22P02 を誘発するため redirect 対象。非 DSQL では空文字を返し各 action 既存の空 Cookie 処理（`loginStamp` の retry-storm 回避 no-op 等）を保持 |
| mutation / lookup repo 入口（`child-repo.updateChild` / `deleteChild`、`checklist-repo.findAssignmentsByChild`、`stamp-card-repo.findCardByChildAndWeek` / `findCardsByChild`、#3581 ②） | `isUuidFormat`（DSQL backend 時のみ） | 各メソッド既存の not-found shape（`undefined` / no-op / `[]`）を維持。signature 不変。`stampToday`（`stampCard` / `loginStamp` action → `getOrCreateCurrentCard`）が find 系へ raw id を直達させる repo 層 fail-safe（route guard と二重防御、push-down-pyramid） |

- **backend 対称性**: 形式検証は `isDsqlBackend()` 真のときのみ実行する。sqlite / demo / dynamodb backend の数値 id（`'903'` 等）は正当なため、それらの経路では検証を無効化し、既存の空 Cookie 処理（各 action の `fail` / `redirect`）に委ねる（有効な数値 id を誤って弾かない）。
- **observability**: 非空だが非 uuid の stale id を検知したとき、`warnInvalidUuidId`（`src/lib/server/db/dsql/pg-uuid.ts`、既存 `checkRateLimit` で source 単位 1 分 1 回に抑制）を全 guard 点に配線し、cutover 後の残 stale Cookie を breadcrumb で追跡する。空 Cookie（単なる未選択）は warn 対象外。
- **fitness function（挙動）**: `tests/unit/routes/child-cookie-guard.test.ts`（checklist toggle）+ `tests/unit/routes/home-cookie-guard.test.ts`（home stampCard / loginStamp）が DSQL で非 uuid → Cookie delete + `/switch` redirect / 有効 uuid → service 到達 / 非 DSQL で loginStamp の no-op 契約保持を機械検証する。`tests/unit/db/dsql-child-repo.test.ts` / `dsql-checklist-repo.test.ts`（[A2b]）/ `dsql-stamp-mission-repos.test.ts`（[SC2b]）が repo 入口の not-found 正規化を、`dsql-pg-uuid-guard.test.ts` が warn の rate-limit を固定する。
- **fitness function（構造、ADR-0061 same-class N→guard）**: `tests/unit/architecture/dsql-uuid-guard-ssot-fitness.test.ts` が本 trust 境界の 2 不変条件を CI hard-fail 化し、22P02 → 500 の同一クラス（#3709 → #3714 → #3792 と 3 ラウンド再発）を **人の注意ではなく機械**で再発防止する。[SSOT-1] uuid 形式判定の hand-rolled regex は `pg-uuid.ts` の 1 箇所のみ（別実装で `warnInvalidUuidId` observability と not-found 正規化契約を迂回させない、ADR-0063 単一強制点）。[OBS-2] guard module（dsql repos + child-cookie-guard）で `isUuidFormat` guard を使う各 module は `warnInvalidUuidId` を同数以上呼ぶ（silent guard = 観測点ゼロの guard 追加を禁止）。
