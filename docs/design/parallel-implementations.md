...
- **#2146 (2026-05-17 完了)**: 上記 Tier 2.5 で統合した `MustProgressBar` 専用セクションを撤廃し、`ActivityCard.isMust` (riboon badge + gold border) に統合。`ProdDashboardSections.svelte` / `DashboardView.svelte` の両方で `priority='must'` の活動カードに直接 badge を付与する設計に変更（本番 + demo 同期 SSOT 1 箇所修正で両方反映）
