...
| `archived_reason` enum 3 値 (#2642 / Phase 5 子 4 / Phase 7 PR-1) | `src/lib/server/db/schema.ts` 4 location (`children:40` / `activities:70` / `child_activities:109` / `checklist_templates:430`、本 PR は `text('archived_reason')` 維持、drizzle 層 enum 制約は Phase 7 PR-2a で適用) + `src/lib/domain/archive-types.ts` (新規 SSOT、`as const` array + `ArchivedReason` 型 + `getRetentionDays`) + `src/lib/server/db/migration/lazy-startup-migrations.ts` (`migrateBillingPhase6` で既存 NULL row を `'downgrade_user_selected'` で補充、4 location × idempotent + 列存在 guard) | `tests/e2e/global-setup.ts` (NULL 補充 UPDATE 追加、4 location) | `tests/unit/helpers/test-db.ts` (CREATE TABLE 4 location は既存、本 PR では schema レベル変更なし) + `src/lib/server/db/create-tables.ts` (同上、列定義は既存 `archived_reason TEXT`、enum 制約は domain SSOT で表現) | `src/lib/server/db/demo/{child,activity,child-activity,checklist}-repo.ts` 4 file は Phase 7 PR-2a (atom 統合 + drizzle enum 制約適用) で `archivedReason: ArchivedReason \| undefined` に型強制 (本 PR scope 外) | #2642 (Phase 5 子 4 archive 統合) + #2675 (Phase 6 子 3 DB migration plan) + #2685 (Phase 7 PR-1) |
