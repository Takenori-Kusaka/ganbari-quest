...
- **⚠️ セキュリティ不変条件（必須）**: `owner_guard` UNIQUE は「owner ≤1」を DB で守るが「**誰が role を書けるか**」は守らない。**membership role 変更・owner 移譲・member 削除は `requireRole(['owner'])` 必須**（owner 専用 route guard。現状 owner 機能が `/admin`=owner+parent 配下に同居し parent が到達しうるため、移管で水平/垂直権限昇格を作り込まない）。invite 受諾時は `accepting_user.email == invite.email` 束縛も検証（判定 SSOT は `src/lib/server/auth/invite-email-binding.ts` #3742: `email_verified=false` fail-closed + `trim().toLowerCase()` 正規化。service 層 `invite-service.ts` と DSQL txn `db/dsql/invite-accept.ts` が同一関数を共有）。
...
6. **非 repo DynamoDB アクセス（N3、scope integrity）**: DynamoDB SDK の **app 層（`src/`）残存は 0 file**（#3438 Phase 3 で完了）。#3805 で analytics 系 2 file（`analytics/providers/dynamo.ts` + `analytics-aggregate-service.ts`）を撤去し on-demand 集計へ載せ替え、#3438 Phase 3 で probe（DynamoDB 接続 probe）+ migration hydrate writeback を撤去した。regression guard は `tests/unit/architecture/db-access-boundary.test.ts`（`src/` からの `@aws-sdk/*-dynamodb` import 0 を機械強制）が担う。**`@aws-sdk/client-dynamodb` / `@aws-sdk/lib-dynamodb` deps は保持**（app 層外の live tooling = `tests/e2e/global-teardown-aws.ts`（`playwright.aws.config.ts` globalTeardown）+ `scripts/seed-cognito-test-data.ts`（`seed:cognito-test-data`）が orphan 化する prod DynamoDB table を操作するため）。deps + tooling の撤去は infra DynamoDB table 本体撤去 PR（prod removalPolicy=RETAIN の orphan）と同期する。
...
| 6 | dialect-parity: 生成列(owner_guard/active_key/email_lower)+UNIQUE / CHECK 強制 / temporal `{mode:'string'}` を**両 backend で assert**（揮発した spike script を dialect-parity テストとして恒久化、SQLite parity Finding 4。実装フェーズ (#3424) で新設、現時点では未作成） | 新規 test |
...
| 16 | **DB アクセス経路の一本化（散在アクセス再発防止、ADR-0061 same-class→guard）**: **DB SDK/ドライバの直接 import（`@aws-sdk/*-dynamodb` / 生 pg `Client` / drizzle driver）は `src/lib/server/db/<backend>/`（repo 実装層）内でのみ許可**。services/routes/analytics 等それ以外は `getRepos()` インターフェース経由必須。既存 `tests/unit/architecture/route-db-boundary.test.ts`（#3152、routes 限定）を **全レイヤ**に拡張。移管中/移管後とも backend 非依存で「直接アクセスの silent 混入」を CI hard-fail（過去の散在アクセス refactor の再発を機械封鎖）。**例外は allowlist で明示**（probe/migration ツーリング等、理由付きで登録） | architecture test（import AST、allowlist 駆動） |
