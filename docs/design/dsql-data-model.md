...
- **spike#7 スケール確証（12,480 行・24 child・520 行/child）**: `point_ledger (family_id,child_id,type,recorded_date)` secondary は must-bonus 冪等 hot path で**採用・2x 高速**（最初から張る）。`activity_logs` の date secondary は getTodayActivityCounts で**不採用**（PK-prefix 520 行 scan+filter で 1.5ms、家庭スケールで不要）→ 初期 PK のみ。
...
| `families`(=tenant+subscription) | `(family_id)` | UNIQUE `(stripe_customer_id)`、status/plan CHECK | stripe 2hop→1hop。owner_user_id は cache（SSOT=memberships）。licenseStatus は算出（列なし） |
...
- **方言差**: pg `uuid/timestamptz/gen_random_uuid()/jsonb` ↔ sqlite `text/$defaultFn(randomUUID)/strftime/text`。生成列・CHECK・UNIQUE は両対応。drizzle `text(enum)` は CHECK 非生成（既知落とし穴）→ `check()` 明示必須。CHECK 値は subscription-status.ts/plan.ts から生成（手書き二重化禁止）。
...
- optional を core に含めない理由 = SAVEPOINT 不可（spike#4）で「optional 失敗が core を 25P02 で巻き込む」ため。optional は additive（point 追加 / challenge 進捗 / 証書発行）で core の整合に不要、独立失敗を許容できる。**⚠️ optional 欠落許容は product 判断**（combo/mission/challenge を core commit 後 best-effort・retry なし＝現状と同等の regression なし、だが §10 で PO 明示確認）。
...
9. **stripe_webhook_events の実装要否**: schema 配備済だが repo 実装・結線が未稼働（§3.5.3）。billing 冪等性に関わるため移管前に実装要否を判断。
...
children (
  ...
  -- → birth_date から算出。これにより日次 age-recalc cron を撤去（N4 / §8.1）。birth_date NULL の旧データは移行時に補完 or age 表示を birth_date 有無で出し分け。
...
| certificates | `(family_id, child_id, certificate_type)` | — | 自然複合（昇格） |
...
5. 接続は IAM トークン（~1.45s cold）、ASYNC index build 完了待ちを CDK/migration に組込。
...
| # | 不変条件 | 検証 |
|---|---|---|
| 1 | 全テナント表に `family_id` NOT NULL + 全 repo メソッドに tenant 述語（§P9、security Finding 1） | architecture test（schema walk + repo AST） |
| 2 | `consents` への UPDATE/DELETE 禁止（append-only、erasure 専用パス除く） | repo に update/delete 非定義 + GRANT 除外 + lint |
| 3 | owner_guard UNIQUE + membership role 変更は `requireRole(['owner'])` | route guard test + DDL 検証 |
| 4 | backup-entity-registry: schema ⊆ registry（no silent gap）+ auth/consent/subscription は excluded | 既存 registry test 拡張 |
| 5 | backup round-trip completeness（**backend パラメタ化して pg でも 1 回**）+ chunk 部分失敗→旧データ保全 | 既存 test の backend 化 + 新規 1 本 |
...
| 7 | core txn work に `await fetch`/`await import`/通知 が出たら fail（§8、yield 汚染防止） | lint/AST |
| 8 | cross-tenant E2E（他テナント childId で 404 + storage 非到達、#3139 踏襲） | 既存 IDOR test 移植 |
| 9 | DDL 二重ソース drift 防止: pg DDL と sqlite DDL の生成列式・CHECK 値が文字列一致（create-tables.ts 手書き path 縮退、SQLite parity Finding 4） | 新規 parity test |
| 10 | **派生列 drift 突合**: `children.total_point` == `SUM(point_ledger.amount)` / `statuses.total_xp` == 集計 / `activity_logs.streak_days` 整合をバッチで突合（§5/§P7 不変条件、F2）。正本は派生列、突合で乖離検出 | バッチ drift 検出 test |
| 11 | **cron バッチの per-tenant chunk 強制**: 全テナント横断 cron（age 系 / retention / grace-period / trial-notif / analytics）が単一 txn で全行を書かず per-tenant + chunk ≤3,000 行 + OCC retry で回す（§8.1、N2） | cron 実装の AST/lint + 統合 test |
