...
> **リージョン SSOT（決裁 2026-07-01、法務/DB/UX パネル + PO 承認）**: 移管先 = **`ap-northeast-1`（東京・国内保存）**。DSQL 東京は 2026-05 提供開始済。国内保存で APPI「外的環境の把握」開示義務・米国制度把握義務が自体的に不要（法務）、日本ユーザーへのレイテンシが §4.1/§3.5 の性能前提（intra-region ~1.5ms）を成立させる（DB/UX）、children biometric の data residency 上も優位。旧「us-east-1」は DSQL 東京提供前(GA 当時 米国のみ)の名残で撤回。spike#1-7 の us-east-1 実測値は intra-Tokyo でも同等（同一 DSQL エンジン）。
...
### §3.5.5 子供ホーム read 予算 + 1 描画 1 パス（UX パネル B1、PK 凍結前の設計要件）

§3.5.1 の 21+ クエリは DSQL の 1 クエリ=ネットワーク往復。**region=ap-northeast-1（国内）で intra-region ~1.5ms/クエリ**が成立するが、それでも段内を逐次 await にすると累積する。UX 許容から逆算した契約を課す（schema 変更なし、repository read-plan のみ、PK/txn 不変）:
- **描画予算**: 子供ホーム p75 first-interaction ≤ 600ms（warm）。段内 read は **parallel-batch（Promise.all）を規定**（逐次 await 禁止）。
...
- **first-paint**: 2 段 `await parent()` 連鎖 + cold start（IAM ~1.45s）に対し skeleton/optimistic first-paint + 接続 warm 再利用を規定（preschool の空白待ち回避、UX R1）。
...
- **data residency（法務パネル 2026-07-01）**: 保存先 = 国内 `ap-northeast-1`（冒頭 SSOT）。国内保存で APPI 外的環境把握・開示義務は自体的に不要。クラウド例外（AWS が保存データを取扱わない DPA + アクセス制御、PPC FAQ 1-7-53）成立の証跡（DPA 依拠）は運用で記録。市場は **日本限定**を前提（COPPA は対象=米国 13 歳未満・サーバ所在地で非発火＝適用外だが、将来 US 在住家族提供時は再付着するため scoping を明記）。consent の ip/UA 保持はデータ最小化観点で必要性を再評価（証跡は timestamp+version+user_id で足りる）。
...
- **単一リージョン SPOF を明示受容 + RPO/RTO を Pre-PMF 水準で記載**（ap-northeast-1 内 3AZ 同期でリージョン内は堅牢）。**multi-region active-active は ADR-0010 bucket C として明示 scope 外**。
- ap-northeast-1/2/3 内 linked cluster は将来オプション（今は不要）。
