...
- **取込 / 復元 gate (`activity-quota.ts`) の 2 適用方式**: 取込 (`dispatchImport` 経由) は超過分を書き込み計画から**外す** (`enforceActivityQuota`)。復元 (backup ZIP / JSON の全体復元・クラウド取込) は顧客のデータを落とせないため超過分を **`isArchived=1` + `archived_reason=downgrade_user_selected` で書く** (`archiveActivityQuotaOverflow`)。後者はアップグレード時に `restoreArchivedResources` が自動復帰させる。
