...
- **取込 / 復元 gate (`activity-quota.ts`) の 2 適用方式**: 判定は共通 (`judgeActivityQuota`) で、適用だけが違う。
  ...
  - **復元** (`presetId` 無しの `dispatchImport` = 活動管理の ︙ →「バックアップから復元」/ `api/v1/activities/import` merge、backup ZIP / JSON の全体復元、クラウド取込) → 超過分を **`isArchived=1` + `archived_reason=downgrade_user_selected` で書く** (`archiveActivityQuotaOverflow`)。捨てない。アップグレード時に `restoreArchivedResources` が自動復帰させる。**復元 4 経路すべてが同じ適用方式・同じ顧客文言** (`ACTIVITY_QUOTA_LABELS.restoreArchivedResult`)
