...
- **tenant 無束縛 Scan の残党 (棚卸 2026-06-12)**: write 解決 = trial-history
  ...
  read = activity-repo `hasActivityLogs` / `findActivityLogById` の 2 箇所は **#3044 (PR、2026-06-16)
  で根治済**: SK prefix Scan に `begins_with(PK, :tenantPrefix)` (= `tenantPK('CHILD#', tenantId)`)
  を追加し自 tenant 内に束縛 (childId を interface が受けないため exact Key 不可、in-file 先例
  `resolveChildIdForActivity` と同型の tenant Scan + paging)。cross-tenant read IDOR 形状を解消。
- tenant 束縛済だが childId を interface が受けない **tenant Scan 解決の同型残党** (cross-tenant
  ...
  (`updateActivity` / `deleteActivity` / `setActivityVisibility`) / child-challenge
  (`resolveKeyById` 系 mutation) / battle (`completeBattle`) / certificate (`findCertificateById`、
  read) / sibling-cheer (`markShown`)。新規 repo は本原則を必須とし、残党は当該 repo を触る PR で
  本原則に揃える。
...
| 2026-06-16 | 5.21 | #3044 — #2845 棚卸が認定した read 残党の根治。activity-repo (DynamoDB) の `hasActivityLogs` / `findActivityLogById` の tenant 無束縛 SK prefix Scan に `begins_with(PK, :tenantPrefix)` (= `tenantPK('CHILD#', tenantId)`) を追加し自 tenant 内に束縛 (両関数とも childId を interface が受けないため exact Key 不可、in-file 先例 `resolveChildIdForActivity` と同型の tenant Scan + paging を維持)。tenant 別採番 id/activityId 衝突時に別 tenant の log record/存在を返す cross-tenant read IDOR 形状を解消。呼び出し側 (facade / service / route) は既に tenantId を渡しており配線変更なし。SQLite / demo は単一テナント (NUC 自宅 1 家庭 / in-memory fixture) で cross-tenant 行が存在せず scope 外。regression guard: `tests/unit/db/dynamodb-activity-repo-read-idor-3044.test.ts` (literal tenant prefix assert + 別 tenant no-op + paging)。schema 変更なし / CDK 変更なし |
