...
- **tenant 無束縛 Scan の残党 (棚卸 2026-06-12 / #3044 修正後)**: write 解決 = trial-history
  ...
  read 系の activity-repo `hasActivityLogs` / `findActivityLogById` 2 箇所は #3044 で tenant 束縛済
  (下記「tenant 束縛済だが childId を interface が受けない tenant Scan 解決の同型残党」へ移行)。
- tenant 束縛済だが childId を interface が受けない **tenant Scan 解決の同型残党** (cross-tenant
  ...
  (`updateActivity` / `deleteActivity` / `setActivityVisibility`) / activity-repo read
  (`hasActivityLogs` / `findActivityLogById`、#3044 で `begins_with(PK, T#<tenant>#CHILD#)` を
  FilterExpression に追加し全テーブル Scan を tenant child partition に閉じた。read 系も write 系と
  同じく FilterExpression での tenant 束縛が原則。childId 未受領のため exact Key 化は不可) /
