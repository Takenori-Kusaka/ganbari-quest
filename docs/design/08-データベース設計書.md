...
3. `deriveTenantLicenseStatus(tenant)` で `licenseStatus` を決定（認証経路 `cognito.ts` と共用の SSOT、`src/lib/server/auth/tenant-license-status.ts`）
   - `stripeSubscriptionId` あり + `status=active|grace_period` → `'active'`
   - `stripeSubscriptionId` あり + その他 → `'suspended'`
   - `stripeSubscriptionId` なし + `plan` あり + 期限内（or 無期限） → `'active'`（license 時代 paid テナントの entitlement 保全、#2894）
   - `stripeSubscriptionId` なし + `plan` あり + 期限切れ → `'expired'`
   - `stripeSubscriptionId` なし + `plan` あり + `status=suspended|terminated` → `'none'`
   - `stripeSubscriptionId` なし + `plan` なし（free 新規） → `'none'`
