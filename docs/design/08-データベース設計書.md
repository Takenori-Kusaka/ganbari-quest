...
> **#2824 Wave 6B（本実装で stub 全廃の最終段階）**: cloud_exports / report_daily_summaries /
> viewer_tokens の 3 repo を本実装し、`scripts/dynamodb-stub-baseline.json` から削除した。
> これにより stub baseline は 49 → 31 method に減少。残り 5 repo（activity-repo / auto-challenge /
> battle / certificate / sibling-cheer）は本 PR scope 外で、別 Wave で順次本実装する（baseline
> ratchet `scripts/check-dynamodb-stub-parity.mjs` が新規 stub の再混入を CI で禁止し続ける）。
