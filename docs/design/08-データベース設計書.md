...
- 適用済: `markMessageShown` / `updateCardStatus*` / `updateRedemptionRequestStatus` /
  ...
  未適用の同型残党 (childId を interface が受けない tenant Scan 解決): activity facade
...
| `incrementDownloadCount(id)` | Scan（id filter）で raw key 解決 → UpdateItem（ADD +1） | id のみ受けるため tenant 不明、Scan で 1 件特定 |
