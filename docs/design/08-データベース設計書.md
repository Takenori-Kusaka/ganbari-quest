...
- **tenant 無束縛 Scan の残党 (棚卸 2026-06-12、本 PR 修正後)**: write 解決 = trial-history
  ...
  read = activity-repo `hasActivityLogs` / `findActivityLogById` の 2 箇所 (`_tenantId` 未使用の
  SK prefix Scan。cross-tenant 情報開示形状のため同様に当該 repo を触る PR で tenant 束縛する)。
