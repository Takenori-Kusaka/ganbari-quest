...
| **持ち物チェックリスト LP スクショ** (#1164 / #2097 PR-B1 / B-3) | `site/screenshots/feature-belongings-checklist{,-desktop}.webp` (`/checklist?childId=903` 撮影、PR-B3 #2188 で本番 path に統一) | 高解像度の持ち物タブ実画面 | `site/screenshots/` | 780×1688 (mobile@2x) / 2880×1800 (desktop@2x) WebP | 2 |
...
- **家庭視点ベネフィット（1 行）併記**（#1793）: 各 scrshot 直下に文脈別の 1 行ベネフィット（例: 「家庭で起きること」「家庭で楽になること」「家庭ごとにカスタマイズできること」「家族で実感できること」）を必ず併記し、機能の効果を「親が監視する」のではなく「家庭で実感する」視点で言語化する。旧表記「親が観測できること」は 2026-05 に刷新済み（PO 指針: 監視連想で permission marketing が毀損するため不採用）
...
| ファイル名 | 配置 | サイズ | 生成方法 |
|----------|------|--------|---------|
| `core-loop-summary.webp` (#1889 で正本化) | `static/assets/lp/`（git tracked、SSOT） | 640×320 viewBox（透過） | 手動編集（ベクター + 透過ネイティブ） |
...
1. **archive SVG (`_archive/core-loop-summary.webp.bak`) を `static/assets/lp/core-loop-summary.webp` に正本化**
...
#   → static/assets/lp/core-loop-summary.webp を読み込み
...
- **画像内テキストは置かない**（HTML 側 `figcaption` + `alt` が SSOT、ブランド A-1 整合）
  ...
  - #1889 で SVG 正本化（`static/assets/lp/core-loop-summary.webp`）+ 背景 rect 撤去で透過化済
