...
|---|---|---|---|
| 1 | `GRACE_PERIOD_DAYS=7` app ハードコード (config.ts:95)、`handlePaymentFailed` が独自に grace 計算 (stripe-service.ts:346) | Stripe Smart Retries 2週を SSOT に | **変更** (app 日数管理を廃止、Stripe SSOT 化) |
| 2 | `handleSubscriptionDeleted` は status を SUSPENDED にするのみ、plan 無料化なし (stripe-service.ts:394) | unpaid/canceled で plan を無料に戻す | **新規実装** (FR-3) |
| 3 | webhook 冪等性 (event.id dedup) なし | event.id 冪等性 | **新規構築** (NFR-1、DB table 新設) |

**影響範囲**: 支払い失敗 grace は **LP 非言及のため LP 影響なし**。変更は config.ts (GRACE_PERIOD_DAYS 廃止) + stripe-service.ts (handlePaymentFailed/Deleted) + webhook 冪等性 DB + Stripe Dashboard (Smart Retries 設定) に限定。実装は Phase 6/7。
