...
| `LICENSE_KEY_STATUS` enum | **物理削除** (OQ-4 確定、§3.8) |
| `LICENSE_PLAN` enum | **`SUBSCRIPTION_PLAN` へ rename** (PR-L5 #2879)。OQ-4 判断時に license key 関連と誤分類していたが、実体は Tenant の課金プラン (Stripe Subscription) 種別の正規 enum で 10+ live billing file が依存。物理削除でなく rename で `license` 語彙を排除し leak gate に追加 |
...
3. **contract (PR-L5)**: migration script で `licenseKey` 列 DROP + `LICENSE_KEY_STATUS` enum 定義削除 + `LicenseRecord` table DROP (4 backend: sqlite/dynamodb/demo/fixture)。`LICENSE_PLAN` enum は物理削除でなく `SUBSCRIPTION_PLAN` へ rename (PR-L5 #2879、Tenant 課金プランの正規 enum のため)
...
| 10 | 設計書 archive / deprecation 同期 | 5 file deprecation header + 参照元 link 同期 | ✅ **本 PR #2892 で完遂** (07-API / 08-DB / 19-pricing / 24-arch / account-deletion-flow / plan-change-flow / stripe-dashboard-runbook / runbook) |
