...
flowchart TB
    ...
    Sub -->|ファミリーにする CTA| Confirm[/admin/subscription/confirm<br/>本画面 #2573]
    ...
    Success -->|webhook 権限付与| Activated[ファミリー機能解放]
    Confirm -.「やめる」.->Sub
...
│ ご家族のプランを「ファミリー」に変更します                   │  ← subtitle
└─────────────────────────────────────────────────────────────┘
...
export const SUBSCRIPTION_CONFIRM_LABELS = {
	...
	pageSubtitleFamily: `ご家族のプランを「${PLAN_TERMS.family}」に変更します`,
	// ① 分量 + ② 販売価格
	planLabel: 'プラン',
	...
	autoRenewalNoticeFamily: `自動更新を停止しない限り、月額 ${PRICE_TERMS.family} ${PRICE_TERMS.taxNote}が毎月課金されます`,
...
- `PLAN_TERMS.standard` / `.family` (現状の atom 表示名は「スタンダード」/「ファミリー」を維持、Phase 3 #2609 SSOT 衝突解決で「プレミアム」表記は採用しない)
...
- FreeToFamily        // 無料 → ファミリー 新規 (proration なし、6 ブロックのみ)
- FreeToStandard      // 無料 → スタンダード 新規 (6 ブロック、価格 ¥500)
- StandardToFamily    // スタンダード → ファミリー (proration 差額表示 + 6 ブロック)
- FamilyToStandard    // ファミリー → スタンダード (downgrade、proration 戻入表示 + #2575 DowngradeResourceSelector 統合事前確認)
...
| 変数 | URL | 状態 | 用途 |
|---|---|---|---|
| `confirm-free-to-family` | `/admin/subscription/confirm?plan=family` | 無料 → ファミリー | 6 ブロック網羅 (PC + mobile 2 viewport) |
| `confirm-free-to-standard` | `/admin/subscription/confirm?plan=standard` | 無料 → スタンダード | 6 ブロック (価格 ¥500) |
| `confirm-standard-to-family` | `/admin/subscription/confirm?plan=family` (Standard 加入者) | proration 差額表示 | 機能 3 動的計算 |
...
test('plan-change (standard→family) で proration 差額表示', async ({ page }) => {
...
- `PLAN_TERMS.family` (= 「ファミリー」、Phase 3 #2609 SSOT 衝突解決で表示名は維持) vs 内部 enum `'family'` / DB `plan_tier='family'` の整合 — Phase 1 #2588 FR-5 で明文化済
...
- `SaasSubscriptionPanel.svelte` (Phase 7 rename) の「ファミリーにする」CTA → 本画面遷移 (現状の Stripe Checkout 直遷移を 1 段階噛ませる変更)
...
- 「ご家族のプランを『ファミリー』に変更します」(subtitle) で **無料からの能動的選択** を明示
...
7. `SaasSubscriptionPanel.svelte` の「ファミリーにする」CTA を `/admin/subscription/confirm` 遷移に変更
...
| 8 | **3 ペルソナ UX レビュー前倒し検討** (Phase 7 SS A/B でなく本 PR docs 段階で Figma mockup レビュー判断)。§7.4 で慎重派 / 即決派 / 法務確認の 3 ペルソナ UX review を Phase 7 PR で実施予定だが、本 docs PR 段階で Figma mockup (6 ブロック構造 + proration 差額表示) を作成して**事前 3 ペルソナ review を前倒し**するべきか。前倒しメリット: Phase 7 実装着手前に UX 不適合を発見できる / コスト: docs PR scope 膨張。判断軸: 法務 critical case の特殊性 (失敗コスト高) で前倒し優先か、Phase 7 PR の即時着手 (Phase 進行優先) か | 要 PO 判断、推奨は前倒し (#2525 critical case 整合) |
