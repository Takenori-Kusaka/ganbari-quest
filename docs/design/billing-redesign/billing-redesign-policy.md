...
| 項目 | 内容 |
|------|------|
| 版数 | 確定 (Phase 1 基本方針、PO 確定 2026-05-27) |
| 作成日 | 2026-05-27 |
| 関連 Epic | #2525 (上位 Epic「課金/プラン体系 再設計」、7 phase。#2514 は Phase 6 = 実装詳細の子 issue) |
| 本 Phase | #2526 (Phase 1/7 要件定義) |
| ステータス | 確定 (現状把握 / 業界標準 / 任意タイミングトライアル を PO 確定。Phase 2 以降の土台) |
| 配置 | docs/design/billing-redesign/ (Phase 1 成果物として確定配置) |

> **本書の位置づけ**: 全体計画 (要件定義 → UX → UI → 動線 → アーキ → 実装詳細 → 実装) の**最上流 = 基本方針**。本書を土台に、詳細設計書 (購入/契約/トライアル/解約/データ/セキュリティ/法務 の各フロー) に展開する。索引は [README.md](README.md) を参照。
...
| 版 | 日付 | 変更内容 |
|----|------|---------|
| 0.1 | 2026-05-27 | 初版 (調査完了 → 基本方針、PO レビュー待ち、配置 tmp/reviews) |
| 確定 | 2026-05-27 | PO 確定。docs/design/billing-redesign/ へ昇華。上位 Epic を #2525 に確定 (#2514 は Phase 6 子)。Phase 1 成果物として確定配置 (索引 README.md) |
