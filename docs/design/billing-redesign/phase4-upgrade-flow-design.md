...
| #2790 license key 全廃整合 | Phase 1 補強 3 (#2790、PR #2790 マージ済) で **license key = SaaS / NUC 問わず完全全廃** に確定。本アップグレード動線の entitlement 解放は **Stripe `subscriptions.update` (`proration_behavior=always_invoice`) → webhook `customer.subscription.updated` → capability 即時解放** (本書 §3.3 図 1) であり、**license key を一切経由しない** (#2790 §2.1 認可一本化)。動線終端 `/admin/subscription` は #2620 で `/admin/license` 308 redirect 確定済 + #2790 §3.4 / §3.5 で `/help/license-key` 削除 (301) + `/ops/license/*` 削除 (redirect 不要) と整合 |
