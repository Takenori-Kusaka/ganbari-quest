...
| Phase 7 連動 | atom 3 種 + compound 5 種 追加 + `LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS` rename (naming 軸) + `SUBSCRIPTION_PAGE_LABELS` 内 `licenseKey*` 文言値 (27 key) **削除** (license key 全廃軸) + `PLAN_TERMS.family` → `PLAN_TERMS.premium` rename + `generate-lp-labels.mjs` 再生成 (5 step PR 計画 §6 + license key 削除 step §6.6) |
| 担当 PO 手動操作 | なし (atom rename は機械置換、95 件は atom 1 行修正で伝播。license key 文言値削除も機械置換) |
| license key 全廃整合 (#2788) | Phase 1 補強 3 (`phase1-license-key-removal-final-requirements.md`、PR #2790 マージ済) で license key = SaaS / NUC 問わず**全廃**確定。本 docs の atom 設計は **新 `LICENSE_KEY_TERMS` atom を起こさず**、`SUBSCRIPTION_PAGE_LABELS` 内の「ライセンスキー」文言値 27 key を**削除**する (license key 概念消滅、NUC でも billing proof 不要)。naming rename (`LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS`) と license key 文言値削除は**別軸**。詳細 §6.6 |
...
- **license key 文言値が `SUBSCRIPTION_PAGE_LABELS` 内に 27 key 残存 (#2788 で全廃確定後の未反映)**: Phase 7 PR-2c (#2699) の `LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS` rename で「ライセンスキー適用」section の `licenseKeyTitle` / `licenseKeyDesc` / `licenseKeyConfirm*` 等 27 key が **rename 元から機械的に引き継がれた**。だが Phase 1 補強 3 (#2788、PR #2790) で license key = SaaS / NUC 問わず**全廃**確定 (`phase1-license-key-removal-final-requirements.md` §3.1 機械削除 / §3.3 冗長層削除)。これらの文言値は **新 `LICENSE_KEY_TERMS` atom に集約するのではなく、key ごと削除**する (license key 概念消滅、NUC でも billing proof 不要 = `NUC_EDITION_TERMS` 全機能無制限のため)。naming rename と license key 文言値削除を **同一視すると、rename 完了報告で「license key 文言が消えた」と誤認**する
...
- **構造継承 (license key key を除く)**: `LICENSE_PAGE_LABELS` 既存 namespace の全 key を `SUBSCRIPTION_PAGE_LABELS` に rename + Phase 3 #2567 §文言 atom 確定済 9 key を追加。ただし **`licenseKey*` 文言値 (27 key、§6.6 一覧) は継承対象外で削除**する (license key 全廃軸 = #2788、naming rename とは別軸)
- **rename 影響**: 95 件参照 → atom rename と同時に 1 PR 内で一括置換 (Phase 7 step 3 SSOT)
- **naming rename ≠ license key 削除 (最重要)**: `LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS` は **namespace 名の rename (naming 軸)**。`licenseKey*` key 削除は **license key 機能の全廃 (#2788 全廃軸)**。2 つは独立した変更で、PR-2c (#2699、naming rename 済) の延長線上に license key 削除があるわけではない。新 `LICENSE_KEY_TERMS` atom は **起こさない** (license key 概念消滅、NUC でも billing proof 不要)
...
|---|---|---|
| `PLAN_TERMS.standard` / `.family` (Phase 7 step 4 で `PLAN_TERMS.premium` rename 計画、B-1 SSOT 衝突 user 判断委ね中) | スタンダード / ファミリー (rename 後の atom 名は未確定) | #2567 / #2573 / #2575 / #2621 / #2624 全件 |
| `PLAN_FULL_TERMS.*` | スタンダードプラン / ファミリープラン (Phase 7 step 4 rename 後 atom 名は規約 21 SSOT 衝突解消後に user 判断、B-1 escalation 中) | 同上 |
...
### 4.1 SUBSCRIPTION_PAGE_LABELS (新規 + rename 元継承、license key key 削除) — Phase 3 #2567 §文言 atom 確定済

Phase 7 step 3 で **既存 `LICENSE_PAGE_LABELS` (L1633) を本 namespace に rename + #2567 §文言 atom 9 key 追加 + `licenseKey*` 文言値 27 key を削除** (§6.6)。docs SSOT は #2567 §文言 atom を継承。

> **license key 文言値の扱い (#2788 全廃整合)**: 現状 `SUBSCRIPTION_PAGE_LABELS` には PR-2c (#2699) の naming rename で引き継がれた `licenseKeyTitle` / `licenseKeyDesc` / `licenseKeyApplyButton` / `licenseKeyConfirm*` / `licenseKeyHelp*` / `licenseKeyAgree*` / `demoLicenseKey*` 等 27 key が残存するが、Phase 1 補強 3 (#2788) で license key 全廃確定のため **本 namespace から削除**する。新 `LICENSE_KEY_TERMS` atom には集約しない (license key 概念が消えるため atom 化の対象自体が消滅)。削除 step は §6.6。
...
- **表示プラン名 (`PLAN_TERMS.family`) vs 内部識別子 (`'family'` enum / `family-tenant`)**: Phase 1 補強 2 FR-5 で明文化済。step 4 atom rename PR で表示名のみ rename、enum / DB 値は別 PR (Phase 7 step 別)。なお **`LICENSE_KEY_STATUS` / `LICENSE_PLAN` enum は #2788 で物理削除確定** (`phase1-license-key-removal-final-requirements.md` §3.4 / §3.8、PR-L5 contract)。license key 概念消滅に伴い、本 docs では同名異義の対象から除外する (atom 設計には一切登場しない)
...
| **A-2 DB 保存済 string / enum (license key 全廃)** | `LICENSE_KEY_STATUS` / `LICENSE_PLAN` enum + `licenseKey` 列を物理削除 (#2788 §3.4 / §3.8、本 docs atom 設計には登場しない) | Phase 1 補強 3 PR-L5 (contract、`phase1-license-key-removal-final-requirements.md`、本 §6 atom step とは直交) |
| **その他 16 カテゴリ** | 影響なし (DB の license key 以外 / Stripe / Cognito / email template / analytics 等) | 確認のみ、Phase 7 PR 別 |
...
### Step 3: `LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS` rename (naming 軸) + `licenseKey*` 文言値削除 (全廃軸) (1 PR、推定 250 行 + 警告 1 件)

| 項目 | 内容 |
|---|---|
| rename 対象 (naming 軸) | `labels.ts` の `LICENSE_PAGE_LABELS` (L1633) → `SUBSCRIPTION_PAGE_LABELS` (Step 2 で追加した新規 9 key と統合) |
| 削除対象 (全廃軸、§6.6) | `SUBSCRIPTION_PAGE_LABELS` 内 `licenseKey*` 文言値 27 key を削除 (#2788 license key 全廃整合)。新 `LICENSE_KEY_TERMS` atom は起こさない |
| 影響範囲 | 218 件 参照 (Phase 1 補強 1 確認、`*.svelte` / `*.ts` 全件) — atom 1 行修正で全件自動伝播 + license key 参照 svelte (Phase 1 補強 3 PR-L3 で物理削除済) |
| AC | Step 2 マージ済 + `npx svelte-check` PASS + `npx vitest run` PASS + 218 件 grep 0 件 (旧名残存ゼロ) + `grep "licenseKey" labels.ts` 0 件 (`SUBSCRIPTION_PAGE_LABELS` 配下) + `check-license-key-leak.mjs` PASS + Storybook 起動確認 + `npm run pre-ready -- --pr <num>` 10 step PASS |
| Pre-merge | Step 2 マージ必須 + Phase 1 補強 3 PR-L3 (license key 参照 svelte 物理削除) マージ済 (labels.ts key 削除前に参照消滅、import error 回避) + PR size 250 行警告 (ADR-0020) 分割不可理由明示 (rename は一体性必要、分割すると intermediate state でビルド失敗) |
...
### 6.6 license key 文言値削除 step (#2788 全廃整合、Step 3 に統合)

Phase 1 補強 3 (#2788、PR #2790 マージ済) で license key = SaaS / NUC 問わず**全廃**確定したため、`SUBSCRIPTION_PAGE_LABELS` 内の `licenseKey*` 文言値を **labels.ts から削除**する。本作業は **naming rename (§6 Step 3 = `LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS`) と別軸の license key 全廃軸**だが、同一 namespace を触るため **Step 3 PR に統合**して conflict を避ける (新規 Step 6 として並列化しない)。

| 項目 | 内容 |
|---|---|
| 削除対象 (27 key) | `currentPlanLicenseKey` / `licenseKeyTitle` / `licenseKeyDesc` / `licenseKeyApplySuccess` / `licenseKeyInputLabel` / `licenseKeyHelpToggle` / `licenseKeyHelpOnce` / `licenseKeyHelpOnceDesc` / `licenseKeyHelpOverwrite` / `licenseKeyHelpOverwriteDesc` / `licenseKeyHelpBound` / `licenseKeyHelpBoundDesc` / `licenseKeyHelpIrreversible` / `licenseKeyHelpIrreversibleDesc` / `licenseKeyApplyButton` / `licenseKeyConfirmTitle` / `licenseKeyConfirmDesc` / `licenseKeyConfirm*` (Once/Plan/Bound/Irreversible 系) / `licenseKeyEnteredKey` / `licenseKeyAgree*` / `licenseKeyCancel` / `licenseKeyApplyLoading` / `licenseKeyApplyConfirm` / `demoLicenseKeyTitle` / `demoLicenseKeyDesc` (実数は labels.ts 現状で `grep -c "licenseKey" src/lib/domain/labels.ts` で確定、約 27 key) |
| atom 化 | **しない** — 新 `LICENSE_KEY_TERMS` atom は起こさない。license key 概念消滅 (NUC でも billing proof 不要 = `NUC_EDITION_TERMS` 全機能無制限) のため、atom 集約ではなく **key ごと物理削除** |
| 参照側削除 | `src/routes/(parent)/admin/license/**` (Phase 1 補強 3 §3.1 で物理削除) の参照消滅と同期。削除順は「参照 svelte 削除 → labels.ts key 削除」(import error 回避) |
| PR 統合 | Phase 1 補強 3 PR-L3 (routes 物理削除) で参照側を消し、本 labels.ts key 削除を Phase 7 Step 3 (naming rename PR) に同梱。**naming rename と license key 削除を同一 PR で完結**させ、共存期間 alias (`LICENSE_PAGE_LABELS = SUBSCRIPTION_PAGE_LABELS`) 撤去と整合 |
| AC | `grep "licenseKey" src/lib/domain/labels.ts` 0 件 (`SUBSCRIPTION_PAGE_LABELS` 配下) + `npx svelte-check` PASS (参照消滅確認) + `check-license-key-leak.mjs` (#2788 §5 項目 7) PASS |

> **5 step 計画との関係**: 本 §6.6 は新規 Step を増やさず、§6 Step 3 (naming rename) に license key key 削除を統合する判断を明記したもの。`PLAN_TERMS.family` → `.premium` rename (Step 4) とは独立 (family は plan 軸、license key は billing proof 軸)。
...
| Phase 1 補強 3 (license-key-removal、#2788 / PR #2790) | ✅ license key 全廃整合: `SUBSCRIPTION_PAGE_LABELS` 内 `licenseKey*` 文言値 27 key を **削除** (§4.1 / §6.6)、新 `LICENSE_KEY_TERMS` atom は**起こさない** (license key 概念消滅、NUC でも billing proof 不要)。naming rename (§6 step 3) と license key 削除 (全廃軸) を別軸として §2 原則 4 で明示。`LICENSE_KEY_STATUS` / `LICENSE_PLAN` enum 物理削除 (#2788 §3.8) は本 docs atom 設計と直交 (§5 L4 A-2) |
...
| **security** | Step 1 で `TOKUSHOHO_TERMS` 追加時、改正特商法第12条の6第2項「誤認表示禁止」遵守のため、特商法 6 項目見出し / 主要文言が tokushoho.html / Stripe Checkout `custom_text` / `/admin/subscription/confirm` 3 経路で完全一致するか自動検証する CI gate を追加するか (Phase 3 #2573 §法令根拠で確定済要件) | **暫定: 既存 `npm run pre-ready` Step 8 (`generate-lp-labels --check`) で部分担保 + Phase 7 step 5 後に 3 経路一致 grep 検証スクリプトを scripts 配下に新規追加 (check-tokushoho-consistency という命名で Phase 1 #2541 法務統合 PR で実装予定)**。本 docs では SSOT 配置確定のみ、CI gate 追加は別 PR (Phase 7 別 step) | PO 判断必要 + security 必須 |

### Adversarial 3 軸 Phase 7 申し送り (Round 2 追記、#2659 ADR-0058 整合後)

本 PR (#2643) は Phase 5 atom SSOT 配置確定までを scope とし、実装は Phase 7 各 step PR で行う。Adversarial Reviewer 3 軸 (business / UX / security) で識別された主要リスクと緩和策を以下に集約し、Phase 7 担当者への申し送りとする。

- **business**: ADR-0058 で `family` → `premium` rename が SSOT 化済 (#2659 merge)。Phase 7 Step 4 atom rename 計画は ADR-0058 と整合済だが、atom 名称 (例: `PLAN_TERMS.premium` / `PLAN_FULL_TERMS.premium` の確定値) を本 PR では空欄のまま先行 merge する場合、Phase 7 各 step PR で SSOT 名称未確定リスクが残る → Phase 7 Step 1 で atom 名称確定を担当割当
- **UX**: Phase 7 95 件一斉 rename での UX 断絶対策として、移行期 banner / 旧名称 (「ファミリープラン」) 併記期間 / 既存契約者通知の 3 緩和策を §5 緩和策に列挙済。1 人っ子家庭の除外感は Phase 3 #2567 §FR-4 framing 軸 V4 (decoy: premium 最右 + standard 「✓ お勧め」) で担保。Phase 7 SS UX レビュー (3 ペルソナ: 1 人っ子家庭 / 兄弟複数 / 卒業期) で「プレミアム」誤認率を測定、SSOT は Phase 7 担当者に委譲
- **security**: `TOKUSHOHO_TERMS` atom 追加と LP 反映の時間差で 3 経路 (Stripe Checkout / tokushoho.html / /admin/subscription/confirm) 乖離 window が発生するリスク。緩和策として Phase 7 step 5 で 3 経路一致 grep 検証スクリプトを scripts 配下に新規追加 (check-tokushoho-consistency という命名、Phase 1 #2541 法務統合 PR で実装予定) し、3 経路 atom 値統一を CI gate 化する → Phase 7 step 5 担当に SSOT 委譲
...
- **license key 全廃整合 (#2788 / PR #2790、2026-06-03 反映)**:
  - deep-research v3 は「`LICENSE_KEY_TERMS` 新 atom (NUC 内部用語) を起こす」案を提案していたが、Phase 1 補強 3 (#2788) で **license key = SaaS / NUC 問わず全廃**確定 (NUC でも billing proof 不要 = `NUC_EDITION_TERMS` 全機能無制限) のため、**新 atom 不要・文言値ごと削除**に訂正
  - `SUBSCRIPTION_PAGE_LABELS` 内の `licenseKey*` 文言値 27 key (PR-2c #2699 naming rename で引き継がれた残存) は §4.1 / §6.6 で削除 step を SSOT 化
  - naming rename (`LICENSE_PAGE_LABELS` → `SUBSCRIPTION_PAGE_LABELS`、PR-2c 済) と license key 文言値削除 (全廃軸) は別軸 (§2 原則 4 / §6.6 で明示)
- **関連 Phase 1+2+3+4 docs**:
  ...
  - [phase1-license-key-removal-final-requirements.md](phase1-license-key-removal-final-requirements.md) (#2788 / PR #2790 補強 3 — license key 全廃。§4.1 / §6.6 で `SUBSCRIPTION_PAGE_LABELS` 内 `licenseKey*` 文言値 27 key 削除 + 新 `LICENSE_KEY_TERMS` atom 不要を反映)
