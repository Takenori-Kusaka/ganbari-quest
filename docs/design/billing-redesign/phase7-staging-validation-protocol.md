...
| 関連 ADR | 旧 ADR-0059 (Phase 7 cutover sequence §3 rollback 3 期間別マトリクス + §2 kill switch 機構、#2440 PR-A5 で削除。cutover sequence / kill switch の設計内容は本 docs および phase6-* SSOT が保持、決定経緯は git 履歴参照) |
...
| 1.2 | PR-3b の prerequisite (#2718 本 docs / #2719 yearly plan **物理削除完遂**: env var 6 → 2 (`.env.example` / `src/lib/runtime/env.ts` / `infra/lib/compute-stack.ts` / `.github/workflows/deploy.yml` 4 配布経路全て）、`getPriceId()` interval 引数削除、checkout +server.ts validPlanIds 月額 2 種化、QA #2722 BLOCK V-3 yearly silent failure risk 完全消滅 / #2720 Sentry alert) 全 close 確認 | gh issue view + `grep -r 'STRIPE_PRICE.*YEARLY' .env.example infra/ .github/workflows/`= 0 件 |
...
| 1.5 | `HISTORICAL_YEARLY_AMOUNTS` SSOT 確認 — 過去 yearly 契約者 (`tenant.plan === 'yearly' \| 'family-yearly'`) の MRR は `src/lib/server/services/stripe-metrics-service.ts` 26 行目 `HISTORICAL_YEARLY_AMOUNTS = { yearly: 5000, 'family-yearly': 7800 }` (旧 STRIPE_PRICE_STANDARD_YEARLY ¥5,000 / STRIPE_PRICE_FAMILY_YEARLY ¥7,800) で fallback。新規 yearly checkout は `stripe-service.ts` で reject。本 map は historical compat 唯一の SSOT。値変更は過去契約者の MRR 表示にのみ影響 | grep `HISTORICAL_YEARLY_AMOUNTS` src/lib/server/services/stripe-metrics-service.ts + vitest `tests/unit/services/stripe-metrics-service.test.ts` `calculateMRR` PASS |
| 1.6 | historical yearly tenant の admin/license UX 動線整備 — `LICENSE_PLAN.YEARLY` / `FAMILY_YEARLY` constants 維持 + `SaasLicensePanel` の `planLabel()` switch 維持で display 崩れなし。yearly request 400 reject 時の代替経路は `/admin/license` から「プラン変更」CTA 経由で monthly 2 種への switch を案内 (新規 yearly 訴求 LP 側で既に non-existent、cache 経由古 request は `INVALID_PLAN` 400 で明示エラー) | UI 目視 (`/admin/license` for historical yearly tenant fixture) |
...
| 3.5 | Stripe API 障害 fallback test (artificial network error 注入で `USE_LOOKUP_KEY=false` 即時切戻し成功) | PASS | FAIL | kill switch 機構 (旧 ADR-0059 §2、git 履歴参照) |
...
- 旧 ADR-0059 (Phase 7 cutover sequence + kill switch、#2440 PR-A5 で削除) — cutover sequence / kill switch の設計 SSOT は本 docs および phase6-* に保持、決定経緯は git 履歴参照
