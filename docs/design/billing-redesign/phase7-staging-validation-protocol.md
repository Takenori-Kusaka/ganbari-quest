...
| 1.2 | PR-3b の prerequisite (#2718 本 docs / #2719 yearly plan **物理削除完遂**: env var 6 → 2、`getPriceId()` interval 引数削除、checkout +server.ts validPlanIds 月額 2 種化、QA #2722 BLOCK V-3 yearly silent failure risk 完全消滅 / #2720 Sentry alert) 全 close 確認 | gh issue view |
