...
| ステータス | Phase 1 完了 (deep-research: nuc-saas-runtime-bifurcation.md + セルフホスト OSS 5 件公式 + 実コード照合 → PO 確定 2026-05-27、事業判断 2 論点も確定) |
...
根拠: ① 業界 4/5 が信頼ベース ② ADR-0010 (Pre-PMF 過剰防衛禁止) — offline DRM は典型的過剰防衛、NUC は家庭内 offline 機器で攻撃面極小 ③ nuc-saas-runtime-bifurcation.md (NUC=Edition 配布形態、買い切り SKU でない=ライセンス検証対象でない、配布物自体が正規の証跡) ④ 既存 `SelfHostedPlanProvider` family 固定設計が妥当
...
| ID | 要件 | 設計意図 + 根拠 |
|----|------|----------------|
| FR-1 | NUC は plan tier を **family 固定で解決**、Stripe/license key 非依存 | 既存 SelfHostedPlanProvider 妥当性確認。nuc-saas-runtime-bifurcation.md「NUC=全機能」 |
...
| FR-4 | `/admin/license` は nuc-saas-runtime-bifurcation.md の NucLicensePanel (Edition badge + 利用状況 + サポート link) を維持、key 入力欄なし | nuc-saas-runtime-bifurcation.md で既に決定済 |
| FR-5 | SaaS 版は Stripe Subscription を plan SSOT、NUC とは plan 解決経路を完全分離 | nuc-saas-runtime-bifurcation.md bifurcation |

## 非機能要件 (NFR)

- NFR-1: NUC は**完全オフライン動作** (外部到達ゼロで全機能) | NUC 存在意義 (nuc-saas-runtime-bifurcation.md 家庭にデータを閉じる)
- NFR-2: 正規性判定機構を**新規に作らない** (DRM 不採用) | ADR-0010、業界 4/5 信頼ベース
- NFR-3: NUC/SaaS 分岐は `locals.runtimeMode` SSOT 経由・page 層 1 箇所集約 | nuc-saas-runtime-bifurcation.md
...
| 3 | NUC の機能差別化 (全機能 family vs 一部 SaaS 限定) | 全機能 family が nuc-saas-runtime-bifurcation.md 整合、収益モデル次第 |
...
- nuc-saas-runtime-bifurcation.md (NUC-SaaS Bifurcation、Edition 配布形態)
