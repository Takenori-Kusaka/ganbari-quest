...
### 3.10-c R12 (#3980 新規): subscription item が複数になり `items.data[0]` が plan を表さなくなる

| 項目 | 内容 |
|---|---|
| **シナリオ** | `resolvePlanFromSubscription()` は `subscription.items.data[0].price` から plan を解決する。これは「subscription item は常に 1 件」という現行の Stripe 商品構成 (checkout は単一 `line_items` / 2 Product × 各 1 Price) に依存している。add-on (追加子どもアカウント枠 等)・従量課金 item・年額 item の同居のいずれかを導入すると、`data[0]` は「最初に追加された item」を返し、それが plan を表す item である保証が消える。`planIdFromPriceId` / `planIdFromLookupKey` は add-on の price を解決できず `null` を返すため、**課金は変わったのに plan が据え置きになる** (#3960 と同一症状) |
| **検知 method** | (a) Discord alert `stripe-subscription-multi-item` (#3980 で新規追加) (b) CloudWatch Logs Insights `filter kind = "stripe-subscription-multi-item"` で `subscriptionId` / `itemCount` / `priceIds` を特定 |
| **ロールバック手順** | (1) alert の `tags.priceIds` で「どの price が plan で、どれが add-on か」を確定 (2) plan を表す item を lookup_key で選別する分岐を `resolvePlanFromSubscription()` に追加して deploy (3) alert 期間中に誤解決された tenant があれば Stripe 現行 price と DB plan を突合して修復 |
| **再発防止** | (a) 先頭参照の前提が崩れた瞬間を alert で検知 (plan 解決の挙動自体は変えない — 単一 item のうちは検証できない分岐を増やさない) (b) `items.length > 1` / `=== 1` 双方の unit test (`tests/unit/services/stripe-service.test.ts`) で発火・誤発火を固定 |

### 3.10-d R13 (#3981 新規): context 解決の bare `catch {}` が障害を「tenant not found」に化けさせる

| 項目 | 内容 |
|---|---|
| **シナリオ** | `resolveSubscriptionContext()` の bare `catch {}` が、**Stripe API 障害 / customer 参照なし / tenant 不在**という 3 つの異なる事実をすべて `undefined` に潰していた。呼び出し側 (`handleInvoicePaid` 等) は `if (!tenant) return` で warn 1 行を出して終わるため、Stripe API 障害が事実と異なるログに化ける。さらに #3960 で新設した `stripe-plan-unresolved` alert は context 解決の**成功後**の plan 解決失敗にしか掛からないため、context 解決自体が落ちた場合は alert 経路の手前で return され、**落ちても無通知**になっていた (#3959 と同 class) |
| **検知 method** | (a) Discord alert `stripe-context-unresolved` (#3981 で新規追加、`tags.reason` = `stripe_api_error` \| `repo_error`) (b) CloudWatch Logs Insights `filter kind = "stripe-context-unresolved"` (c) `[STRIPE] subscription context の解決に失敗しました (<Stripe API 障害\|データストア障害>)` の `logger.error` |
| **ロールバック手順** | (1) `tags.reason` で Stripe 側障害か DB 側障害かを切り分け (2) Stripe 側なら status.stripe.com と API key の有効性を確認、DB 側なら DSQL の接続状況を確認 (3) **障害中に到達した event は自然収束しない。手動突合が必須**。`resolveSubscriptionContext` は障害時も例外を投げず `undefined` を返すため handler は正常終了し、`stripe_webhook_events` に `handlerResult='success'` として記録される ([phase5-webhook-idempotency-architecture.md](phase5-webhook-idempotency-architecture.md) の dedup 台帳、#3985)。以後、同一 `event.id` の Stripe 再送は skip され、**Dashboard からの Resend も同一 `event.id` のため skip される**。復旧後は (a) CloudWatch Logs Insights で alert 期間の `subscriptionId` を列挙 → (b) Stripe 上の現行 subscription / price と DB の `plan` / `status` を突合 → (c) 差分があれば手動修復する。恒久対策 (障害時は台帳に記録せず Stripe の再送に載せる) は #4108 で追跡 |
...
| **R12 (#3980 新規)** | **subscription item 複数化で `items.data[0]` が plan を表さなくなる** | Discord alert `stripe-subscription-multi-item` (`tags.priceIds`) | plan item を lookup_key で選別する分岐を追加して deploy + 誤解決 tenant を Stripe 現行 price と突合修復 | `items.length > 1` / `=== 1` 双方の unit test で発火・誤発火を固定 |
| **R13 (#3981 新規)** | **context 解決の bare `catch {}` が障害を「tenant not found」に化けさせる** | Discord alert `stripe-context-unresolved` (`tags.reason`) + `logger.error` | `reason` で Stripe 側 / DB 側を切り分け → 復旧。**再送では収束しない**ため CloudWatch → Stripe / DB 突合で手動修復 (恒久対策 #4108) | try 範囲を分割し bare `catch {}` を不能化 + 3 経路 (API throw / repo throw / tenant 不在) unit test |
