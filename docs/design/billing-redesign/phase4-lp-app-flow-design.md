...
> **注**: `${PLAN_FULL_TERMS.family}` は Phase 1 #2588 で atom 1 行修正 (`'プレミアムプラン'`) すれば自動的に「プレミアムプラン」表示になる (ADR-0045 §3.3 atom 伝播原則)。本書では LP HTML 変更時に Phase 1 #2588 と整合を取る前提。
...
- **本書は docs 設計のみ**、テスト追加はなし。Phase 7 実装時に以下を計画:
  ...
  - **E2E**: `tests/e2e/lp-to-app-flow.spec.ts` (新規) で以下シナリオを Phase 7 実装時に追加:
...
- **ADR**:
  - [ADR-0010](../../decisions/0010-pre-pmf-scope-judgment.md) (Pre-PMF) / [ADR-0012](../../decisions/0012-anti-engagement-principle.md) (Anti-engagement) / [ADR-0013](../../decisions/0013-lp-truth-from-implementation.md) (LP truth) / [ADR-0028](../../decisions/0028-pre-pmf-founder-inquiry-removal.md) (引き止めない) / [ADR-0045](../../decisions/0045-terms-ssot-2-layer.md) (terms.ts 2 階層)
