...
`status: 'pending' | 'building' | 'ready' | 'failed'` + `failureReason: string | null` を追加（`schema.ts` + `types/index.ts` + `cloud-export-repo.interface.ts` + sqlite/dynamodb/demo 3 実装）。DB migration は DSQL 移管（#3433）と同期。既存カラム（id/tenantId/exportType/pinCode/s3Key/fileSizeBytes/label/expiresAt/downloadCount/maxDownloads/createdAt）は不変。
...
### 3.7 実装対象ファイル
`cloud-export-service.ts`（生成分離）/ `storage.interface.ts` + dynamodb/sqlite/demo storage-repo（`getDownloadUrl`）/ `schema.ts` + types + cloud-export-repo 3 実装（status）/ `routes/api/v1/export/cloud/[id]/download/+server.ts`（新設）/ `schedule-registry.ts`（export-build job）/ `infra/lib/compute-stack.ts`（cron rule）。
