...
| **件数連動 CTA** | `selectedCount` (`$derived`) を CTA 文言に反映 (`MARKETPLACE_LABELS.detailCtaImportUnified(count)`、取込 4 type 共通の「このテンプレートを取り込む (N件)」)。`selectedCount === 0` のときは disabled (`MARKETPLACE_LABELS.detailActivityPackSelectedZero` 文言、誤遷移防止) | 同上 |
