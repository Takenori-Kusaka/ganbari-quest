...
**取込 CTA 統一形式** (#2774、User 指摘 #2/#4 根治):

全 5 type (activity-pack / reward-set / checklist / rule-preset bonus / challenge-set) の認証済
取込 CTA は **`<a href="/admin/<page>?import=${itemId}">`** 形式に統一する。`<form action="?/import...">`
+ server action 経由は撤去 (旧 `importRewardSet` / `importChecklist` / `importChallengeSet` action 削除済)。
admin 側 server load は **`?import=<presetId>` query** を一律に読取り、ChildSelectionDialog auto-open する。
...
| challenge-set | `/admin/challenges?import=${itemId}` (旧 `?marketplace-import=` 廃止) | `?import=` | `challenge-set-import-cta` |
| rule-preset exchange | 暫定: marketplace 内 `<form action="?/importRulePreset">` (childId 直渡し) | — | `rule-import-submit` |

**rule-preset exchange のみ Phase 2 で統一予定** (Issue #2774 follow-up): special_rewards table への
per-child 取込を admin/rewards 側 ChildSelectionDialog 経由で受領する mechanism 整備が必要なため、
本 PR では暫定的に旧 form を維持。
...
- marketplace 詳細 `?import=<presetId>` で `/admin/challenges` に遷移 (childId URL に出さず、CWE-598 整合、旧 `?marketplace-import=` 廃止)
...
### 5.2 ChildSelectionDialog auto-open mechanism (5 type 共通の正規取込経路、#2774 で query 統一済)
...
`ChildSelectionDialog` を auto-open する mechanism を保持する。**#2774 で challenges の旧
`?marketplace-import=` query 名を撤去**し、全 5 type で `?import=` に揃った (5 type 取込導線完全統一)。

Marketplace 詳細 → `<a href="/admin/<page>?import=${itemId}">` 直遷移 → admin 画面で
ChildSelectionDialog auto-open → child binding 決定 → server action へ POST、の正規経路 (§3.1) は
本 PR で **server action 経由を撤去し、`<a>` 直遷移に簡素化**した (reward-set / checklist /
challenge-set の旧 marketplace server action は削除済)。
