...
#### [03] コアループ (#1311: 2 系統分離)

- **タイトル**: 2 つの独立サイクルで、毎日の動機を支える
- **構造**: **(A) 活動 × ポイント × レベルアップ** と **(B) ログインボーナス × おみくじ × スタンプカード** の 2 トラック並置 (Finch 一点突破メタファは (A) に集約、(B) は日 1 回 cap の継続リズム)
- **訴求**: (A) ポイント累積 / レベルアップ 長期可視化。(B) 射幸性を排した「ログイン → おみくじ → スタンプ押印 → 週末カード完成」の短尺儀式。両者は独立した仕組みで、「引き続ければもっと貰える」誘導なし (ADR-0012)
- **禁句**: 「シールガチャ」(#1311 で撤回、実装実体は日 1 回 cap omikuji + 週次 stamp card / ADR-0013 LP truth)
...
| 要素 | 配置 | 役割 |
|---|---|---|
| 活動 → ポイント → レベルアップ (トラック A) | [03] コアループ | 活動動機の長期可視化。1 点突破メタファ |
| ログインボーナス → おみくじ → スタンプカード (トラック B) | [03] コアループ | 日 1 回 cap の継続リズム儀式。射幸性なし (#1311 / ADR-0012) |
