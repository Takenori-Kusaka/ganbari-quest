...
## 3.5 LP セクション 魅せ方規範 § (#1789, 2026-05-01)

> **位置付け**: 9 セクションが独立に最適化され「複数チームの寄せ集め」のようにバラバラに見える問題（PO-A-1）を解消するため、PO お気に入り 2 セクション ([02b] age-panel / [05] soft-features) を **規範** として固定し、残り 7 セクションの濃度を一貫させる SSOT。
>
> **後続 Issue (C5/C6/C8/M2 等) の濃度判定基準** として参照される。本 § を更新せずにセクションごとに独自の濃度を採用することは禁止。

### 3.5.1 規範セクション（PO お気に入り 2 セクション）

| 規範 | セクション | 構造 | 情報密度 | scrshot 占有率 |
|------|-----------|------|---------|---------------|
| **規範 A** | [02b] 年齢スイッチャー | 大 scrshot 1 + h3 親主語 1 行 + UI 軸特徴 1 行 + 学年訴求 1 文 + CTA | 低 | 高 (≥40%) |
| **規範 B** | [05] ソフト機能 | 3 cards (featured 1 + 並列 2) + 各 scrshot + h3 + 1 文 | 中 | 中 (30-40%) |

両セクション共通の特性:

1. **主語は親**（「お子さま」「親」「家族」が必ず h3 / 1 行目に登場）
2. **scrshot で機能を語る**（テキスト過多にならず、画像で実態を即座に把握）
3. **ベネフィット行 ≤ 2**（h3 + 1 段落、もしくは 1 文のみ）
4. **ブロック数 ≤ 4**（パネル / カード / 説明 / CTA で完結）

### 3.5.2 4 トーンマップ（全 9 セクション分類）

| トーン | 該当セクション | 濃度 | 役割 |
|--------|---------------|------|------|
| **高度 0**（一点訴求） | [01] hero | high | 2 秒で価値伝達 + CV 導線（Hero は単独タイトル + 1 段落 + scrshot + CTA で完結） |
| **ペルソナ訴求**（規範 A 系） | [02] versus / [02b] age-panel / [05] soft-features / [05b] growth-roadmap | low-mid | 親が共感する場面の言語化（規範 A に揃える） |
| **機構説明**（規範 B 系） | [03] core-loop / [04] machine-tour | mid | 仕組みを 3 カード型で図解（規範 B に揃える、3-4 カード上限） |
| **信頼/法務** | [07] 安心訴求 / [08] FAQ | mid | 裏付け情報（リスト形式、比較表は最小限） |

### 3.5.3 濃度ルーブリック（後続 Issue の AC で使う検証テンプレ）

各セクションは以下の 5 指標が **規範 A or B の ±20% 以内** に収まること。逸脱がある場合は本 § を更新するか、セクション側を圧縮するか、いずれかで整合させる。

| 指標 | 規範 A 値 | 規範 B 値 | 計測方法 |
|------|----------|----------|---------|
| **ベネフィット行数** | ≤ 2 | ≤ 2 | 1 ブロック内の `<p>` / `<li>` 数 |
| **scrshot 占有率** | ≥ 40% | 30-40% | `<picture>` / `<img>` の縦寸 ÷ セクション縦寸 |
| **ブロック数** | ≤ 4 | ≤ 4 | h3 / scrshot / 説明 / CTA の合計 |
| **主語統一** | 親主語 | 親主語 | 1 行目の主語が「お子さま」「親」「家族」「あなた」のいずれか |
| **文字 px 占有率** | 30-50% | 30-50% | テキスト要素の縦寸合計 ÷ セクション縦寸 |

### 3.5.4 規範違反の例（追加禁止）

- **4 階層以上の入れ子**（section → 2 col → grid → card → step → … は破綻）
- **6 ブロック以上の縦積み**（モバイルで認知負荷が破綻、垂直スクロール疲労）
- **scrshot ゼロ**（機構説明セクションは scrshot 必須、テキスト比較表だけのセクションは作らない）
- **緑 sticky note 等の「主導線と並列に浮かぶ」装飾ブロック**（読み手の主導線を分散させる）
- **STEP 1 / STEP 2 のような「内部チャプター」バッジ**（外部読み手にとって内訳構造は不要、結論で語る）

### 3.5.5 適用状況（2026-05-01 #1787-#1789 bundle 時点）

| セクション | 規範整合 | 残課題 |
|-----------|---------|-------|
| [01] hero | OK（高度 0） | — |
| [02] versus | OK（4 行に圧縮済 #1614） | — |
| **[02b] age-panel** | **規範 A** | — |
| [03] core-loop | **本 PR (#1787) で規範 B に整合化**: 4 階層 → 2 階層 / 6 STEP → 3 STEP / 親子両視点バナー削除 / `cls-num` 削除 / aside 緑 note 削除 / 1-shot summary 画像追加 | — |
| [04] machine-tour | OK（3 cards #1708 で圧縮済） | — |
| **[05] soft-features** | **規範 B** | — |
| [05b] growth-roadmap | OK（5 stage、子ベネフィット行を hero or [02b] 統合は M3 で別途検討） | M3 で更に圧縮検討 |
| [06] 料金プロミスバンド | OK（1 バンド） | — |
| [07] 安心訴求 | OK | — |
| [08] FAQ | OK（Top 3 のみ） | — |
| [09] 最終 CTA | OK | — |
...
- **構造**（#1787, 2026-05-01: 4 階層 → 2 階層 / 6 STEP → 3 STEP に圧縮）:
  - section → 3 カード横並び（PC）/ 縦並び（mobile）の **1 階層**
  - 各カード = `バッジ + 短い h3 + 1 段落（2 行以内）+ 1 個の scrshot or 1-shot summary 画像`
  - **STEP 1 / STEP 2 バッジ廃止**: 内訳構造を読み手に強要しない。1 段落の自然文に統合する
  - **親子両視点バナー (`.core-loop-perspectives`) 削除**: hero 直下で同様訴求があり冗長 / [02b] と重複
  - **緑 sticky note (`.core-loop-aside`) 削除**: 主導線と並列に浮かぶ装飾は規範違反 (§3.5.4)
  - 末尾に **1-shot summary 画像** (`/static/assets/lp/core-loop-summary.png`) を中央配置し「活動 → 習慣 → ごほうび」の循環を 1 枚で示す
- **3 カード内訳**:
  1. **活動** (`data-layer="l1"`): 「はみがきした」「宿題おわった」を 2 タップで記録 → ポイント獲得（1 段落）
  2. **習慣** (`data-layer="l2"`): 1 日 1 回までのおみくじスタンプで、自然に毎日記録する習慣が身につき、スタンプカードが週末に完成（1 段落）
  3. **ごほうび** (`data-layer="l3"`): ポイントを蓄積し、ごほうびショップが唯一の出口で実物・お小遣い・特権と交換（1 段落）
- **訴求方針** (規範 B 整合): 親主語の h3 + 1 段落で機構を説明、子供視点は scrshot で示す
- **ごほうびショップ唯一の出口**: ポイントは「欲しいものと交換できる経済通貨」として機能し、子供の自律的目標設定を促す
- **禁句**: 「シールガチャ」(#1311 で撤回)、「UR を引く」等の射幸心文言 (ADR-0012 / ADR-0013)
- **画像アセット** (#1787):
  - 1-shot summary 画像 `static/assets/lp/core-loop-summary.png` を新規生成（Gemini A-1 ブランドスタイル使用、kawaii chibi、3 アイコン循環図、透過 PNG 推奨だが背景アート扱いで非透過も可）
  - 各カード内 scrshot は既存 `screenshots/feature-point-level{,-desktop}.webp` 等を再利用
- **PC レイアウト**:
  - 1024px+ で `grid-template-columns: repeat(3, 1fr); gap: 16px; max-width: 1080px` の 3 カード並列
  - mobile (<1024px) では 1 列縦積み (`grid-template-columns: 1fr`)
  - 1-shot summary 画像はカード行の下に独立配置、`max-width: 480px; margin: 24px auto`
