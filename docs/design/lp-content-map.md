...
価値の階段化 (BusyKid 方式)。4 幕が「即効 → 毎日 → 長期 → 冒険」の時間軸を示す。

1. **即効の楽しみ**: おみくじ（ログイン時の小さなワクワク）
2. **長期の達成感**: 実績 + 称号（はじめのぼうけんしゃ → でんせつのゆうしゃ）
3. **習慣エンジン**: コンボ + チェックリスト（連続達成で倍率アップ）
4. **冒険のクライマックス**: RPG バトル（#605 実装済み範囲）
...
**設計原則**: 「即効 → 毎日 → 長期 → 冒険」の 4 幕構成。1 メタファ一点突破 (Finch) と価値階段化 (BusyKid) を組合せ。
...
| チェックリスト (持ち物/ルーティン) | [04]-③ 習慣エンジン | 毎日の必須 |
