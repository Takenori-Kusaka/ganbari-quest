...
| 04 | machine-tour | 規範 B | mid | 整合 (2 cards、責務は補足機構) | [03] と重複しない補足機構のみ |
...
- **CSS スタイル**: `#guide` は `padding:24px 16px` / PC (≥1024px) で `padding:32px 16px`。`.guide-grid` は PC (≥768px) で `1.1fr 1fr` 2 カラム、`.guide-shot` は `aspect-ratio:16/10` / max-height 280-380px。`.guide-point` は brand 色のグラデ背景 + brand-200 border
...
- **vc-digital scrshot 配置 (#1784)**: 各 4 row の `.vc-digital` 内にテキスト直下で `.versus-shot` (`<picture>` + WebP) を縦積み配置し、各行で別々のアプリ画面 scrshot を表示する。撮影元の対応:
  ...
  `.versus-shot` は `aspect-ratio: 4/3` 固定 + `max-height: 96/140/180px` (mobile/default/desktop) で行高さを揃え、新規撮影はせず既存 9 種から流用する（ADR-0013 LP truth）。ラベルは `LP_VERSUS_LABELS.row{1..4}ShotAlt` (`alt` 属性) を `data-lp-key-attr="alt:versus.row*ShotAlt"` で注入
...
| **(ii) ステータス装飾** | `💳` クレカ不要 / `🚫` 広告なし / `🔄` いつでも解約 等 hero trust badges | **保持**: 不安解消バッジ等の小型ステータス表現。`#trust` セクション 4 個は SVG 化済 (#1796、`docs/design/asset-catalog.md` §「trust-* SVG 一覧」) |
...
#### 保持されたもの（8 件、#1796 で trust 4 badges を SVG 化したため 12 → 8 に）

hamburger `☰` (1) + hero trust badges `💳` `🚫` `🔄` (3) + 準備モード注釈 `👶` (1) + 親子視点 `🧑‍💻` `🧒` (2) + lightbox close `✕` (1) = 計 8 件

`#trust` 4 badges (`🚫` `👪` `🔑` `🔍`) は OS 依存 (iOS Safari と Android Chrome で見た目が大きく異なる) を解消するため SVG (`static/assets/ui/trust-*.svg`) に置換済（#1796 R-MAJ-6、`docs/DESIGN.md` §7「OS/ブラウザ間で見た目が変わると困る要素」整合）
