...
- **4 行内訳**（#1723 R10 で各行のアイコン絵文字を廃止）:

| 行 | analog 側（事実のみ短文） | digital 側 タイトル + 1 文補足 |
|----|---------------------------|--------------------------------|
...
3. **冒険のクライマックス** (`data-testid="feature-rpg-battle"`): RPG バトル（#605 実装済み範囲）— daily trio (stamp card / omikuji / battle) の 3 つ目。elementary+ 全年齢共通の累積努力可視化儀式。upper 代表機能ではない (§2.1 参照)
...
| **(ii) ステータス装飾** | `💳` クレカ不要 / `🚫` 広告なし / `🔑` カギ付き 等 trust badges | **保持**: 不安解消バッジ等の小型ステータス表現 |
...
hamburger `☰` (1) + hero trust badges `💳` `🚫` `🔄` (3) + 準備モード注釈 `👶` (1) + 親子視点 `🧑‍💻` `🧒` (2) + trust 4 badges `🚫` `👪` `🔑` `🔍` (4) + lightbox close `✕` (1) = 計 12 件
