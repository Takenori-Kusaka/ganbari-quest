...
| data-stage | 年齢ラベル | h3（stage タイトル） | 訴求 |
|-----------|-----------|--------------------|------|
| `preschool` | 幼児 3-5 歳 | はみがき・おかたづけを自分で | 大ボタンとひらがな UI で「自分で押した！」の達成感を毎日体験 |
| `elementary` | 小学生 6-12 歳 | 宿題・お手伝いを自分から | 漢字 UI に切替、称号で「次は何を達成しよう？」と自分で目標を立てる力が育つ |
| `junior` | 中学生 13-15 歳 | 習い事・部活と勉強の両立 | 月次レポートで「自分のペース」を客観視し、自律的なリズム調整 |
| `senior` | 高校生 16-18 歳 | 受験・将来設計を自分の手で | 15 年分の活動ログが「自分はこれだけやってきた」という自信に |
| `graduate` | そして **卒業** | アプリが必要なくなる日へ | 「使わなくなる」ことが私たちの成功。15 年分の記録は書き出して家族の手元に残せる |

- **ラベル管理**: 全 H3 / 訴求文 / 年齢ラベルは `LP_GROWTH_ROADMAP_LABELS`（labels.ts）→ `shared-labels.js` 経由で `data-lp-key="growthRoadmap.*"` で注入（並行実装チェックリストに準拠）
