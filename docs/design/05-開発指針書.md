...
開発チーム（Claude Code）が従うべき技術方針・規約を定める。CLAUDE.md の詳細版として位置づける。
...
- チケット単位でPRを作成
- Claude Code が作成した PR は QM が承認する（作成者 ≠ 承認者、ADR-0022）
- マージ前にビルド＋テスト通過を確認
...
| 入力検証 | 全 API でスキーマバリデーション必須（ライブラリは `package.json` が SSOT） |
...
| Prod | NUCサーバー本番 | http://`<NUC_HOST>`:3000 (家庭内 LAN) |

### NUC 接続情報の管理 (SSOT、#2987)

NUC の実 LAN IP / SSH ユーザー名は **public repo にコミットしない** (CWE-200 / SSH user brute-force の的)。本設計書を含む全 docs / runbook / script は以下の規約で統一する:

| 表記 | 意味 | 実値の管理場所 |
|------|------|---------------|
| `<NUC_HOST>` | NUC の LAN IP or hostname | NUC 運用者の `.env.local` (`NUC_SSH_HOST`)、コミット禁止 |
| `<NUC_USER>` | NUC の SSH ユーザー名 | 同上 (`NUC_SSH_USER`) |

- script (`scripts/deploy.sh` / `scripts/setup-server.sh`) は env 経由 (`NUC_SSH_HOST` / `NUC_SSH_USER` / `NUC_HOST`) でのみ実値を受け取り、未設定時は即エラー終了する (ADR-0024 silent fallback 禁止)。env key の定義は `.env.example` §「NUC SSH 接続情報」参照
- **再混入の CI 検出は無い（#4420）**: `scripts/check-internal-terms.mjs`（conn-info group）は #4322 で削除済み。レビューで担保する
