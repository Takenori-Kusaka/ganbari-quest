...
| UT-LOG-02 | 活動記録キャンセル（5秒以内） | logId, 記録後3秒 | ログ削除、ポイント返還 |
| UT-LOG-03 | 活動記録キャンセル（5秒超過） | logId, 記録後6秒 | CANCEL_EXPIRED エラー |
| UT-LOG-04 | 活動ログ取得（週間） | childId=1, period="week" | 今週のログとサマリー |
| UT-LOG-05 | 活動ログ取得（月間） | childId=1, period="month" | 今月のログとサマリー |
| UT-LOG-06 | 活動ログ取得（カテゴリ別集計） | childId=1 | byCategory に正しい集計値 |
| UT-LOG-07 | 活動ログ取得（空） | childId=1, 記録なし | 空のログリスト、集計値すべて0 |
...
| TC-ID | エンドポイント | テストケース | 期待ステータス |
|-------|-------------|------------|--------------|
| API-LOG-01 | POST /api/v1/activity-logs | 活動記録（正常） | 201 |
| API-LOG-02 | POST /api/v1/activity-logs | バリデーションエラー | 400 |
| API-LOG-03 | DELETE /api/v1/activity-logs/1 | キャンセル（5秒以内） | 200 |
| API-LOG-04 | DELETE /api/v1/activity-logs/1 | キャンセル（期限超過） | 400 |
| API-LOG-05 | GET /api/v1/activity-logs?childId=1 | ログ取得 | 200 |
| API-LOG-06 | GET /api/v1/activity-logs | childId未指定 | 400 |
