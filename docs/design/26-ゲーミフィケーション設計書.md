...
「がんばりクエスト」のゲーミフィケーション設計は、以下の問いから出発した:

> 子供が「やりなさい」と言われなくても自分から活動を記録したくなるには何が必要か？

既存の子供向けアプリは以下の二極に分かれていた:

| アプローチ | 問題点 |
|-----------|--------|
| 単純ポイント制（スタンプ帳） | 短期的な動機付けにしかならない。飽きる |
| SNS 型ランキング・比較 | 競争による自己否定感・保護者の不安 |
| 過剰な通知・連続ガチャ | Anti-engagement 原則違反（ADR-0012）。アプリ依存を促進する |

この問題を解決するために、2026-04-09 に「コアループ L1/L2/L3 モデル」と「Anti-engagement 原則に準拠したゲーミフィケーション」を設計した（#1328 B6 ブレスト、`docs/rationale/02-core-loop-rationale.md` 参照）。

#### コアループ 3 層モデル（L1/L2/L3）

| 層 | 名称 | 役割 | 代表機能 |
|----|------|------|---------|
| L1 | アクション層 | 活動記録という日常行動 | 活動記録（2タップ）、ポイント獲得 |
| L2 | メタ習慣層 | L1 行動の継続動機付け | おみくじスタンプ、ストリーク、ウィークリーチャレンジ |
| L3 | 長期成長層 | 長期的アイデンティティ | レベルアップ、実績、キャラクター成長、がんばり証明書 |
...
本設計書の §2 に詳細な設計原則を定義している。実装時は §2 を参照すること。
