...
`src/lib/marketplace/export-schema.ts` で定義される 5 type 横断の統一 export envelope。`/api/v1/activities/export` の旧 v1 (formatVersion='1.0') と並行して、新しい admin marketplace export 経路 (#2362) で採用される。
...
- **後方互換**: 旧 v1 (`formatVersion: '1.0'`、`/api/v1/activities/export` 出力) は `migrateV1ActivityPackToV2()` で v2 envelope に変換可能。`parseAnyExportEnvelope()` は v1 / v2 を自動判別
