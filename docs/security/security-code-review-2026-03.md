...
**分類**: Stripe PCI DSS SAQ A コンプライアンス + アプリセキュリティ
...
本アプリは家庭向けゲーミフィケーション Web アプリであり、以下の脅威アクターを想定する:

- **外部攻撃者**: インターネット経由での不正アクセス、決済データの窃取
- **悪意あるテナントユーザーー**: テナント間データ漏洩、権限昇格
...
- 攻撃者が悪意のあるサイトからユーザーーのブラウザを介して、ログイン中のセッションで任意のアクションを実行可能
...
- Context Token はサーバーーサイドのみで使用され、Cookie 経由で送受信される
...
- OTP 検証の信頼性が低下し、ユーザーーがログインできない可能性
- セキュリティ上は「検証が緩くなる」方向のリスクではないが、ユーザーー体験と認証フローの信頼性に影響
...
**注記**: CloudFront の `ResponseHeadersPolicy.SECURITY_HEADERS` がデフォルトの CSP を提供する可能性があるが、アプリ固有の CSP ポリシーを明示的に設定すべきである。
...
- session_id の検証がサーバーー側で行われていることを確認済み
...
開発用のダミーユーザーー認証情報がソースコードにハードコードされている:
...
Identity (誰であるか) と Context (何として操作しているか) を分離した二層セッションモデルが採用されている。Context Token は HMAC-SHA256 で署名され、サーバーーサイドで検証される。この設計により:

- テナント ID がサーバーー側で制御され、クライアントからの改ざんが不可能
- ロール (owner/parent/child) がサーバーー側で検証される
...
| 条件 | 本アプリの状況 | 適合 |
|------|--------------|------|
| カード情報を直接処理・保存・送信しない | Stripe Checkout (ホスト型決済ページ) を使用。カード情報はアプリに一切触れない | OK |
| 決済処理は PCI DSS 準拠の第三者に完全に委託 | Stripe が PCI DSS Level 1 Service Provider として認定済み | OK |
| アプリ内にカード入力フォームがない | Stripe Checkout のリダイレクト方式を使用 | OK |
...
| 8.1 | ユーザーーの識別と認証 | Cognito Email/Password + MFA (TOTP) + Email OTP。アカウントロックアウト実装済み | OK |
| 8.1.6 | 6回以下のアクセス試行でアカウントロック | 10回でロック（推奨は6回以下） | 要検討 |
| 9 | カード会員データへの物理的アクセス制限 | AWS Lambda (サーバーーレス) のため物理アクセス制御は AWS 側で担保 | OK |
...
| チェック項目 | 結果 |
|------------|------|
| Stripe Secret Key はサーバーーサイドのみで使用 | OK: `$lib/server/stripe/client.ts` で環境変数から取得 |
| Secret Key がクライアントに露出しない | OK: `+server.ts` 内のみで使用 |
| Webhook 署名検証の実装 | OK: `stripe.webhooks.constructEvent()` 使用 |
| Checkout Session の tenantId はサーバーー側で設定 | OK: `requireTenantId(locals)` から取得 |
| Customer Portal の tenantId はサーバーー側で設定 | OK: `requireTenantId(locals)` から取得 |
...
2. **Stripe 統合**: SAQ A レベルに適した Checkout Session 方式で、サーバーーサイドでの署名検証が正しく実装されている
3. **アカウントロックアウト**: DynamoDB ベースで Lambda 分散環境に対応した永続化実装
4. **テナント分離**: tenantId がサーバーー側の署名付きトークンから取得され、改ざん不可能
...
本アプリの Stripe 統合は SAQ A の技術的要件を概ね満たしているが、以下の修正が SAQ A 適合の前提条件となる:
...
| WAF | Web Application Firewall. Web アプリ向けのファイアウォール |
