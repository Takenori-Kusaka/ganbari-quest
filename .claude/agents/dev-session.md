...
Fix Agent / Dev Agent が `scripts/check-recent-deploy-deletion.mjs` 等の machine-verify gate を実行する前、**worktree HEAD = PR HEAD 一致を必須 verify**。空 worktree (HEAD = origin/main) での gate 実行は偽陽性原因 (#2613 Round 3 で legal critical 506 行喪失寸前まで進んだ実害観察):
...
if [ "$PR_HEAD" != "$WORKTREE_HEAD" ]; then
  ...
  echo "      gate 実行前に明示 checkout 必須"
  git checkout $PR_HEAD || exit 1
fi
node scripts/check-recent-deploy-deletion.mjs --pr <N>   # 真正実行
```

由来: PR #2613 Fix Round 3 で「`check-recent-deploy-deletion.mjs` exit 0 GATE PASS」と Fix Agent が完了報告したが、QM Re-Review #3 が PR HEAD 明示 checkout で再実行したところ exit 2 (BLOCK) = 7 件目 rebase drift 検出。legal critical 506 行喪失寸前で阻止 (#2618 / ADR-0056 §D)。
