...
Dev セッションと QM セッションが**事業的に正しい行動をし続ける**ための、十分な意思入れと誰が読んでも同じ理解ができる Issue を作成してください。
...
4. **SSOT namespace 重複確認 (#2061、ADR-0045)** — Issue body 内に `XXX_LABELS` / `XXX_TERMS` 形式の namespace 名が含まれる場合、`src/lib/domain/terms.ts` / labels 層 (`src/lib/domain/labels/`) と open Issue を `grep -rn` で確認し scope 重複を避ける。衝突時は scope 統合 / 命名衝突回避を選択（PR #2041 / PR #2044 で `LP_FAQ_TERMS` 重複起票 → TypeScript duplicate identifier conflict 発生の前例）
5. **マーケ/Growth 視点** — サインアップ目標への貢献度は？
6. **法務/コンプライアンス視点** — 法的リスクはないか？
7. **財務視点** — AWS コスト影響は？事業計画の原価枠内か？
8. **仮想顧客レビュー** — ペルソナはこの変更を歓迎するか？
...
- `tmp/issue-bodies/<slug>.md` への Issue body draft 保存（`gh issue create --body-file` の前段、Issue #1172 で HEREDOC が禁止されているため物理的に必須）
- `tmp/pr-bodies/<slug>.md` への PR body draft 保存（`gh pr create / gh pr edit --body-file` 時）

これらは findings / analysis / summary の report file **ではなく** GitHub Issue / PR 作成の前段一時ファイル。
`tmp/` は `.gitignore` 配下なのでリポジトリ汚染は発生しない。**起票 / PR 作成完了後は速やかに削除すること**:

```bash
rm tmp/issue-bodies/<slug>.md
```

**Write tool が拒否された場合のフォールバック**: `cat > tmp/issue-bodies/<slug>.md << 'EOF' ... EOF`。

詳細手順は [Skill: issue-triage SSOT](../skills/issue-triage/SKILL.md) §「`--body-file` 運用」を参照 (#2089)。
