...
- [ ] ナビ構造が desktop / mobile で異なる（`AdminLayout` に両方が同居 / 子供は `BottomNav` / 設定サブナビ / 運営者ナビ）— `grep -rn "<nav\b" src/` で面を洗い出し、横スクロールする面（`ops/+layout.svelte` は `nowrap` + `overflow-x: auto`）を含めて全部見る
...
- [ ] 用語のゆれ（`terms.ts` / labels 層 SSOT からの逸脱。「解約」/「退会」/「キャンセル」の混同）
