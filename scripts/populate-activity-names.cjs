...
console.log('\n=== Phase 1: Merge pairs ===');
const updateKeep = db.prepare('UPDATE activities SET age_min = ?, age_max = ?, name_kanji = ? WHERE id = ?');
const hideActivity = db.prepare('UPDATE activities SET is_visible = 0 WHERE id = ?');
const migrateLogsStmt = db.prepare('UPDATE activity_logs SET activity_id = ? WHERE activity_id = ?');
...
for (const [keepId, hideId, ageMin, ageMax, nameKanji] of mergePairs) {
	...
	const logCount = db.prepare('SELECT COUNT(*) as cnt FROM activity_logs WHERE activity_id = ?').get(hideId);
	if (logCount.cnt > 0) {
	...
	console.log(`  MERGED: id=${keepId} "${keep.name}" (→${ageMin}-${ageMax}, kanji="${nameKanji}") | HIDDEN: id=${hideId} "${hide.name}"`);
}
...
const visibleActivities = db.prepare('SELECT COUNT(*) as cnt FROM activities WHERE is_visible = 1').get();
const withKana = db.prepare('SELECT COUNT(*) as cnt FROM activities WHERE name_kana IS NOT NULL').get();
const withKanji = db.prepare('SELECT COUNT(*) as cnt FROM activities WHERE name_kanji IS NOT NULL').get();
