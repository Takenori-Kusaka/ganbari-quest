...
const combos = db.prepare(`
  ...
  LIMIT 20
`).all();
combos.forEach(c => console.log(`${c.created_at} | ${c.description} (+${c.amount})`));
console.log('コンボ総件数: ' + db.prepare("SELECT COUNT(*) as cnt FROM point_ledger WHERE type = 'combo_bonus'").get().cnt);

console.log('\n=== コンボ種別ごとの発生回数 ===');
const comboTypes = db.prepare(`
  SELECT
    CASE
  ...
  ORDER BY cnt DESC
`).all();
comboTypes.forEach(c => console.log(`${c.combo_type}: ${c.cnt}回 (合計${c.total_points}P)`));
...
const achs = db.prepare(`
  ...
  LIMIT 30
`).all();
achs.forEach(a => console.log(`${a.unlocked_at} | ${a.name}${a.milestone_value ? ' (M=' + a.milestone_value + ')' : ''}`));
console.log('実績総件数: ' + db.prepare('SELECT COUNT(*) as cnt FROM child_achievements').get().cnt);

console.log('\n=== 実績コード別の解除状況 ===');
const achCodes = db.prepare(`
...
`).all();
achCodes.forEach(a => console.log(`${a.code} (${a.name}): ${a.unlock_count}回解除`));
...
`).get();
if (missions.total > 0) {
  console.log(`総ミッション数: ${missions.total}, 達成数: ${missions.completed}, 達成率: ${Math.round(missions.completed / missions.total * 100)}%`);
} else {
...
const kids = db.prepare(`
  ...
  GROUP BY c.id
`).all();
kids.forEach(k => console.log(`${k.nickname} (${k.age}歳): ${k.total_records}回記録, ${k.active_days}日間`));
...
const dailyAvg = db.prepare(`
  ...
  GROUP BY c.id
`).all();
dailyAvg.forEach(d => console.log(`${d.nickname}: ${d.avg_daily}回/日`));
