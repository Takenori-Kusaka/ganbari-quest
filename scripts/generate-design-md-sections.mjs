...
function extractSemanticTokens() {
  ...
  while ((match = semanticPattern.exec(css)) !== null) {
    ...
    else if (name.startsWith('--color-surface')) categories['Surface（背景）'].push({ name, value });
    ...
    else if (name.startsWith('--color-feedback')) categories['Feedback（フィードバック）'].push({ name, value });
...
function extractLabels() {
  ...
  let match;
  while ((match = constPattern.exec(src)) !== null) {
