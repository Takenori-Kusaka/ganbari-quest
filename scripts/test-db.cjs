...
console.log('3. Read test:', log ? 'OK' : 'FAIL', '- child_id:', log?.child_id, 'points:', log?.points);
...
// 5. Balance check
const balance = db.prepare('SELECT SUM(amount) as total FROM point_ledger WHERE child_id = 1').get();
...
// 7. Table counts
const tables = ['children', 'activities', 'achievements', 'settings', 'statuses', 'market_benchmarks'];
