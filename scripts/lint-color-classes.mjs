...
const hexArbitraryPattern = new RegExp(
  `(?:${PREFIXES.join("|")})-\\[#[0-9a-fA-F]{3,8}\\]`,
  "g",
);
...
console.log(
  "注: @theme でブランドカラーに上書き済みのため既存は動作しますが、",
);
console.log(
  "   新規コードでは CSS 変数 (var(--color-*)) の直接使用を推奨します。\n",
);
...
for (const { file, items } of violations) {
  ...
  for (const item of items) {
    const icon =
      item.type === "hex-arbitrary" || item.type === "style-hex" ? "🔴" : "🟡";
...
const hardErrors = violations.reduce(
  (sum, v) =>
    sum +
    v.items.filter(
      (i) => i.type === "hex-arbitrary" || i.type === "style-hex",
    ).length,
  0,
);
