#!/usr/bin/env node
/**
 * scripts/check-dynamodb-stub.mjs (#1012)
 *
 ...
 *   PO 方針は「main マージ = 即本番デプロイ = 即顧客提供」で段階的対応は禁止。
 *
 * 検出パターン:
 ...
 * #1012 導入時点で既に本番稼働している stub。follow-up Issue で順次 eradicate する。
 * 新規追加は厳禁。このリストにファイルを増やす PR は自動で reject すること。
 *
 * Format: 'filename.ts' → follow-up Issue 番号 (1013+)
 */
...
const NOT_IMPLEMENTED_PATTERN = /throw\s+new\s+Error\(\s*['"`](not\s+implemented|未実装|unimplemented)/i;
...
function main() {
	...
	console.error(
		'\nPO 方針: main マージ = 即本番デプロイ = 即顧客提供。段階的対応は禁止 (#1012)。\n' +
			'interface を追加した PR で SQLite + DynamoDB 両実装を完成させてください。\n' +
			'詳細: docs/sessions/dev-session.md 「段階的リリース禁止」セクション',
	);
	process.exit(1);
}
