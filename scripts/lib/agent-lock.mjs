 ...
 * 本 module が唯一の書き手であるという前提は保証されていないので、JSDoc の cast
 * (`/** @type {LockHolder} *\/`) は runtime の保証にならない (#4013 / QM 指摘)。
 * `ownerVia` / `chain` を「後から検証できる証跡」として載せる以上、その土台を
 * 未検証のままにはできないため、読み出し時に 1 枚挟む。
 *
