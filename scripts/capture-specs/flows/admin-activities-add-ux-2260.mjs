...
async function waitForMenuOpen(page, triggerTestId) {
	...
	// menu animation 完了待ち (data-state="open" → positioner 配置 → ARIA contract 安定の最終段階)
	await page
		.locator('[data-part="content"][data-state="open"]')
		.first()
		.waitFor({ state: 'attached', timeout: 3_000 })
		.catch(() => {});
	// requestAnimationFrame 2 回で transition / layout を確実に commit
	await page.evaluate(
		() =>
			new Promise((resolve) =>
...
export default async (page, capture) => {
	...
	// close menu (Esc + waitFor closed state via aria-expanded)
	await page.keyboard.press('Escape');
	await page.waitForFunction(
		() =>
			document
				.querySelector('[data-testid="header-add-activity-btn"]')
	...
	await page.waitForFunction(
		() =>
			document
				.querySelector('[data-testid="header-overflow-menu-btn"]')
				?.getAttribute('aria-expanded') === 'false',
		undefined,
		{ timeout: 3_000 },
	);
	...
	if ((await searchInput.count()) > 0) {
		...
		// filter 適用後 ActivityEmptyState (filteredText) が表示されるのを待つ
		// (テキスト「一致する活動はありません」相当の data-testid は無いため、
		//  filter 結果が DOM に反映されるまで activity item を 0 件になるまで待つ)
		await page
			.waitForFunction(
				() =>
					document.querySelectorAll('[data-tutorial="activity-list"] [data-testid^="activity-"]')
						.length === 0,
				undefined,
				{ timeout: 3_000 },
			)
