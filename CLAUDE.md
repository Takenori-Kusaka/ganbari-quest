...
- UI/デザイン変更後は、**必ず対象画面のスクリーンショットを撮影**してユーザーに提示すること。コードの変更だけで Done にしない。
- LP（`site/`）と SvelteKit アプリ（`src/`）は**別のコードベース**。一方を修正しても他方には反映されない。両方に影響するロゴ・favicon・スタイル変更は、両方のファイルを確認すること。
- ゴールに「目視確認」がある場合、実際にブラウザで確認したうえでチェックを付けること。未検証のチェックは虚偽報告と同等。
