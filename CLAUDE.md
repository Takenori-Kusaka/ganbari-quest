...
- `npm run build` — 本体ビルド
- `npm run build-storybook -- --quiet` — Storybook ビルド (`ci.yml` の `stories` filter が true のときのみ。filter の実体は `ci.yml` が SSOT で、ここには列挙しない。story / `.storybook/` 本体だけでなく、コンパイル結果を変えうる設定・依存も含む = #4859)
