...
**デザインシステムの SSOT は `docs/DESIGN.md`。** 色・コンポーネント・用語・禁忌事項の全ルールはそちらを参照。

要点: 3層トークン（hex禁止→Semantic CSS変数必須）、プリミティブコンポーネント必須、lint で自動検出（`npm run lint`）。
...
**用語辞書の SSOT は `src/lib/domain/labels.ts`。** 詳細は `docs/DESIGN.md` §6 を参照。

要点: UIラベル・機能名はすべて用語辞書から参照。ハードコード禁止。変更時は `grep` で全件確認。
...
- **`vite.config.ts` のカバレッジ閾値（thresholds）を引き下げない。** CI が自動拒否する（`scripts/check-coverage-threshold.js`）。閾値はラチェット方式で引き上げのみ許可。
- **E2E テストで `clearDialogGhosts` を新規使用しない。** アプリ側のダイアログバグを隠蔽するため。`tests/e2e/helpers.ts` 内の既存使用のみ許容。
