...
- [ ] 年齢帯で言い回しが変わる UI 文言を含む場合、ADR-0015 のアンチパターン A1-A7 を確認（`if (uiMode === 'baby')` 散在 / Feature Flag で segment 代替 / runtime 変換ロジック等を避ける）
...
## LP / 販促文言変更時の PR body 記載ルール（ADR-0013 / #1314）

LP (`site/**`)、pricing page、`plan-features.ts`、`pricing-strategy.md` などの販促文言を
**変更・追加する PR** は、以下を PR body に必ず記載すること。

### 必須記載事項

1. **実装コードパス**: 変更した文言が描写する mechanic の実装ファイル・関数名
2. **Committed/Aspirational 区分**: ADR-0013 の区分（Committed = 実装済み、Aspirational = 未実装の将来計画）
3. PR template の `### LP / 販促文言変更時の実装パス明示（ADR-0013）` セクションを埋める

### 禁止事項

- **Aspirational 機能を LP に新規追加・変更することは禁止**（`docs/design/19-プライシング戦略書.md` の Committed/Aspirational 附則で管理）
- `ガチャ`・`抽選`・`コンプリート` 等のギャンブル語彙の使用（`scripts/measure-lp-dimensions.mjs` FORBIDDEN_TERMS、CI が自動拒否）

### 確認手順

```bash
# 禁止語チェック（LP 変更後に必ず実行）
node scripts/measure-lp-dimensions.mjs

# Committed/Aspirational 区分確認
# docs/design/19-プライシング戦略書.md「附則: Committed / Aspirational 機能区分」を参照
```

詳細: [ADR-0013](../docs/decisions/0013-lp-truth-from-implementation.md)
