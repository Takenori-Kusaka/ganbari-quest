...
Issue は仕様書。品質低下が実装品質低下に直結。起票前必須チェック:

- 根本原因特定（症状でなく原因）/ 同一領域の過去 Issue 確認（過去修正がなぜ不十分だったか）
- 解決策は 1 つに絞る（「A または B」併記禁止）/ 再発問題はスクラップ&ビルド前提
- AC に全境界条件列挙 / 設計上の制約明記 / 年齢帯 variant は ADR-0015 アンチパターン A1-A7 確認
