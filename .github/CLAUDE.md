...
- **base branch は develop 二層**（`docs/sessions/branch-strategy.md` §3/§5、#2870 cutover / #2959）: feature/fix/docs PR = `--base develop` 必須 / hotfix（`fix/*` from main）のみ `--base main`。`--base` 省略は default branch（main）向けになり `main-pr-base-guard` で fail する。base 判定 SSOT: `node scripts/lib/resolve-base-branch.mjs`
...
| integration（統合 PR） | per-PR AC マップの代わりに「マージ判定エビデンス表」section + 4 列（含有 PR / 領域 / テスト / 結果）+「残 NG 0 件」明示（audit-team.md §3.5） | 統合 PR 用 section の `- [ ]` 全消化（section 名は env `MERGE_GATE_INTEGRATION_SECTIONS` で差替可能、Phase B #2871 で確定） |
