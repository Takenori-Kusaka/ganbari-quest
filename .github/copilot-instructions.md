...
- **ADR-0045**: [terms.ts SSOT 2 階層化原則 (atom / compound 責務分離)](../docs/decisions/0045-terms-ssot-2-layer.md) — ADR-0009 を supersede (2026-05-07)。SSOT 内部を atom (`terms.ts`) / compound (`labels.ts`) の 2 階層に分離。プラン名 / 年齢モード名 / 機能名は `src/lib/domain/labels.ts` 経由 (LP は `site/shared-labels.js` 経由)。用語変更 1 行修正で全コンテンツ伝播。BANNED_TERMS を CI (`scripts/check-banned-terms.mjs`) で自動検出
...
- **ADR-0011**: [0-2 歳 baby モードは「親の準備モード」](../docs/decisions/0011-baby-mode-as-parent-preparation.md) — コアターゲットを 3-18 歳 (preschool / elementary / junior / senior) の 4 段階に再定義。baby は子供向けゲーミフィケーション UI 非適用、親の準備モードとして残す。LP / 企画書の「5 段階」表記は「4 段階コア + 準備モード」に改める (#1299)
...
- **ADR-0013**: [LP 文言は実装の事実を SSOT とする](../docs/decisions/0013-lp-truth-from-implementation.md) — `site/**` / `plan-features.ts` / pricing-strategy.md は Committed (実装済) / Aspirational (目標、内部限定) を明示分離。LP 文言変更 PR はコードパス明示必須。retrofit design doc 禁止 (#1310)
- **ADR-0017**: [Cognito User Pool 再作成による email mutable 化](../docs/decisions/0017-cognito-pool-recreation-email-mutable.md) — **rejected** (2026-04-21 deploy で CloudFormation が `Mutable` 変更を Replacement ではなく in-place Update として扱い `UPDATE_ROLLBACK_FAILED` 発生)。superseded by ADR-0018
- **ADR-0018**: [Cognito User Pool 論理 ID 変更による明示的 Replacement](../docs/decisions/0018-cognito-user-pool-logical-id-replacement.md) — 論理 ID `UserPool` → `UserPoolV2` に変更し CloudFormation に Replacement を強制。Google OAuth 再認証エラー (#1366) を根本解決。旧 Pool は `removalPolicy: RETAIN` で orphan 化 → deploy 成功後に手動削除。Pre-PMF で既存ユーザー全消失を許容
- **ADR-0019**: [CDK Replacement Detection Gate](../docs/decisions/0019-cdk-replacement-detection-gate.md) — CDK deploy 前に Replacement リソースを自動検出して PR コメントで警告するゲートを導入
- **ADR-0020**: [NUC スケジューラ方式選定](../docs/decisions/0020-nuc-scheduler-choice.md) — node-cron + 専用コンテナ採用 (crond/bree/systemd-timer と比較)。schedule-registry.ts を SSOT として HTTP POST でエンドポイントを呼び出す (#1375, 2026-04-24)
- **ADR-0021**: [Cognito Pool 移行におけるユーザー保全戦略](../docs/decisions/0021-cognito-pool-migration-user-preservation.md) — email を natural key として DynamoDB 変更不要。export-users.mjs / import-users.mjs + migration runbook (#1399, 2026-04-24)
- **ADR-0022**: [admin bypass 禁止と ganbariquestsupport-lab QM Approve 体制の確立](../docs/decisions/0022-admin-bypass-disable-qm-approve.md) — Branch Ruleset の `bypass_actors: []` で admin merge を完全禁止。QA 専用アカウント `ganbariquestsupport-lab` が approve → squash merge する通常フローで運用 (#1481, 2026-04-25)
- **ADR-0024**: [インフラ PR 必須要件: ENV silent skip 禁止 + secrets validation + post-deploy smoke test + alarm](../docs/decisions/0024-infra-pr-required-baseline.md) — ADR-0006 の CDK 適用版。tryGetContext で取った必須 env は throw で assert / deploy.yml に validate-required-secrets step / 新規 Lambda は post-deploy smoke test / scheduled Lambda は CloudWatch Alarm 必須 (#1586, 2026-04-27)
- **ADR-0053**: [LP visual regression: pixelmatch (OSS 6 件比較)](../docs/decisions/0053-lp-visual-regression-pixelmatch.md) — LP (`site/**`) の visual regression を pixelmatch + pngjs + sharp で構造的検出。`scripts/check-lp-visual-regression.mjs` (305 行) が `scripts/lp-screenshot-baseline/*.webp` (git tracked) と CI 撮影 `site/screenshots/*.webp` を per-image diff ≤ 10% で gate (#2401, 2026-05-23)
- **ADR-0055**: [Per-child 主軸 + 限定 family master データモデル原則 (6 type SSOT)](../docs/decisions/0055-per-child-primary-data-model-pattern.md) — EPIC #2362 (PO 4 問題) 解決のための marketplace 6 type データモデル scope 選択 SSOT。activity / reward (exchange) / challenge は per-child instance 主軸、checklist template / rule (bonus) は family master。`data-model-resource-scope.md` / `marketplace-import-flow.md` が下流設計書 SSOT。後続 PR-3〜7 で schema 変更時に参照 (2026-05-23、当初 ADR-0053 → renumber)
