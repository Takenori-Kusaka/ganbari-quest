...
- **ADR-0017**: [Cognito User Pool 再作成による email mutable 化](../docs/decisions/0017-cognito-pool-recreation-email-mutable.md) — `email: { mutable: true }` に変更、User Pool を CloudFormation で再作成。Google OAuth 再認証時の `user.email: Attribute cannot be updated` を構造解消。Pre-PMF で実ユーザー数 < 10 の破壊的変更許容境界での判断 (#1366)
