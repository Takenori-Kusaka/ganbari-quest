...
> 統合 PR は単一 Issue に紐づきません (feature 用の `closes #<single>` を持ちません)。代わりに
> **含有 PR が `## 関連 Issue` で宣言した `closes/fixes/resolves #N` を `integration-pr-body.mjs` が
> `Closes #N` に集約**し、main 反映で該当 issue を一括 auto-close します (#3423、close漏れ防止)。
> 集約は section 内の行頭 closing keyword のみ (code fence / 引用 / 否定文中の参照は除外、#3444)。
...
merge commit (subject digest = sha1:<merge sha>) に紐付き、GH attestations API で改ざん検知付きで遡れる。
注: attestation の subject は git commit のため `gh attestation verify` (file / oci:// 専用) は使えない。
commit subject の照会は GH attestations API (`gh api .../attestations/sha1:<sha>`) で行う。
-->
...
- attestation (merge 後): `integration-attestation-<run_id>` artifact + GH attestations API
  (`gh api repos/<owner>/<repo>/attestations/sha1:<merge sha> --jq '.attestations[].bundle.dsseEnvelope.payload' | base64 -d | jq .`)
...
<!--
#3487 / ADR-0061 原則 5 (accepted-residual gate)。adversarial が生産した 3 件のうち、
filter (重複/severity/ポリシー準拠) 通過後も **Issue 化せず Pre-PMF で受容**する marginal な
残課題をここに記録する (生産≠起票)。これにより「1 PR ≈ 1 follow-up」の treadmill を断つ。
**ガード**: severity ≥ high (3-4) はここに書けない (必ず blocking で解消 or Issue 起票)。
class-lock 対象 (同 root class 2 instance 目) は instance Issue を repeat せず class 全体を
1 件の fitness function/property test で lock する (audit-team.md §3.6)。
0 件なら「_受容 residual なし_」と記す。
-->

| finding (要約) | severity (1-2 のみ) | 受容理由 (Pre-PMF) | 関連 root class |
|---|---|---|---|
| <!-- 例: reset deleted-count の命名精度 --> | <!-- 2 --> | <!-- dev-only 診断値、ユーザー価値影響なし --> | <!-- reset 完全性 --> |
